...
---
### <a name="most-code-access-security-apis-are-obsolete"></a>Die meisten Codezugriffssicherheit-APIs sind veraltet

Die meisten CAS-bezogenen Typen (Codezugriffssicherheit) in .NET sind mittlerweile als Warnung veraltet. Dies gilt auch für CAS-Attribute wie <xref:System.Security.Permissions.SecurityPermissionAttribute>, CAS-Berechtigungsobjekte wie <xref:System.Net.SocketPermission>, von <xref:System.Security.Policy.EvidenceBase> abgeleitete Typen und weitere unterstützende APIs.
...
Außerdem sind programmgesteuerte Aufrufe von expansiven APIs (`Assert`) immer erfolgreich, während programmgesteuerte Aufrufe von restriktiven APIs (`Deny` und `PermitOnly`) zur Laufzeit immer eine Ausnahme auslösen. (<xref:System.Security.Permissions.PrincipalPermission> ist die Ausnahme von dieser Regel. Lesen Sie den Abschnitt [Empfohlene Maßnahme](#cas-action) weiter unten.)
...
In .NET 5.0 und höheren Versionen sind die meisten CAS-bezogenen APIs veraltet und lösen zur Kompilierzeit die Warnung `SYSLIB0003` aus.
...
- Wenn Sie diese Warnungen unbedingt deaktivieren müssen (was nicht empfohlen wird), können Sie die Warnung `SYSLIB0003` im Code unterdrücken.
  ...
  > [!NOTE]
  > Durch das Unterdrücken von `SYSLIB0003` werden nur CAS-bezogene Veraltungswarnungen deaktiviert. Es werden keine anderen Warnungen deaktiviert, und das Verhalten der Runtime von .NET 5.0 oder höheren Versionen wird nicht geändert.
