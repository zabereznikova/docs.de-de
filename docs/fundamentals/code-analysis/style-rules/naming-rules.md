...
| `severity` |  Legt den Schweregrad fest, mit dem die Benennungs Regel erzwungen werden soll. Legen Sie den zugeordneten Wert auf einen der verfügbaren [Schweregrade](../configuration-options.md#severity-level)fest. <sup>1</sup> |
...
Ein Benennungs Stil definiert die Konventionen, die Sie mit der Regel erzwingen möchten. Beispiel:
