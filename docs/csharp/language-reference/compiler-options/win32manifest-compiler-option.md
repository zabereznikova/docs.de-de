...
## Hinweise  
 Standardmäßig bettet der [!INCLUDE[csharp_current_short](../../../csharp/language-reference/compiler-options/includes/csharp-current-short-md.md)]\-Compiler ein Anwendungsmanifest ein, das die angeforderte Ausführungsebene "asInvoker" angibt. Sie erstellt das Manifest im gleichen Ordner, in dem die ausführbare Datei erstellt wird, in der Regel der Ordner bin\\Debug oder bin\\Release, wenn Sie Visual Studio verwenden.  Wenn Sie einer Zollerklärung angeben möchten, beispielsweise, um einer angeforderten Ausführungsebene anzugeben, die von "highestAvailable" oder "vom requireAdministrator, verwenden Sie diese Option, den Namen der Datei an.  
