...
Damit aufrufende Funktionen nicht den Ursprung verändern, sollten Sie den Wert als `readonly ref` zurückgeben:
...
An der Aufrufstelle können die aufrufenden Funktionen die `Origin`-Eigenschaft entweder als `readonly ref` oder als Wert verwenden:
