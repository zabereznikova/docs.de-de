...
Sie könnten sich in einer Situation befinden, in der Sie mehrere Daten gleichzeitig abrufen müssen.  Die `Task` -API enthält zwei Methoden `Task.WhenAll` und `Task.WhenAny` ermöglichen Sie asynchronen Code schreiben, die einen nicht blockierenden Wartevorgang für mehrere Hintergrundaufträge ausführt.
