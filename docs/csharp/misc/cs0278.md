...
Es gibt mehrere Anweisungen in C#, die auf definierten Muster aufsetzen, z. B. `foreach` und `using`. Z. B. die [ `foreach` Anweisung](../language-reference/keywords/foreach-in.md) basiert auf der Auflistungsklasse auf das "enumerable"-Muster implementiert.
