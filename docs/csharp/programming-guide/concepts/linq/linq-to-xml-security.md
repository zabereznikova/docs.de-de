---
title: LINQ to XML-Sicherheit (C#) | Microsoft-Dokumentation
ms.custom: 
ms.date: 2015-07-20
ms.prod: .net
ms.reviewer: 
...
## <a name="linq-to-xml-security-overview"></a>Übersicht über die Sicherheit in LINQ to XML  
 ...
 Wenn Sie nicht vertrauenswürdige Daten aus unbekannten Quellen verarbeiten müssen, wird empfohlen, eine Instanz der Klasse <xref:System.Xml.XmlReader> zu verwenden, die zum Herausfiltern bekannter XML-DoS-Angriffe (Denial of Service) konfiguriert wurde.  
...
## <a name="mitigation-of-xml-xsd-xpath-and-xslt-attacks"></a>Abwehr von XML-, XSD-, XPath- und XSLT-Angriffen  
 LINQ to XML basiert auf <xref:System.Xml.XmlReader>und <xref:System.Xml.XmlWriter>. LINQ to XML unterstützt XSD und XPath durch Erweiterungsmethoden in den Namespaces <xref:System.Xml.Schema?displayProperty=fullName> und <xref:System.Xml.XPath?displayProperty=fullName>. Mithilfe der Klassen <xref:System.Xml.XmlReader>, <xref:System.Xml.XPath.XPathNavigator> und <xref:System.Xml.XmlWriter> können Sie zusammen mit LINQ to XML XSLT zum Transformieren von XML-Strukturen aufrufen.  
  
 Wenn Sie in einer unsichereren Umgebung arbeiten, gibt es einige Sicherheitsprobleme, die mit XML und der Verwendung der Klassen in <xref:System.Xml?displayProperty=fullName>, <xref:System.Xml.Schema?displayProperty=fullName>, <xref:System.Xml.XPath?displayProperty=fullName> und <xref:System.Xml.Xsl?displayProperty=fullName> einhergehen. Dazu gehören u. a. folgende Probleme:  
...
-   Akzeptieren Sie keine unterstützende Komponente von nicht vertrauenswürdigen Assemblys, wie z.B. die Objekte <xref:System.Xml.NameTable>, <xref:System.Xml.XmlNamespaceManager> und <xref:System.Xml.XmlResolver>.  
...
### <a name="avoid-excessive-buffer-allocation"></a>Vermeiden Sie übermäßige Pufferzuordnung  
 ...
 Wenn ein böswilliger Benutzer ein sehr großes XML-Dokument sendet oder hochlädt, kann das dazu führen, dass LINQ to XML übermäßig viele Systemressourcen verbraucht. Dies kann die Grundlage für einen DoS-Angriff sein. Um dies zu verhindern, können Sie die Eigenschaft <xref:System.Xml.XmlReaderSettings.MaxCharactersInDocument%2A?displayProperty=fullName> festlegen und erstellen einen Reader, der anschließend auf die Größe des Dokuments beschränkt wird, die er laden kann. Anschließend erstellen Sie mit dem Reader die XML-Struktur.  
  
 Wenn Sie z.B. wissen, dass sich die maximal zu erwartende Größe Ihrer von einer nicht vertrauenswürdigen Quelle stammenden XML-Dokumente auf unter 50 Kilobytes beläuft, legen Sie für <xref:System.Xml.XmlReaderSettings.MaxCharactersInDocument%2A?displayProperty=fullName> den Wert „100.000“ fest. Die Verarbeitung der XML-Dokumente wird davon nicht negativ beeinflusst, und gleichzeitig verringert sich das Risiko von DoS-Angriffen, bei denen Dokumente hochgeladen werden, die große Mengen des Speichers in Anspruch nehmen.  
  
### <a name="avoid-excess-entity-expansion"></a>Vermeiden Sie eine exzessive Entitätserweiterung  
 Ein bekannter DoS-Angriff mit einer DTD besteht in der Verwendung eines Dokuments, das eine exzessive Entitätserweiterung zur Folge hat. Um dies zu verhindern, können Sie die Eigenschaft <xref:System.Xml.XmlReaderSettings.MaxCharactersFromEntities%2A?displayProperty=fullName> festlegen und einen Reader erstellen, der anschließend in der Anzahl von Zeichen beschränkt wird, die aus einer Entitätserweiterung entstehen. Anschließend erstellen Sie mit dem Reader die XML-Struktur.  
...
### <a name="protect-against-untrusted-xmlreader-or-xmlwriter-implementations"></a>Schützen Sie sich vor nicht vertrauenswürdigen "XmlReader"- oder "XmlWriter"-Implementierungen  
 Administratoren müssen sicherstellen, dass alle extern bereitgestellten <xref:System.Xml.XmlReader>- oder <xref:System.Xml.XmlWriter>-Implementierungen starke Namen besitzen und in der Computerkonfiguration registriert sind. Auf diese Weise wird verhindert, dass bösartiger Code geladen wird, der sich als Reader oder Writer maskiert.  
  
### <a name="periodically-free-objects-that-reference-xname"></a>Geben Sie regelmäßig Objekte frei, die auf "XName" verweisen  
 Anwendungsprogrammierer sollten in regelmäßigen Abständen alle Objekte freigeben, um sich vor bestimmten Arten von Angriffen zu schützen, die auf ein <xref:System.Xml.Linq.XName>-Objekt in der Anwendungsdomäne verweisen.  
  
### <a name="protect-against-random-xml-names"></a>Schützen Sie sich vor zufälligen XML-Namen  
 Für Anwendungen, die Daten aus nicht vertrauenswürdigen Quellen akzeptieren, sollte die Verwendung eines <xref:System.Xml.XmlReader>-Objekts in Erwägung gezogen werden, das von benutzerdefiniertem Code umschlossen wird und nach möglichen zufälligen XML-Namen und -Namespaces sucht. Wenn solche zufälligen XML-Namen und -Namespaces entdeckt werden, kann die Anwendung die Verarbeitung des bösartigen Dokuments abbrechen.  
