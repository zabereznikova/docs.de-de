---
title: Typen (C#-Programmierhandbuch) | Microsoft-Dokumentation
ms.date: 2015-07-20
ms.prod: .net
ms.technology:
- devlang-csharp
...
-   Es unterstützt das Prinzip der Vererbung. Typen können von anderen Typen abgeleitet werden, die als *Basistypen* bezeichnet werden. Der abgeleitete Typ erbt (mit einigen Beschränkungen) die Methoden, Eigenschaften und anderen Member des Basistyps. Der Basistyp kann wiederum von einem anderen Typ abgeleitet sein. In diesem Fall erbt der abgeleitete Typ die Member beider Basistypen in der Vererbungshierarchie. Alle Typen, einschließlich integrierter numerischer Typen wie „<xref:System.Int32?displayProperty=fullName>“ (C#-Schlüsselwort: [int](../../../csharp/language-reference/keywords/int.md)), leiten sich letztendlich von einem einzigen Basistyp ab, der „<xref:System.Object?displayProperty=fullName>“ (C#-Schlüsselwort: [object](../../../csharp/language-reference/keywords/object.md)) lautet. Diese einheitliche Typhierarchie wird als [Allgemeines Typsystem](http://msdn.microsoft.com/library/53c57c96-83e1-4ee3-9543-9ac832671a89) (CTS) bezeichnet. Weitere Informationen zur Vererbung in C# finden Sie unter [Vererbung](../../../csharp/programming-guide/classes-and-structs/inheritance.md).  
...
-   Jeder Typ im CTS ist als *Werttyp* oder *Referenztyp* definiert. Dies betrifft auch alle benutzerdefinierten Typen in der .NET Framework-Klassenbibliothek und Ihre eigenen benutzerdefinierten Typen. Typen, die Sie mithilfe des [struct](../../../csharp/language-reference/keywords/struct.md)-Schlüsselworts definieren, sind Werttypen. Alle integrierten numerischen Typen sind `structs`. Typen, die Sie mithilfe des [class](../../../csharp/language-reference/keywords/class.md)-Schlüsselworts definieren, sind Referenztypen. Für Referenztypen und Werttypen gelten unterschiedliche Kompilierzeitregeln und ein anderes Laufzeitverhalten.  
 ...
 ![Werttypen und Verweistypen](../../../csharp/programming-guide/types/media/valuetypescts.png "ValueTypesCTS")  
...
### <a name="value-types"></a>Werttypen  
 Werttypen leiten sich vom <xref:System.ValueType?displayProperty=fullName>-Element ab, das sich wiederum vom <xref:System.Object?displayProperty=fullName>-Element ableitet. Typen, die sich vom <xref:System.ValueType?displayProperty=fullName>-Element ableiten, weisen ein besonderes Verhalten in der CLR auf. Werttypvariablen enthalten die zugehörigen Werte direkt, d. h., der Speicher wird inline in dem Kontext belegt, in dem die Variable deklariert ist. Für Werttypvariablen erfolgt keine getrennte Heapzuordnung oder ein Mehraufwand für die Garbage Collection.  
...
```  
 ...
 Werttypen sind *versiegelt*. Das bedeutet beispielsweise, dass Sie keinen Typ von „<xref:System.Int32?displayProperty=fullName>“ ableiten und für eine Struktur nicht definieren können, dass sie von einer beliebigen benutzerdefinierten Klasse oder Struktur erbt, weil eine Struktur nur von „<xref:System.ValueType?displayProperty=fullName>“ erben kann. Eine Struktur kann jedoch eine oder mehrere Schnittstellen implementieren. Sie können einen Strukturtyp in einen Schnittstellentyp umwandeln. Dadurch wird ein *Boxing*-Vorgang gestartet, mit dem die Struktur von einem Referenztypobjekt im verwalteten Heap umschlossen wird. Boxing-Vorgänge erfolgen, wenn Sie einen Werttyp an eine Methode übergeben, die „<xref:System.Object?displayProperty=fullName>“ als Eingabeparameter akzeptiert. Weitere Informationen finden Sie unter [Boxing und Unboxing](../../../csharp/programming-guide/types/boxing-and-unboxing.md).  
 ...
 Weitere Informationen über Strukturen finden Sie unter [Strukturen](../../../csharp/programming-guide/classes-and-structs/structs.md). Weitere Informationen über Werttypen in [!INCLUDE[dnprdnshort](../../../csharp/getting-started/includes/dnprdnshort_md.md)] finden Sie unter [Allgemeines Typsystem](http://msdn.microsoft.com/library/53c57c96-83e1-4ee3-9543-9ac832671a89).  
 ...
 Alle Enumerationen erben vom <xref:System.Enum?displayProperty=fullName>-Element, das wiederum vom <xref:System.ValueType?displayProperty=fullName>-Element erbt. Alle Regeln, die für Strukturen gelten, gelten auch für Enumerationen. Weitere Informationen über Enumerationen finden Sie unter [Enumerationstypen](../../../csharp/programming-guide/enumeration-types.md).  
...
## <a name="types-of-literal-values"></a>Typen von Literalwerten  
 ...
 Da Literale typisiert sind und sich alle Typen letztendlich vom <xref:System.Object?displayProperty=fullName>-Element ableiten, können Sie Code wie den Folgenden schreiben und kompilieren:  
...
## <a name="generic-types"></a>Generische Typen  
 Ein Typ kann mit einem oder mehreren *Typparametern* deklariert werden, die als Platzhalter für den eigentlichen Typ verwendet werden (den *konkreten Typ*), der vom Clientcode beim Erstellen einer Instanz des Typs bereitgestellt wird. Solche Typen werden als *generische Typen* bezeichnet. Beispiel: Der .NET Framework-Typ „<xref:System.Collections.Generic.List%601?displayProperty=fullName>“ verfügt über einen Typparameter, der nach Konvention den Namen *T* erhält. Beim Erstellen einer Instanz des Typs geben Sie die Objekte an, die die Liste enthalten soll, z. B. string:  
...
## <a name="implicit-types-anonymous-types-and-nullable-types"></a>Implizite Typen, anonyme Typen und Typen, die NULL-Werte zulassen  
 ...
 Gewöhnliche Werttypen können den Wert [NULL](../../../csharp/language-reference/keywords/null.md) nicht aufweisen. Sie können jedoch auf NULL festlegbare Werttypen erstellen, indem Sie nach dem Typ ein `?` anfügen. Zum Beispiel ist `int?` ein `int`-Typ, der auch den Wert [NULL](../../../csharp/language-reference/keywords/null.md) aufweisen kann. In CTS sind Nullable-Typen Instanzen mit dem generischen Strukturtyp „<xref:System.Nullable%601?displayProperty=fullName>“. Typen, die NULL-Werte zulassen, sind besonders hilfreich, wenn Sie Daten an und aus Datenbanken übergeben, in denen die numerischen Werte NULL sein können. Weitere Informationen finden Sie unter [Nullable-Typen](../../../csharp/programming-guide/nullable-types/index.md).  
...
-   [Variablen und Ausdrücke](http://go.microsoft.com/fwlink/?LinkId=221228) in [Einstieg in Visual C# 2010](http://go.microsoft.com/fwlink/?LinkId=221214)  
...
## <a name="see-also"></a>Siehe auch  
 ...
 [Konvertierung von XML-Datentypen](http://msdn.microsoft.com/library/a2aa99ba-8239-4818-9281-f1d72ee40bde)   
