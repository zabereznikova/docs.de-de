...
---
# <a name="overview-of-markup-extensions-for-xaml"></a>Übersicht über Markuperweiterungen für XAML

Markuperweiterungen sind eine XAML-Technik zum Abrufen eines Werts, der kein primitiver oder ein bestimmter XAML-Typ ist. Für die Attributverwendung verwenden Markuperweiterungen die bekannte Zeichensequenz einer öffnenden geschweiften Klammer `{` für den Anfang des Markuperweiterungsbereichs und eine schließende geschweifte Klammer `}` zum Beenden. Wenn Sie .NET XAML Services verwenden, können Sie einige der vordefinierten XAML-Sprachmarkuperweiterungen aus der System.Xaml-Assembly verwenden. Sie können zudem in „System.Xaml“ definierte Subklassen aus der Klasse <xref:System.Windows.Markup.MarkupExtension> verwenden und Ihre eigenen Markuperweiterungen definieren. Sie können auch Markuperweiterungen verwenden, die durch ein bestimmtes Framework definiert sind, wenn Sie bereits auf dieses Framework verweisen.
...
Mehrere Markuperweiterungen werden von .NET XAML Services für XAML-Sprachunterstützung implementiert. Diese Markuperweiterungen entsprechen Teilen der Spezifikation von XAML als Sprache. Diese lassen sich für gewöhnlich durch das `x:` -Präfix in der Syntax bestimmen, wie dies unter der häufigen Verwendung zu sehen ist. .NET XAML Services-Implementierungen für diese XAML-Sprachelemente stammen alle von der <xref:System.Windows.Markup.MarkupExtension> Basisklasse ab.

> [!NOTE]
> Das Präfix `x:` wird für die typische XAML-Namespacezuordnung des XAML-Sprachnamespace im Stammelement einer XAML-Produktion verwendet. Beispielsweise initiieren das Visual Studio-Projekt und die Seitenvorlagen für verschiedene spezifische `x:` Frameworks eine XAML-Datei mithilfe dieser Zuordnung. Sie können ein anderes Präfixtoken in Ihrer XAML-Namespacezuordnung auswählen. In dieser Dokumentation wird jedoch von der standardmäßigen `x:` -Zuordnung im Zuge der Ermittlung dieser Entitäten ausgegangen, die im Gegensatz zum standardmäßigen XAML-Namespace eines bestimmten Frameworks oder anderer willkürlicher CLR- oder XML-Namespaces ein definierter Bestandteil des XAML-Sprachennamespace sind.
...
Es sind weitere `x:` -Konstrukte für die Unterstützung von XAML-Sprachfeatures vorhanden. Diese sind jedoch nicht als Markuperweiterungen implementiert. Weitere Informationen finden Sie unter [XAML Namespace (x:) Language Features](namespace-language-features.md).
...
Durch <xref:System.Windows.Markup.MarkupExtension.ProvideValue%2A>hat eine benutzerdefinierte Markuperweiterung Zugriff auf einen Dienstkontext, der die Umgebung meldet, in der die Markuperweiterung von einem XAML-Prozessor aufgerufen wird. Im Ladepfad ist dies <xref:System.Xaml.XamlObjectWriter>in der Regel eine . Im Speicherpfad ist dies für gewöhnlich ein <xref:System.Xaml.XamlXmlWriter>. Jeder Bericht der Dienstkontext als eine interne XAML-Dienstanbieter-Kontextklasse, die ein Dienstanbietermuster implementiert. Weitere Informationen über die verfügbaren Dienste und was sie darstellen finden Sie unter [Type Converters and Markup Extensions for XAML](type-converters-and-markup-extensions.md).
...
Wenn Sie .NET XAML Services oder Frameworks verwenden, die auf .NET XAML Services aufbauen, haben Sie zwei Möglichkeiten, wie Sie den Unterstützungstyp für die Markuperweiterung benennen. Der Typname ist relevant für die Art und Weise des Zugriffsversuchs und Aufrufs von XAML-Objekt-Writern auf einen Markup-Erweiterungsunterstützungstyp, wenn sie eine Markuperweiterungsverwendung in XAML feststellen. Verwenden Sie eine der folgenden Strategien für die Benennung:
...
### <a name="the-parameterless-constructor"></a>Der parameterlose Konstruktor

Für alle Unterstützungstypen für Markuperweiterungen sollten Sie einen öffentlichen parameterlosen Konstruktor verfügbar machen. Ein parameterloser Konstruktor ist für jeden Fall erforderlich, in dem ein XAML-Objektschreiber die Markuperweiterung aus einer Objektelementverwendung instanziiert. Das Unterstützen der Objektelementverwendung ist eine angemessene Erwartung für eine Markuperweiterung, insbesondere für die Serialisierung. Sie können jedoch eine Markuperweiterung ohne einen öffentlichen Konstruktor implementieren, wenn Sie nur die Attributverwendungen der Markuperweiterung unterstützen möchten.

Wenn Ihre Markuperweiterungsverwendung keine Argumente enthält, ist der parameterlose Konstruktor erforderlich, um die Verwendung zu unterstützen.
...
- Wie bereits erwähnt, muss ein Markuperweiterungsunterstützungstyp keinen parameterlosen Konstruktor haben, um in XAML instanziiert zu werden. Seine Objektkonstruktion wird verzögert, bis seine möglichen Argumente in der Textsyntax als Positions- oder Namensargumente in Token übersetzt und ausgewertet werden und der entsprechende Konstruktor zu diesem Zeitpunkt aufgerufen wird.
...
Der Einsatz dieser Verarbeitung wurde im vorherigen Beispiel gezeigt. .NET XAML Services XAML-Objektschreiber verarbeitet Enumerationskonstantennamen in aufgezählten Werten auf systemeigener Ebene.
...
### <a name="duplicate-arity-of-positional-arguments"></a>Doppelte Arität von Positionsargumenten
...
## <a name="property-element-usage-of-a-markup-extension"></a>Eigenschaftenelementverwendung einer Markuperweiterung
...
Um die Verwendung von Eigenschaftenelementen ihrer Markuperweiterung zu unterstützen, definieren Sie einen öffentlichen parameterlosen Konstruktor. Hierbei sollte es sich um einen Instanzkonstruktor und nicht um einen statischen Konstruktor handeln. Dies ist erforderlich, da ein XAML-Prozessor im Allgemeinen den parameterlosen Konstruktor für jedes Objektelement aufrufen muss, das er aus Markup verarbeitet, und dies schließt Markuperweiterungsklassen als Objektelemente ein. Für erweiterte Szenarien können Sie nicht standardmäßige Konstruktionspfade für Klassen definieren. (Weitere Informationen finden Sie unter [x:FactoryMethod Directive](xfactorymethod-directive.md).) Sie sollten diese Muster jedoch nicht für Markuperweiterungszwecke verwenden, da dies die Erkennung des Verwendungsmusters sowohl für Designer als auch für Benutzer von Rohmarkup erheblich erschwert.

## <a name="attributing-for-a-custom-markup-extension"></a>Attributieren für eine benutzerdefinierte Markuperweiterung
