 ...
 **X AVOID** Unterstützung der Serialisierung zur Laufzeit oder XML-Serialisierung nur für die allgemeine Persistenz Gründe. Verwenden Sie stattdessen Datenvertragsserialisierung.  
 ...
 **✓ CONSIDER** Serialisierung zur Laufzeit unterstützen, wenn die Typen mit .NET Remoting verwendet werden. Z. B. die <xref:System.AddIn?displayProperty=nameWithType> Namespace verwendet .NET Remoting und daher sind alle Typen ausgetauscht werden, zwischen `System.AddIn` -add-ins Serialisierung zur Laufzeit unterstützt werden müssen.  
