---
title: "&#196;ndern von XML-Daten mit &quot;XPathNavigator&quot; | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
|---------------------------------------------------------------------------------------------------------------------------------------------|---------------------|  
|<xref:System.Xml.XPath.XPathNodeType>|Wird nicht unterstützt.|  
|<xref:System.Xml.XPath.XPathNodeType>|Der Inhalt des Elements.|  
|<xref:System.Xml.XPath.XPathNodeType>|Der Wert des Attributs.|  
|<xref:System.Xml.XPath.XPathNodeType>|Der Textinhalt.|  
|<xref:System.Xml.XPath.XPathNodeType>|Der Inhalt mit Ausnahme des Ziels.|  
|<xref:System.Xml.XPath.XPathNodeType>|Der Inhalt des Kommentars.|  
|<xref:System.Xml.XPath.XPathNodeType>|Nicht unterstützt.|  
  
> [!NOTE]
>  Das Bearbeiten von <xref:System.Xml.XPath.XPathNodeType>\-Knoten oder des <xref:System.Xml.XPath.XPathNodeType>\-Knotens wird nicht unterstützt.  
  
 Die <xref:System.Xml.XPath.XPathNavigator>\-Klasse stellt ebenfalls eine Gruppe von Methoden zum Einfügen und Entfernen von Knoten bereit.  Weitere Informationen zum Einfügen und Entfernen von Knoten aus einem XML\-Dokument finden Sie unter dem [Einfügen von XML\-Daten mit "XPathNavigator"](../../../../docs/standard/data/xml/insert-xml-data-using-xpathnavigator.md)\-Thema und dem [Entfernen von XML\-Daten mit "XPathNavigator"](../../../../docs/standard/data/xml/remove-xml-data-using-xpathnavigator.md)\-Thema.  
...
>  Attribute können nur einfache Typen haben.  
 ...
 Da die Gültigkeit eines Elements von der Gültigkeit seiner untergeordneten Elemente und Attribute abhängt, führen Änderungen dazu, dass sich die Gültigkeit des Elements ändert, wenn es vorher gültig war.  Insbesondere wird die Gültigkeit eines Elements unbekannt, wenn untergeordnete Elemente oder Attribute des Elements eingefügt, aktualisiert oder gelöscht werden.  Dies wird durch Festlegen der <xref:System.Xml.Schema.IXmlSchemaInfo.Validity%2A>\-Eigenschaft der <xref:System.Xml.XPath.XPathNavigator.SchemaInfo%2A>\-Eigenschaft des Elements auf <xref:System.Xml.Schema.XmlSchemaValidity> dargestellt.  Darüber hinaus setzt sich diese Auswirkung rekursiv aufwärts innerhalb des XML\-Dokuments fort, da die Gültigkeit des übergeordneten Elements \(und des diesem Element übergeordneten Elements usw.\) eines Elements ebenfalls unbekannt wird.  
  
 Weitere Informationen über Schemavalidierung und die <xref:System.Xml.XPath.XPathNavigator>\-Klasse finden Sie unter [Schema\-Validierung mithilfe von "XPathNavigator"](../../../../docs/standard/data/xml/schema-validation-using-xpathnavigator.md).  
...
## Die Eigenschaften "InnerXml" und "OuterXml"  
 ...
 Im folgenden Beispiel wird mit der <xref:System.Xml.XPath.XPathNavigator.OuterXml%2A>\-Eigenschaft der Wert des `price`\-Elements geändert und in das erste `book`\-Element der Datei `contosoBooks.xml` ein neues `discount`\-Attribut eingefügt.  
...
## Ändern von xsi:nil\-Knoten  
 ...
 Wenn einem gültigen Element, dessen `xsi:nil`\-Attribut den Wert `true` hat, mittels eines <xref:System.Xml.XPath.XPathNavigator>\-Objekts Inhalt hinzugefügt wird, wird der Wert des `xsi:nil`\-Attributs auf `false` festgelegt.  
