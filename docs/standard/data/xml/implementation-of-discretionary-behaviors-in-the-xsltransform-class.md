...
|Ergebnisse der Instanziierung des Inhalts von den `xsl:comment` enthält die Zeichenfolge "--", oder enden mit "-".|Wiederherstellen|7.4|  
...
-   Unterschiedliche Prozessoren können für eine bestimmte Sprache unterschiedliche Sortierungen durchführen, die nicht in `xsl:sort.` angegeben sind.  
 ...
 In der folgenden Tabelle wird das Sortierverhalten dargestellt implementiert, die für jeden Datentyp in der .NET Framework-Implementierung einer Transformation mit <xref:System.Xml.Xsl.XslTransform>.  
