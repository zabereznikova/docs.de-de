...
Es gibt einen bemerkenswerten Unterschied zwischen dem Entladen mithilfe von `AssemblyLoadContext` und der Verwendung von AppDomains. Mit AppDomains wird das Entladen erzwungen. Zum Zeitpunkt der Entladung werden beispielsweise alle Threads abgebrochen, die in der Zielanwendungsdomäne ausgeführt werden, und verwaltete COM-Objekte werden gelöscht, die in der Ziel-AppDomain erstellt wurden. Mit `AssemblyLoadContext` ist den Entladevorgang „kooperativ“. Durch Aufrufen der <xref:System.Runtime.Loader.AssemblyLoadContext.Unload%2A?displayProperty=nameWithType>-Methode wird der Entladevorgang nur initiiert. Das Entladen wird abgeschlossen, nachdem die folgenden Bedingungen erfüllt sind:
...
Der folgende Code stellt die `test.dll` dar, die an die `ExecuteAndUnload`-Methode im Haupttestprogramm übergeben wird.
