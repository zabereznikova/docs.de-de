...
Um bestimmte APIs aus Ihrem Projekt zu verwenden, müssen Sie Verweise auf deren Assemblys hinzufügen. Sie können Verweise direkt Implementierungsassemblys oder Verweisassemblys hinzufügen. Es wird dringend empfohlen, immer dann Verweisassemblys zu nutzen, wenn sie verfügbar sind, da so sichergestellt wird, dass Sie nur API-Member verwenden, die in der Zielversion unterstützt werden und für die Verwendung durch API-Designer vorgesehen sind (die also nicht von Implementierungsdetails abhängig sind).
...
Da sie keine Implementierung enthalten, können Verweisassemblys nicht zur Ausführung geladen werden. Der Versuch, sie zu laden, führt zu einer <xref:System.BadImageFormatException?displayProperty=nameWithType>. Sie können jedoch trotzdem in den reinen Reflektionskontext (mit der <xref:System.Reflection.Assembly.ReflectionOnlyLoad%2A?displayProperty=nameWithType>-Methode) geladen werden, wenn Sie ihren Inhalt untersuchen müssen.
...
Die Metadaten in Verweisassemblys behalten die folgenden Informationen bei:
