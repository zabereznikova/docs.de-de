...
Docker-Container können in Ihrer Linux- oder Windows-Infrastruktur oder in einem Clouddienst wie [Azure Container Service](https://azure.microsoft.com/services/container-service/) gehostet werden. Azure Container Service kann containerbasierte Anwendungen in der Cloud verwalten, orchestrieren und skalieren.
...
* ASP.NET Web Pages-Anwendungen: ASP.NET Web Pages ist nicht in ASP.NET Core enthalten. Die [Razor-Seiten](/aspnet/core/mvc/razor-pages/) von ASP.NET Core ähneln Web Pages.

* Implementierung von ASP.NET SignalR-Server/-Client. Aktuell ist ASP.NET SignalR für ASP.NET Core nicht verfügbar (weder Client noch Server). ASP.NET Core SignalR ist für ASP.NET Core 2.1 geplant. Weitere Informationen finden Sie unter [ASP.NET Core Schedule and Roadmap (Zeitplan und Meilensteinplan für ASP.NET Core)](https://github.com/aspnet/Home/wiki/Roadmap). Die Vorschau ist für die GitHub-Repositorys sowohl für die [Serverseite](https://github.com/aspnet/SignalR-Server) als auch für die [Clientbibliotheken](https://github.com/aspnet/SignalR-Client-Net) verfügbar.
...
* Workflowbezogene Dienste: Windows Workflow Foundation (WF), Workflow Services (WCF + WF in einem einzigen Dienst) und WCF Data Services (zuvor bekannt als „ADO.NET Data Services“) sind nur in .NET Framework verfügbar.  Eine Integration von WF/WCF+WF/WCF Data Services in .NET Core ist nicht geplant.
...
* Sprachunterstützung: Visual Basic und F# werden aktuell in .NET Core unterstützt, allerdings nicht für alle Projekttypen. Eine Liste der unterstützten Projektvorlagen finden Sie im Abschnitt [Vorlagenoptionen für dotnet new](../core/tools/dotnet-new.md#arguments).
