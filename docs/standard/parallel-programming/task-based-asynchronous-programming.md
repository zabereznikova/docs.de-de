---
title: Aufgabenbasierte asynchrone Programmierung
ms.custom: 
ms.date: 03/30/2017
ms.prod: .net
ms.reviewer: 
...
> [!IMPORTANT]
>  Beachten Sie, dass die Kultur des aufrufenden Threads als Bestandteil des Kontexts einer Aufgabe gilt für Anwendungen, die *gezielt* [!INCLUDE[net_v46](../../../includes/net-v46-md.md)] verwenden, und nicht für Anwendungen, deren *Ausführung unter* [!INCLUDE[net_v46](../../../includes/net-v46-md.md)] erfolgt. Sie können eine bestimmte Version von .NET Framework abzielen, wenn Sie Ihr Projekt in Visual Studio erstellen, durch Auswahl von dieser Version aus der Dropdownliste am oberen Rand der **neues Projekt** (Dialogfeld), oder außerhalb von Visual Studio Sie können die <xref:System.Runtime.Versioning.TargetFrameworkAttribute> Attribut. Für Anwendungen, die gezielt Versionen von .NET Framework vor der [!INCLUDE[net_v46](../../../includes/net-v46-md.md)] verwenden oder die keine bestimmte Version von .NET Framework gezielt verwenden, wird die Kultur einer Aufgabe weiterhin durch die Kultur des Threads bestimmt, über den sie ausgeführt werden.  
 ...
 Wenn Sie Visual Studio verwenden, können Sie auslassen der <xref:System.Runtime.Versioning.TargetFrameworkAttribute> Attribut, und stattdessen .NET Framework 4.6 als Ziel wählen, bei der Erstellung des Projekts in der **neues Projekt** Dialogfeld.  
...
## <a name="creating-task-continuations"></a>Erstellen von Aufgabenfortsetzungen  
 Die <xref:System.Threading.Tasks.Task.ContinueWith%2A?displayProperty=nameWithType> und <xref:System.Threading.Tasks.Task%601.ContinueWith%2A?displayProperty=nameWithType> Methoden können Sie angeben, eine Aufgabe, um beim Starten der *Vorgängeraufgabe* abgeschlossen ist. An den Delegaten der Fortsetzungsaufgabe wird ein Verweis auf die vorherige Aufgabe übergeben, damit dieser den Status der vorherigen Aufgabe überprüfen kann und durch Abruf des Werts der <xref:System.Threading.Tasks.Task%601.Result%2A?displayProperty=nameWithType>-Eigenschaft die Ausgabe der vorherigen Aufgabe als Eingabe für die Fortsetzung verwenden kann.  
...
## <a name="waiting-for-tasks-to-finish"></a>Warten auf die Beendigung von Aufgaben  
 Die <xref:System.Threading.Tasks.Task?displayProperty=nameWithType> und <xref:System.Threading.Tasks.Task%601?displayProperty=nameWithType> -Typ bieten mehrere Überladungen, die von der <xref:System.Threading.Tasks.Task.Wait%2A?displayProperty=nameWithType> und <!--zz <xref:System.Threading.Tasks.Task%601.Wait%2A?displayProperty=nameWithType>--> `System.Threading.Tasks.Task.Wait` Methoden, die Ihnen ermöglichen, auf den Abschluss einer Aufgabe gewartet. Außerdem können Sie mittels Überladungen der statischen <xref:System.Threading.Tasks.Task.WaitAll%2A?displayProperty=nameWithType>-Methode und der <xref:System.Threading.Tasks.Task.WaitAny%2A?displayProperty=nameWithType>-Methode auf den Abschluss einer bestimmten oder aller Aufgaben in einem Array warten.  
...
-   Wenn Sie Fortsetzungsaufgaben aus mehreren Vorgängern erstellen, verwenden Sie die <xref:System.Threading.Tasks.TaskFactory.ContinueWhenAll%2A> Methode oder <xref:System.Threading.Tasks.TaskFactory.ContinueWhenAny%2A> -Methode oder die äquivalenten in der <xref:System.Threading.Tasks.Task%601> Klasse. Weitere Informationen finden Sie unter [Verketten von Aufgaben mithilfe von Fortsetzungsaufgaben](../../../docs/standard/parallel-programming/chaining-tasks-by-using-continuation-tasks.md).  
