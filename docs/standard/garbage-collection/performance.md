...
-   [<span data-ttu-id="6fe07-150">Die Pausen der Garbage Collections sind zu lang</span><span class="sxs-lookup"><span data-stu-id="6fe07-150">Garbage collection pauses are too long</span></span>](#Issue_LongPauses)  
...
|<span data-ttu-id="6fe07-182">Überprüfen der Leistung</span><span class="sxs-lookup"><span data-stu-id="6fe07-182">Performance checks</span></span>|  
|------------------------|  
...
-   <span data-ttu-id="6fe07-198">Häufiges Laden und Entladen von vielen kleinen Assemblys.</span><span class="sxs-lookup"><span data-stu-id="6fe07-198">Frequent loading and unloading of many small assemblies.</span></span>  
...
### <a name="workstation-or-server-garbage-collection"></a><span data-ttu-id="6fe07-242">Garbage Collection für die Arbeitsstation oder Garbage Collection auf dem Server</span><span class="sxs-lookup"><span data-stu-id="6fe07-242">Workstation or Server Garbage Collection</span></span>  
...
-   <span data-ttu-id="6fe07-259">Geben Sie in WinDbg bei geladener SOS-Debuggererweiterung folgenden Befehl ein:</span><span class="sxs-lookup"><span data-stu-id="6fe07-259">In WinDbg with the SOS debugger extension loaded, type the following command:</span></span>  
 ...
 [<span data-ttu-id="6fe07-265">Zurück nach oben</span><span class="sxs-lookup"><span data-stu-id="6fe07-265">Back to top</span></span>](#top)  
...
-   [<span data-ttu-id="6fe07-279">Bestimmen Sie die Anzahl der fixierten Objekte.</span><span class="sxs-lookup"><span data-stu-id="6fe07-279">Determine the number of pinned objects.</span></span>](#Pinned)  
...
3. <span data-ttu-id="6fe07-304">Sie können den folgenden Befehl verwenden, um geschachtelte Ausnahmen zu speichern.</span><span class="sxs-lookup"><span data-stu-id="6fe07-304">You can use the following command to dump nested exceptions.</span></span>  
...
-   <span data-ttu-id="6fe07-339">Geben Sie im WinDbg-Debugger oder im Visual Studio-Debugger bei geladener SOS-Debuggererweiterung den folgenden Befehl ein:</span><span class="sxs-lookup"><span data-stu-id="6fe07-339">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>  
     ...
     **<span data-ttu-id="6fe07-347">!dumpheap -type System.String -min 150 -max 200</span><span class="sxs-lookup"><span data-stu-id="6fe07-347">!dumpheap -type System.String -min 150 -max 200</span></span>**  
...
1. <span data-ttu-id="6fe07-368">Geben Sie im WinDbg-Debugger oder im Visual Studio-Debugger bei geladener SOS-Debuggererweiterung den folgenden Befehl ein:</span><span class="sxs-lookup"><span data-stu-id="6fe07-368">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>  
     ...
     **<span data-ttu-id="6fe07-369">!finalizequeue</span><span class="sxs-lookup"><span data-stu-id="6fe07-369">!finalizequeue</span></span>**  
...
-   <span data-ttu-id="6fe07-391">Der folgende Befehl gibt den freien Platz im Bereich der Generation 0 aus:</span><span class="sxs-lookup"><span data-stu-id="6fe07-391">The following command dumps the free space within the generation 0 range:</span></span>  
...
-   <span data-ttu-id="6fe07-398">Geben Sie im WinDbg-Debugger oder im Visual Studio-Debugger bei geladener SOS-Debuggererweiterung den folgenden Befehl ein:</span><span class="sxs-lookup"><span data-stu-id="6fe07-398">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>  
     ...
     **<span data-ttu-id="6fe07-399">!gchandles</span><span class="sxs-lookup"><span data-stu-id="6fe07-399">!gchandles</span></span>**  
...
-   <span data-ttu-id="6fe07-402">Überprüfen Sie den Speicherleistungsindikator `% Time in GC`.</span><span class="sxs-lookup"><span data-stu-id="6fe07-402">Examine the `% Time in GC` memory performance counter.</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="6fe07-408">Diese Informationen geben nicht an, wann die Garbage Collection erfolgt ist, Sie können aber die Anzahl der Garbage Collections ermitteln, die in einem bestimmten Zeitintervall aufgetreten sind.</span><span class="sxs-lookup"><span data-stu-id="6fe07-408">This information does not show when the garbage collection occurred, but you can determine the number of garbage collections that occurred in an interval of time.</span></span> <span data-ttu-id="6fe07-409">Im schlimmsten Fall wurde die Garbage Collection der zehnten Generation 0 zu Beginn des zweiten Intervalls abgeschlossen und die Garbage Collection der elften Generation 0 zum Ende des fünften Intervalls.</span><span class="sxs-lookup"><span data-stu-id="6fe07-409">Assuming the worst case, the tenth generation 0 garbage collection finished at the start of the second interval, and the eleventh generation 0 garbage collection finished at the end of the fifth interval.</span></span> <span data-ttu-id="6fe07-410">Die Zeit zwischen dem Ende der zehnten und dem Ende der elften Garbage Collection ist ungefähr 2 Sekunden, und der Leistungsindikator zeigt 3%. Daher war die Dauer der elften Garbage Collection der Generation 0 (2 Sekunden \* 3% = 60ms).</span><span class="sxs-lookup"><span data-stu-id="6fe07-410">The time between the end of the tenth and the end of the eleventh garbage collection is about 2 seconds, and the performance counter shows 3%, so the duration of the eleventh generation 0 garbage collection was (2 seconds \* 3% = 60ms).</span></span>  
  
     <span data-ttu-id="6fe07-411">In diesem Beispiel gibt es 5 Intervalle.</span><span class="sxs-lookup"><span data-stu-id="6fe07-411">In this example, there are 5 periods.</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="6fe07-412">Die zweite Garbage Collection der Generation 2 wurde während des dritten Intervalls gestartet und im fünften Intervall abgeschlossen.</span><span class="sxs-lookup"><span data-stu-id="6fe07-412">The second generation 2 garbage collection started during the third interval and finished at the fifth interval.</span></span> <span data-ttu-id="6fe07-413">Im schlimmsten Fall erfolgte die letzte Garbage Collection für eine Collection der Generation 0, die zu Beginn des zweiten Intervalls abgeschlossen wurde, und die Garbage Collection der Generation 2 wurde zum Ende des fünften Intervalls abgeschlossen.</span><span class="sxs-lookup"><span data-stu-id="6fe07-413">Assuming the worst case, the last garbage collection was for a generation 0 collection that finished at the start of the second interval, and the generation 2 garbage collection finished at the end of the fifth interval.</span></span> <span data-ttu-id="6fe07-414">Daher ist die Zeit zwischen dem Ende der Garbage Collection der Generation 0 und dem Ende der Garbage Collection der Generation 2 4 Sekunden.</span><span class="sxs-lookup"><span data-stu-id="6fe07-414">Therefore, the time between the end of the generation 0 garbage collection and the end of the generation 2 garbage collection is 4 seconds.</span></span> <span data-ttu-id="6fe07-415">Da der `% Time in GC`-Indikator 20% angibt, beträgt die maximale Zeitdauer, die die Garbage Collection der Generation 2 benötigt haben kann: (4 Sekunden \* 20% = 800ms).</span><span class="sxs-lookup"><span data-stu-id="6fe07-415">Because the `% Time in GC` counter is 20%, the maximum amount of time the generation 2 garbage collection could have taken is (4 seconds \* 20% = 800ms).</span></span>  
  
-   <span data-ttu-id="6fe07-416">Alternativ können Sie die Länge einer Garbage Collection mithilfe der [ETW-Ereignisse der Garbage Collection](../../../docs/framework/performance/garbage-collection-etw-events.md) bestimmen und die Informationen analysieren, um so die Dauer der Garbage Collection zu ermitteln.</span><span class="sxs-lookup"><span data-stu-id="6fe07-416">Alternatively, you can determine the length of a garbage collection by using [garbage collection ETW events](../../../docs/framework/performance/garbage-collection-etw-events.md), and analyze the information to determine the duration of garbage collection.</span></span>  
  
     <span data-ttu-id="6fe07-417">Beispielsweise enthalten die folgenden Daten eine Ereignissequenz, die während einer nicht gleichzeitigen Garbage Collection aufgetreten ist.</span><span class="sxs-lookup"><span data-stu-id="6fe07-417">For example, the following data shows an event sequence that occurred during a non-concurrent garbage collection.</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="6fe07-418">Das Anhalten des verwalteten Threads dauerte 26us (`GCSuspendEEEnd` - `GCSuspendEEBegin_V1`).</span><span class="sxs-lookup"><span data-stu-id="6fe07-418">Suspending the managed thread took 26us (`GCSuspendEEEnd` – `GCSuspendEEBegin_V1`).</span></span>  
  
     <span data-ttu-id="6fe07-419">Die tatsächliche Garbage Collection dauerte 4,8ms (`GCEnd_V1` - `GCStart_V1`).</span><span class="sxs-lookup"><span data-stu-id="6fe07-419">The actual garbage collection took 4.8ms (`GCEnd_V1` – `GCStart_V1`).</span></span>  
  
     <span data-ttu-id="6fe07-420">Das Fortsetzen der verwalteten Threads dauerte 21us (`GCRestartEEEnd` - `GCRestartEEBegin`).</span><span class="sxs-lookup"><span data-stu-id="6fe07-420">Resuming the managed threads took 21us (`GCRestartEEEnd` – `GCRestartEEBegin`).</span></span>  
  
     <span data-ttu-id="6fe07-421">Die folgende Ausgabe zeigt ein Beispiel für eine Garbage Collection im Hintergrund und enthält Felder für den Prozess, den Thread und das Ereignis.</span><span class="sxs-lookup"><span data-stu-id="6fe07-421">The following output provides an example for background garbage collection, and includes the process, thread, and event fields.</span></span> <span data-ttu-id="6fe07-422">(Es werden nicht alle Daten gezeigt.)</span><span class="sxs-lookup"><span data-stu-id="6fe07-422">(Not all data is shown.)</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="6fe07-423">Das `GCStart_V1`-Ereignis bei 42504816 gibt an, dass dies eine Garbage Collection im Hintergrund ist, da das letzte Feld `1` ist.</span><span class="sxs-lookup"><span data-stu-id="6fe07-423">The `GCStart_V1` event at 42504816 indicates that this is a background garbage collection, because the last field is `1`.</span></span> <span data-ttu-id="6fe07-424">Dies wird Garbage Collection Nr.</span><span class="sxs-lookup"><span data-stu-id="6fe07-424">This becomes garbage collection No.</span></span> <span data-ttu-id="6fe07-425">102019.</span><span class="sxs-lookup"><span data-stu-id="6fe07-425">102019.</span></span>  
  
     <span data-ttu-id="6fe07-426">Das `GCStart`-Ereignis tritt auf, weil eine kurzlebige Garbage Collection erforderlich ist, bevor eine Garbage Collection im Hintergrund gestartet wird.</span><span class="sxs-lookup"><span data-stu-id="6fe07-426">The `GCStart` event occurs because there is a need for an ephemeral garbage collection before you start a background garbage collection.</span></span> <span data-ttu-id="6fe07-427">Dies wird Garbage Collection Nr.</span><span class="sxs-lookup"><span data-stu-id="6fe07-427">This becomes garbage collection No.</span></span> <span data-ttu-id="6fe07-428">102020.</span><span class="sxs-lookup"><span data-stu-id="6fe07-428">102020.</span></span>  
  
     <span data-ttu-id="6fe07-429">Bei 42514170 wird Garbage Collection Nr.</span><span class="sxs-lookup"><span data-stu-id="6fe07-429">At 42514170, garbage collection No.</span></span> <span data-ttu-id="6fe07-430">102020 beendet.</span><span class="sxs-lookup"><span data-stu-id="6fe07-430">102020 finishes.</span></span> <span data-ttu-id="6fe07-431">Die verwalteten Threads werden an diesem Punkt neu gestartet.</span><span class="sxs-lookup"><span data-stu-id="6fe07-431">The managed threads are restarted at this point.</span></span> <span data-ttu-id="6fe07-432">Dies wird im Thread 4372 abgeschlossen, der diese Garbage Collection im Hintergrund ausgelöst hat.</span><span class="sxs-lookup"><span data-stu-id="6fe07-432">This is completed on thread 4372, which triggered this background garbage collection.</span></span>  
  
     <span data-ttu-id="6fe07-433">Im Thread 4744 tritt eine Unterbrechung auf.</span><span class="sxs-lookup"><span data-stu-id="6fe07-433">On thread 4744, a suspension occurs.</span></span> <span data-ttu-id="6fe07-434">Dies ist das einzige Mal, dass die Garbage Collection im Hintergrund verwaltete Threads anhalten muss.</span><span class="sxs-lookup"><span data-stu-id="6fe07-434">This is the only time at which the background garbage collection has to suspend managed threads.</span></span> <span data-ttu-id="6fe07-435">Die Dauer beträgt ungefähr 99ms ((63784407-63685394)/1000).</span><span class="sxs-lookup"><span data-stu-id="6fe07-435">This duration is approximately 99ms ((63784407-63685394)/1000).</span></span>  
  
     <span data-ttu-id="6fe07-436">Das `GCEnd`-Ereignis für die Garbage Collection im Hintergrund wird bei 89931423 angezeigt.</span><span class="sxs-lookup"><span data-stu-id="6fe07-436">The `GCEnd` event for the background garbage collection is at 89931423.</span></span> <span data-ttu-id="6fe07-437">Dies bedeutet, dass die Garbage Collection im Hintergrund etwa 47 Sekunden gedauert hat ((89931423-42504816)/1000).</span><span class="sxs-lookup"><span data-stu-id="6fe07-437">This means that the background garbage collection lasted for about 47seconds ((89931423-42504816)/1000).</span></span>  
  
     <span data-ttu-id="6fe07-438">Während die verwalteten Threads ausgeführt werden, können beliebig viele kurzlebige Garbage Collections stattfinden.</span><span class="sxs-lookup"><span data-stu-id="6fe07-438">While the managed threads are running, you can see any number of ephemeral garbage collections occurring.</span></span>  
  
<a name="Triggered"></a>   
##### <a name="to-determine-what-triggered-a-garbage-collection"></a><span data-ttu-id="6fe07-439">So ermitteln Sie, was eine Garbage Collection ausgelöst</span><span class="sxs-lookup"><span data-stu-id="6fe07-439">To determine what triggered a garbage collection</span></span>  
  
-   <span data-ttu-id="6fe07-440">Geben Sie im WinDbg-Debugger oder im Visual Studio-Debugger bei geladener SOS-Debuggererweiterung den folgenden Befehl ein, um alle Threads mit ihren Aufruflisten anzuzeigen:</span><span class="sxs-lookup"><span data-stu-id="6fe07-440">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command to show all the threads with their call stacks:</span></span>  
  
     **<span data-ttu-id="6fe07-441">~\*kb</span><span class="sxs-lookup"><span data-stu-id="6fe07-441">~\*kb</span></span>**  
  
     <span data-ttu-id="6fe07-442">Dieser Befehl zeigt eine Ausgabe ähnlich der folgenden an.</span><span class="sxs-lookup"><span data-stu-id="6fe07-442">This command displays output similar to the following.</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="6fe07-443">Wenn die Garbage Collection durch eine Benachrichtigung über unzureichenden Arbeitsspeicher vom Betriebssystem verursacht wurde, sieht die Aufrufliste ähnlich aus, mit dem Unterschied, dass der Thread ein Finalizerthread ist.</span><span class="sxs-lookup"><span data-stu-id="6fe07-443">If the garbage collection was caused by a low memory notification from the operating system, the call stack is similar, except that the thread is the finalizer thread.</span></span> <span data-ttu-id="6fe07-444">Der Finalizerthread erhält eine asynchrone Benachrichtigung über unzureichenden Arbeitsspeicher und führt die Garbage Collection aus.</span><span class="sxs-lookup"><span data-stu-id="6fe07-444">The finalizer thread gets an asynchronous low memory notification and induces the garbage collection.</span></span>  
  
     <span data-ttu-id="6fe07-445">Wenn die Garbage Collection durch die Speicherbelegung verursacht wurde, sieht der Stapel wie folgt aus:</span><span class="sxs-lookup"><span data-stu-id="6fe07-445">If the garbage collection was caused by memory allocation, the stack appears as follows:</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="6fe07-446">Ein Just-In-Time-Hilfsprogramm (`JIT_New*`) ruft schließlich `GCHeap::GarbageCollectGeneration` auf.</span><span class="sxs-lookup"><span data-stu-id="6fe07-446">A just-in-time helper (`JIT_New*`) eventually calls `GCHeap::GarbageCollectGeneration`.</span></span> <span data-ttu-id="6fe07-447">Wenn Sie feststellen, dass die Garbage Collections der Generation 2 durch Belegungen verursacht werden, müssen Sie ermitteln, welche Objekte von einer Garbage Collection der Generation 2 freigegeben werden und wie sie dies vermeiden können.</span><span class="sxs-lookup"><span data-stu-id="6fe07-447">If you determine that generation 2 garbage collections are caused by allocations, you must determine which objects are collected by a generation 2 garbage collection and how to avoid them.</span></span> <span data-ttu-id="6fe07-448">Das heißt, Sie müssen den Unterschied zwischen dem Anfang und am Ende einer Garbage Collection der Generation 2 sowie die Objekte bestimmen, welche die Collection der Generation 2 verursacht haben.</span><span class="sxs-lookup"><span data-stu-id="6fe07-448">That is, you want to determine the difference between the start and the end of a generation 2 garbage collection, and the objects that caused the generation 2 collection.</span></span>  
  
     <span data-ttu-id="6fe07-449">Geben Sie beispielsweise den folgenden Befehl im Debugger ein, um den Anfang einer Collection der Generation 2 anzuzeigen:</span><span class="sxs-lookup"><span data-stu-id="6fe07-449">For example, type the following command in the debugger to show the beginning of a generation 2 collection:</span></span>  
  
     **<span data-ttu-id="6fe07-450">!dumpheap –stat</span><span class="sxs-lookup"><span data-stu-id="6fe07-450">!dumpheap –stat</span></span>**  
  
     <span data-ttu-id="6fe07-451">Beispielausgabe (gekürzt, um die Objekte anzuzeigen, die den meisten Platz benötigen):</span><span class="sxs-lookup"><span data-stu-id="6fe07-451">Example output (abridged to show the objects that use the most space):</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="6fe07-452">Wiederholen Sie den Befehl am Ende der Generation 2:</span><span class="sxs-lookup"><span data-stu-id="6fe07-452">Repeat the command at the end of generation 2:</span></span>  
  
     **<span data-ttu-id="6fe07-453">!dumpheap –stat</span><span class="sxs-lookup"><span data-stu-id="6fe07-453">!dumpheap –stat</span></span>**  
  
     <span data-ttu-id="6fe07-454">Beispielausgabe (gekürzt, um die Objekte anzuzeigen, die den meisten Platz benötigen):</span><span class="sxs-lookup"><span data-stu-id="6fe07-454">Example output (abridged to show the objects that use the most space):</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="6fe07-455">Die `double[]`-Objekte am Ende der Ausgabe sind verschwunden, das heißt, sie wurden freigegeben.</span><span class="sxs-lookup"><span data-stu-id="6fe07-455">The `double[]` objects disappeared from the end of the output, which means that they were collected.</span></span> <span data-ttu-id="6fe07-456">Diese Objekte umfassen etwa 70 MB.</span><span class="sxs-lookup"><span data-stu-id="6fe07-456">These objects account for approximately 70 MB.</span></span> <span data-ttu-id="6fe07-457">Bei den übrigen Objekten gab es kaum Änderungen.</span><span class="sxs-lookup"><span data-stu-id="6fe07-457">The remaining objects did not change much.</span></span> <span data-ttu-id="6fe07-458">Daher waren diese `double[]`-Objekte der Grund, weshalb diese Garbage Collection der Generation 2 ausgeführt wurde.</span><span class="sxs-lookup"><span data-stu-id="6fe07-458">Therefore, these `double[]` objects were the reason why this generation 2 garbage collection occurred.</span></span> <span data-ttu-id="6fe07-459">Der nächster Schritt besteht darin, zu bestimmen, warum die `double[]`-Objekte vorhanden sind und warum sie inaktiv waren.</span><span class="sxs-lookup"><span data-stu-id="6fe07-459">Your next step is to determine why the `double[]` objects are there and why they died.</span></span> <span data-ttu-id="6fe07-460">Sie können den Codeentwickler fragen, woher diese Objekte stammen, oder Sie können den Befehl **gcroot** verwenden.</span><span class="sxs-lookup"><span data-stu-id="6fe07-460">You can ask the code developer where these objects came from, or you can use the **gcroot** command.</span></span>  
  
<a name="HighCPU"></a>   
##### <a name="to-determine-whether-high-cpu-usage-is-caused-by-garbage-collection"></a><span data-ttu-id="6fe07-461">So ermitteln Sie, ob die hohe CPU-Auslastung durch die Garbage Collection verursacht wird</span><span class="sxs-lookup"><span data-stu-id="6fe07-461">To determine whether high CPU usage is caused by garbage collection</span></span>  
  
-   <span data-ttu-id="6fe07-462">Setzen Sie den Wert des Speicherleistungsindikators `% Time in GC` zur Prozesszeit in Beziehung.</span><span class="sxs-lookup"><span data-stu-id="6fe07-462">Correlate the `% Time in GC` memory performance counter value with the process time.</span></span>  
  
     <span data-ttu-id="6fe07-463">Wenn der `% Time in GC`-Wert zum selben Zeitpunkt einen Spitzenwert erreicht wie die Prozesszeit, dann verursacht die Garbage Collection die hohe CPU-Auslastung.</span><span class="sxs-lookup"><span data-stu-id="6fe07-463">If the `% Time in GC` value spikes at the same time as process time, garbage collection is causing a high CPU usage.</span></span> <span data-ttu-id="6fe07-464">Erstellen Sie andernfalls ein Profil der Anwendung, um zu ermitteln, wie die hohe Auslastung zustande kommt.</span><span class="sxs-lookup"><span data-stu-id="6fe07-464">Otherwise, profile the application to find where the high usage is occurring.</span></span>  
  
## <a name="see-also"></a><span data-ttu-id="6fe07-465">Siehe auch</span><span class="sxs-lookup"><span data-stu-id="6fe07-465">See also</span></span>

- [<span data-ttu-id="6fe07-466">Garbage Collection</span><span class="sxs-lookup"><span data-stu-id="6fe07-466">Garbage Collection</span></span>](../../../docs/standard/garbage-collection/index.md)
