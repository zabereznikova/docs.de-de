---
title: "Isolierte Speicherung | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
ms.topic: "article"
helpviewer_keywords: 
  - "Code, Isolierter Speicher"
  - "Datenspeicherung mit dem isolierten Speicher"
  - "Datenspeicherung mit dem isolierten Speicher, Optionen"
  - "Datenspeicherung mit dem isolierten Speicher, Ungeeignete Fälle"
  - "Isolierter Speicher"
  - "Isolierter Speicher, Optionen"
  - "Isolierter Speicher, Ungeeignete Fälle"
  - "Isolation"
  - "Speicherort des isolierten Speichers im Dateisystem"
  - "Standardisierung von Speichersystemen"
  - "Speicher"
  - "Speichern von Daten mit dem isolierten Speicher"
  - "Speichern von Daten mit dem isolierten Speicher, Optionen"
  - "Speichern von Daten mit dem isolierten Speicher, Ungeeignete Fälle"
...
<a name="quotas"></a>   
## Kontingente für isolierte Speicherung  
 ...
 Kontingente werden nicht in Roamingspeichern durchgesetzt. Dies ist der Grund, weshalb für ihre Verwendung eine etwas höhere Berechtigungsebene erforderlich ist. Die Enumerationswerte <xref:System.Security.Permissions.IsolatedStorageContainment> und <xref:System.Security.Permissions.IsolatedStorageContainment> geben eine Berechtigung zur Verwendung von isolierter Speicherung für einen Roamingbenutzer an.  
...
|--------------------------|---------------------|-----------------------|  
|<xref:System.Security.Permissions.IsolatedStorageContainment>|Es ist keine Verwendung von isolierter Speicherung zulässig.|Es besteht kein Sicherheitsrisiko.|  
|<xref:System.Security.Permissions.IsolatedStorageContainment>|Isolation nach Benutzer, Domäne und Assembly. Jede Assembly verfügt über einen separaten Unterspeicher innerhalb der Domäne. Speicher, die diese Berechtigung verwenden, werden auch implizit nach Computer isoliert.|Diese Berechtigungsebene bietet Ressourcen keinen Schutz vor unbefugter Überbeanspruchung, wobei erzwungene Kontingente allerdings die Funktion haben, dies zu erschweren. Dies wird Denial\-of\-Service\-Angriff genannt.|  
|<xref:System.Security.Permissions.IsolatedStorageContainment>|Wie bei `DomainIsolationByUser`, jedoch befindet sich der Speicher an einem Speicherplatz, für den Roaming möglich ist, sofern Roamingbenutzerprofile aktiviert und Kontingente nicht erzwungen sind.|Da Kontingente deaktiviert sein müssen, sind die Speicherressourcen anfälliger für Denial\-of\-Service\-Angriffe.|  
|<xref:System.Security.Permissions.IsolatedStorageContainment>|Isolation nach Benutzer und Assembly. Speicher, die diese Berechtigung verwenden, werden auch implizit nach Computer isoliert.|Kontingente werden auf dieser Ebene erzwungen, um einen Denial\-of\-Service\-Angriff zu verhindern. Dieselbe Assembly in einer anderen Domäne kann auf diesen Speicher zugreifen, wodurch ein Verlust von Informationen zwischen Anwendungen möglich ist.|  
|<xref:System.Security.Permissions.IsolatedStorageContainment>|Wie bei `AssemblyIsolationByUser`, jedoch befindet sich der Speicher an einem Speicherplatz, für den Roaming möglich ist, sofern Roamingbenutzerprofile aktiviert und Kontingente nicht erzwungen sind.|Wie bei `AssemblyIsolationByUser`. Allerdings erhöht sich ohne Kontingente das Risiko eines Denial\-of\-Service\-Angriffs.|  
|<xref:System.Security.Permissions.IsolatedStorageContainment>|Isolation nach Benutzer. Normalerweise gilt diese Berechtigungsebene nur für Verwaltungs\- oder Debugtools.|Durch den Zugriff mit dieser Berechtigung kann Code jede der Dateien oder Verzeichnisse in isolierten Speicherplätzen anzeigen oder löschen \(unabhängig von Assemblyisolation\). Es besteht u. a. das Risiko eines Informationslecks und des Datenverlusts.|  
|<xref:System.Security.Permissions.IsolatedStorageContainment>|Isolation nach Benutzern, Domänen und Assemblys. Normalerweise gilt diese Berechtigungsebene nur für Verwaltungs\- oder Debugtools.|Durch diese Berechtigung sind sämtliche isolierte Speicher aller Benutzer potenziell gefährdet.|  
...
|[!INCLUDE[win8](../../../includes/win8-md.md)], Windows 7, Windows Server 2008, Windows Vista|Für Roaming aktivierte Speicher \=<br /><br /> \<SYSTEMLAUFWERK\>\\Benutzer\\\<Benutzer\>\\Anwendungsdaten\\Roaming<br /><br /> Nicht für Roaming eingerichtete Speicher \=<br /><br /> \<SYSTEMLAUFWERK\>\\Benutzer\\\<Benutzer\>\\Anwendungsdaten\\Lokal|  
...
-   <xref:System.IO.IsolatedStorage.IsolatedStorageFile> wird von <xref:System.IO.IsolatedStorage.IsolatedStorage?displayProperty=fullName> abgeleitet und ermöglicht die Verwaltung von gespeicherten Assembly\- und Anwendungsdateien. Eine Instanz der <xref:System.IO.IsolatedStorage.IsolatedStorageFile>\-Klasse stellt einen einzelnen Speicher im Dateisystem dar.  
  
-   <xref:System.IO.IsolatedStorage.IsolatedStorageFileStream> wird aus <xref:System.IO.FileStream?displayProperty=fullName> abgeleitet und ermöglicht den Zugriff auf Dateien in einem Speicher.  
...
|[Gewusst wie: Auflisten von Speichern für isolierten Speicher](../../../docs/standard/io/how-to-enumerate-stores-for-isolated-storage.md)|Zeigt, wie mithilfe der <xref:System.IO.IsolatedStorage.IsolatedStorageFile.GetEnumerator%2A?displayProperty=fullName>\-Methode die Größe des gesamten isolierten Speichers für den Benutzer berechnet wird.|  
|[Gewusst wie: Löschen von Speichern im isolierten Speicher](../../../docs/standard/io/how-to-delete-stores-in-isolated-storage.md)|Zeigt, wie mit der <xref:System.IO.IsolatedStorage.IsolatedStorageFile.Remove%2A?displayProperty=fullName>\-Methode isolierte Speicher auf zwei unterschiedliche Arten gelöscht werden.|  
