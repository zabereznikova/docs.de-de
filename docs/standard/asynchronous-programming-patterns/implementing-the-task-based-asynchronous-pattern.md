...
- Verwenden Sie in [!INCLUDE[net_v45](../../../includes/net-v45-md.md)] und höheren Versionen (einschließlich .NET Core und .NET Standard) die statische <xref:System.Threading.Tasks.Task.Run%2A?displayProperty=nameWithType>-Methode als Verknüpfung zu <xref:System.Threading.Tasks.TaskFactory.StartNew%2A?displayProperty=nameWithType>. Sie können <xref:System.Threading.Tasks.Task.Run%2A> verwenden, um problemlos eine rechnergebundene Aufgabe zu starten, die auf den Threadpool abzielt. In [!INCLUDE[net_v45](../../../includes/net-v45-md.md)] und höheren Versionen ist dies der bevorzugte Mechanismus für das Starten eines rechnergebundenen Tasks. Verwenden Sie `StartNew` nur dann direkt, wenn Sie eine präzisere Kontrolle über den Task haben möchten.
