...
3. Wenn die Zeit mehrdeutig ist, rufen <xref:System.TimeZoneInfo.GetAmbiguousTimeOffsets%2A> Sie die-Methode auf, <xref:System.TimeSpan> um ein Array von-Objekten abzurufen. Jedes Element im Array enthält einen UTC-Offset, dem die mehrdeutige Zeit zugeordnet werden kann.
...
6. Mit der `static` (`Shared` in Visual Basic .net) <xref:System.DateTime.SpecifyKind%2A> -Methode können Sie die-Eigenschaft des UTC- <xref:System.DateTime.Kind%2A> Datums- <xref:System.DateTimeKind.Utc?displayProperty=nameWithType>und Uhrzeitwerts auf festlegen.
...
- [Vorgehensweise: Mehrdeutige Zeiten auflösen](../../../docs/standard/datetime/resolve-ambiguous-times.md)
