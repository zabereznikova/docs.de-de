---
title: "Gewusst wie: Instanziieren eines TimeZoneInfo-Objekts | Microsoft Docs"
ms.custom: ""
ms.date: "04/10/2017"
ms.prod: ".net"
ms.reviewer: ""
...
2.  Rufen Sie die `static` \(`Shared` in Visual Basic\) <xref:System.TimeZoneInfo.FindSystemTimeZoneById%2A?displayProperty=fullName>\-Methode auf.  
...
## Beispiel  
 ...
 Der einzige Parameter der Methode, <xref:System.TimeZoneInfo.FindSystemTimeZoneById%2A?displayProperty=fullName>, ist der Bezeichner der Zeitzone, die abgerufen werden soll; er entspricht der <xref:System.TimeZoneInfo.Id%2A?displayProperty=fullName>\-Eigenschaft des Objekts. Der Zeitzonenbezeichner ist ein Schlüsselfeld, das die Zeitzone eindeutig identifiziert. Während die meisten Schlüssel relativ kurz sind, ist der Zeitzonenbezeichner vergleichsweise lang. In den meisten Fällen entspricht der dazugehörige Wert der <xref:System.TimeZoneInfo.StandardName%2A>\-Eigenschaft eines <xref:System.TimeZoneInfo>\-Objekts, das zum Bereitstellen des Namens der Standardzeit für die Zeitzone verwendet wird. Es gibt jedoch auch Ausnahmen. Die beste Möglichkeit, um sicherzustellen, dass Sie einen gültigen Bezeichner angeben, besteht darin, die auf Ihrem System verfügbaren Zeitzonen aufzulisten und die Bezeichner der darin enthaltenen Zeitzonen zur Kenntnis zu nehmen. Eine Veranschaulichung finden Sie unter [Gewusst wie: Auflisten der auf einem Computer vorhandenen Zeitzonen](../../../docs/standard/datetime/enumerate-time-zones.md). Das Thema [Suchen der auf einem lokalen System definierten Zeitzonen](../../../docs/standard/datetime/finding-the-time-zones-on-local-system.md) enthält auch eine Liste ausgewählter Zeitzonenbezeichner.  
