---
title: "Windows Workflow Foundation 4 – Leistung | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# Windows Workflow Foundation 4 – Leistung
Dustin Metzgar  
...
## Terminologie  
 Die in [!INCLUDE[netfx40_short](../../../includes/netfx40-short-md.md)] eingeführte [!INCLUDE[wf1](../../../includes/wf1-md.md)]\-Version wird in den übrigen Abschnitten dieses Themas als WF4 bezeichnet.[!INCLUDE[wf1](../../../includes/wf1-md.md)] wurde in .NET 3.0 eingeführt und hatte bis [!INCLUDE[netfx35_short](../../../includes/netfx35-short-md.md)] SP1 einige geringfügige Revisionen.Die [!INCLUDE[netfx35_short](../../../includes/netfx35-short-md.md)]\-Version von Workflow Foundation wird in den restlichen Abschnitten dieses Themas als WF3 bezeichnet.WF3 wird in [!INCLUDE[netfx40_short](../../../includes/netfx40-short-md.md)] parallel mit WF4 ausgeliefert.[!INCLUDE[crabout](../../../includes/crabout-md.md)] zum Migrieren von WF3\-Artekfaten zu WF4 finden Sie im: [Migrationshandbuch zu Windows Workflow Foundation 4](http://go.microsoft.com/fwlink/?LinkID=153313)  
  
 [!INCLUDE[indigo1](../../../includes/indigo1-md.md)] ist Microsofts einheitliches Programmiermodell zum Erstellen von dienstorientierten Anwendungen.Es wurde zuerst als Teil von .NET 3.0 zusammen mit WF3 eingeführt und ist jetzt eine der Hauptkomponenten von [!INCLUDE[dnprdnshort](../../../includes/dnprdnshort-md.md)].  
...
## Übersicht über die Leistungsverbesserungen in WF4  
...
### Leistung  
 ![Leistungsdiagramm für Onlineshopdienst](../../../docs/framework/windows-workflow-foundation//media/onlinestoreperfgraph.gif "OnlineStorePerfGraph")  
...
#### Workflowaktivierungslatenzzeit  
...
#### Testsetup  
 ![Workflowtest für Korrelationsdurchsatz](../../../docs/framework/windows-workflow-foundation//media/correlationthroughputworkflow.gif "CorrelationThroughputWorkflow")  
  
 Der oben angezeigte Workflow entspricht dem Workflow, der unten im Abschnitt "Persistenz" verwendet wurde.Für die Korrelationstests ohne Persistenz ist kein Persistenzanbieter in der Laufzeit installiert.Korrelation tritt an zwei Stellen auf: CreateOrder und CompleteOrder.  
...
#### Testergebnisse  
 ...
 Dieses Diagramm zeigt eine Leistungsabnahme als Anzahl der in inhaltsbasierten Korrelationszunahmen verwendeten Schlüssel an.Die Ähnlichkeit der Kurven zwischen TCP und HTTP gibt den Mehraufwand an, der diesen Protokollen eigen ist.  
...
### Testsetup  
 ...
 ![Gleichung zur Berechnung der Anzahl von Aktivitäten](../../../docs/framework/windows-workflow-foundation//media/numberofactivitiesequation.gif "NumberOfActivitiesEquation")  
 ...
 ![Gleichung zur Berechnung der Anzahl von Aktivitäten](../../../docs/framework/windows-workflow-foundation//media/w3numberofactivitiesequation.gif "W3NumberOfActivitiesEquation")  
 ...
 ![Vergleichen der in jedem Test verwendeten Anzahl von Aktivitäten](../../../docs/framework/windows-workflow-foundation//media/comparechart.gif "CompareChart")  
...
### Arbeitsspeicher  
 Der Arbeitsspeichermehraufwand von Windows Workflow Foundation wird in zwei Hauptbereichen gemessen: Workflowkomplexität und Anzahl von Workflowdefinitionen.Arbeitsspeichermessungen wurden auf einem Windows 7\-64\-Bit\-PC vorgenommen.Zum Messen der Workingsetgröße gibt es viele Möglichkeiten, z. B. das Überwachen der Leistungsindikatoren, Abrufen von Environment.WorkingSet oder Verwenden eines Tools wie etwa VMMap, das unter [VMMap](http://technet.microsoft.com/sysinternals/dd535533.aspx) verfügbar ist.Eine Kombination von Methoden wurde verwendet, um die Ergebnisse aller Tests abzurufen und zu überprüfen.  
  
### Workflowkomplexitätstest  
...
### Test mehrerer Workflowdefinitionen  
 ...
 Beim Hosten von Workflows in IIS kann ein <xref:System.Web.Hosting.VirtualPathProvider> zum Erstellen eines neuen <xref:System.ServiceModel.WorkflowServiceHost> verwendet werden, statt alle XAMLX\- oder XOML\-Dateien zu generieren.Der <xref:System.Web.Hosting.VirtualPathProvider> behandelt die eingehende Anforderung und antwortet mit einer "virtuellen Datei", die aus einer Datenbank geladen oder wie in diesem Fall in diesem Vorgang generiert werden kann.Daher ist es nicht erforderlich, 1000 physische Dateien zu erstellen.  
...
### Testsetup  
 ...
 Die beiden Workflows wurden anhand einer empfangenen Meldung erstellt.Der Workflow wird beibehalten, nachdem eine erste Antwort gesendet wurde.In WF3 wird eine leere <xref:System.Workflow.ComponentModel.TransactionScopeActivity> verwendet, um die Persistenz zu initiieren.Dasselbe kann in WF3 erreicht werden, indem eine Aktivität mit "Beim Schließen beibehalten" markiert wird. Eine zweite, korrelierte Meldung schließt den Workflow ab.Die Workflows werden beibehalten, aber nicht entladen.  
...
### Testergebnisse  
 ...
 Beachten Sie, dass der WF4\-SQL\-Persistenzanbieter weitere Aufgaben in der Datenbankebene ausführt.Die SQL\-Datenbank kann zu einem Engpass führen. Daher ist die Überwachung der CPU und Datenträgerverwendung in diesem Fall von Bedeutung.Nehmen Sie die folgenden Leistungsindikatoren aus der SQL\-Datenbank beim Leistungstest von Workflowanwendungen auf:  
...
-   Physischer Datenträger\\% Durchschnittl.Länge der Datenträgerwarteschlange  
...
-   Informationen zum Prozessor\\Prozessorzeit \(%\)  
...
## Interop  
 WF4 ist fast vollständig neue Version von [!INCLUDE[wf1](../../../includes/wf1-md.md)]. Daher sind WF3\-Workflows und Aktivitäten nicht direkt mit WF4 kompatibel.Viele Kunden, die [!INCLUDE[wf2](../../../includes/wf2-md.md)] früh übernommen haben, verfügen über interne Workflowdefinitionen oder Workflowdefinitionen eines Drittanbieters und benutzerdefinierte Aktivitäten für WF3.Eine Möglichkeit, den Übergang zu WF4 zu vereinfachen, ist die Verwendung der Interop\-Aktivität, mit der WF3\-Aktivitäten in WF4\-Workflows ausgeführt werden können.Es wird empfohlen, dass die <xref:System.Activities.Statements.Interop>\-Aktivität nur bei Bedarf verwendet wird.[!INCLUDE[crabout](../../../includes/crabout-md.md)] zur Migration zu WF4 finden Sie in der [WF4\-Migrationsanleitung](http://go.microsoft.com/fwlink/?LinkID=153313).  
