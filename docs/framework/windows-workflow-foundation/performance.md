...
ms.locfileid: "79182977"
---
# <a name="windows-workflow-foundation-4-performance"></a>Windows Workflow Foundation 4 – Leistung

 .NET Framework 4 enthält eine umfassende Überarbeitung der Windows Workflow Foundation (WF) mit hohen Investitionen in die Leistung. Diese neue Überarbeitung führt zu erheblichen [!INCLUDE[wf1](../../../includes/wf1-md.md)] Entwurfsänderungen gegenüber den früheren Versionen, die als Teil von .NET Framework 3.0 und .NET Framework 3.5 ausgeliefert wurden. Die Architektur wurde auf Grundlage von Programmiermodell, Laufzeit und Tools neu gestaltet, um Leistung und Benutzerfreundlichkeit erheblich zu verbessern. In diesem Thema werden die wichtigen Leistungsmerkmale dieser Revisionen gezeigt und mit denen der vorherigen Version verglichen.

 Die Leistung der einzelnen Workflowkomponenten wurde von WF3 zu WF4 um ein Vielfaches gesteigert.  Dadurch bleibt die Lücke zwischen handcodierten Windows Communication Foundation (WCF)-Diensten und WCF-Workflowdiensten recht gering.  Die Workflowlatenzzeit wurde in WF4 bedeutend verringert.  Die Persistenzleistung wurde um den Faktor 2,5 bis 3,0 gesteigert.  Der Mehraufwand für die Systemüberwachung mittels Workflownachverfolgung wurde bedeutend verringert.  Dies sind überzeugende Gründe für die Migration zu WF4 oder zur Verwendung dieser Technologie in Ihren Anwendungen.

## <a name="terminology"></a>Begriff

 Die in [!INCLUDE[wf1](../../../includes/wf1-md.md)] .NET Framework 4 eingeführte Version wird für den Rest dieses Themas als WF4 bezeichnet. [!INCLUDE[wf1](../../../includes/wf1-md.md)]wurde in .NET Framework 3.0 eingeführt und hatte einige kleinere Überarbeitungen über .NET Framework 3.5 SP1. Die .NET Framework 3.5-Version von Workflow Foundation wird für den Rest dieses Themas als WF3 bezeichnet. WF3 wird in .NET Framework 4 seiteneben WF4 ausgeliefert. Weitere Informationen zum Migrieren von WF3-Artefakten zu WF4 finden Sie unter: [Windows Workflow Foundation 4 Migration Guide](migration-guidance.md).

 Windows Communication Foundation (WCF) ist Microsofts einheitliches Programmiermodell für das Erstellen serviceorientierter Anwendungen. Es wurde zuerst als Teil von .NET 3.0 zusammen mit WF3 eingeführt und ist jetzt eine der Schlüsselkomponenten von .NET Framework.
...
### <a name="wf-runtime"></a>WF-Laufzeit
 ...
 Im Vergleich zu WF3 ist der Planer der WF4-Laufzeit effizienter. Es nutzt denselben E/A-Threadpool, der für WCF verwendet wird, was bei der Ausführung von Batcharbeitsaufgaben sehr effizient ist. Die interne Warteschlange des Arbeitselementplaners ist für die gängigsten Verwendungsmuster optimiert. Die WF4-Laufzeit verwaltet die Ausführungszustände auch auf sehr einfache Weise mit minimaler Synchronisierungs- und Ereignisbehandlungslogik, während WF3 von der Registrierung von Schwergewicht-Ereignis und dem Aufruf abhängt, um komplexe Synchronisierungen für Zustandsübergänge durchzuführen.

### <a name="data-storage-and-flow"></a>Datenspeicherung und -fluss
 In WF3 werden einer Aktivität zugeordnete Daten durch Abhängigkeitseigenschaften modelliert, die vom Typ <xref:System.Windows.DependencyProperty> implementiert werden. Das Abhängigkeitseigenschaftsmuster wurde in Windows Presentation Foundation (WPF) eingeführt. Im Allgemeinen ist dieses Muster sehr flexibel, sodass eine einfache Datenbindung und andere Benutzeroberflächenfunktionen unterstützt werden. Für das Muster müssen jedoch die Eigenschaften als statische Felder in der Workflowdefinition definiert sein. Jedes Mal, wenn die [!INCLUDE[wf1](../../../includes/wf1-md.md)]-Laufzeit die Eigenschaftswerte festlegt oder abruft, wird eine komplexe Suchlogik angewendet.

 WF4 verwendet eine Bewertungslogik mit Klartextdaten, um die Verarbeitung von Daten in einem Workflow erheblich zu verbessern. Die in einer Aktivität gespeicherten Daten werden durch zwei verschiedene Konzepte (Variablen und Argumente) von den Daten getrennt, die über die Aktivitätsgrenzen fließen. Durch die Verwendung eines klaren hierarchischen Bereichs für Variablen und "In/Out/InOut"-Argumente wird die Datennutzungskomplexität für Aktivitäten drastisch reduziert und die Lebensdauer der Daten wird ebenfalls automatisch begrenzt. Aktivitäten verfügen über eine gut definierte Signatur, die von ihren Argumenten beschrieben wird. Durch die einfache Überprüfung einer Aktivität können Sie bestimmen, welche Daten erwartet und welche Daten als Ausführungsergebnis erzeugt werden.
...
### <a name="control-flow"></a>Ablaufsteuerung
 ...
 WF4 behandelt diesen Vorgang anders. Es nimmt die Aktivitätsvorlage, erstellt ein neues ActivityInstance-Objekt und fügt es der Planerwarteschlange hinzu. Dieser gesamte Prozess beinhaltet nur explizite Objekterstellung und ist sehr leicht.
...
```
 ...
 Der Fehlerhandler `WorkScope` zielt auf die Aktivität ab. Der WF4-Workflow ist ebenso vereinfacht.  Eine <xref:System.Activities.Statements.CompensableActivity> verfügt über einen Textkörper und einen Kompensierungshandler.  Als Nächstes folgt eine explizite Kompensierung in der Sequenz.  Die Textkörperaktivität und die Kompensationshandleraktivität sind jeweils leere Implementierungen:
...
Das folgende Diagramm zeigt den grundlegenden Vergütungsworkflow. Der WF3-Workflow befindet sich links, der WF4-Workflow rechts.

![WF3- und WF4-Basiskompensations-Workflows](./media/performance/basic-compensation-workflows-wf3-wf4.gif)

### <a name="performance-test-results"></a>Ergebnisse des Leistungstests

 ![Tabelle mit Leistungstestergebnissen](./media/performance/performance-test-data.gif)

 ![Säulendiagramm zum Vergleich von WF3- und WF4-Leistungstestdaten](./media/performance/performance-test-chart.gif)
...
## <a name="service-composition-scenario"></a>Dienstkompositionsszenario
 Wie im vorherigen Abschnitt "Leistungsvergleiche auf Komponentenebene" gezeigt, ist der Overhead zwischen WF3 und WF4 erheblich reduziert worden.  WCF-Workflowdienste können jetzt fast mit der Leistung von handcodierten WCF-Diensten übereinstimmen, haben aber dennoch alle Vorteile der [!INCLUDE[wf1](../../../includes/wf1-md.md)] Laufzeit.  In diesem Testszenario wird ein WCF-Dienst mit einem WCF-Workflowdienst in WF4 verglichen.

### <a name="online-store-service"></a>Onlineshopdienst
 Eine der Stärken von Windows Workflow Foundation ist die Möglichkeit, Prozesse mit mehreren Diensten zu erstellen.  Für dieses Beispiel gibt es einen Onlineshopdienst, der zwei Dienstaufrufe orchestriert, um eine Bestellung abzuwickeln.  Der erste Schritt besteht darin, die Bestellung mithilfe eines Bestellungsvalidierungsdiensts zu validieren.  Der zweite Schritt besteht darin, die Bestellung mithilfe eines Warehouse-Diensts auszufüllen.

 Die zwei Back-End-Dienste, Bestellungsvalidierungsdienst und Warehouse-Dienst, bleiben für beide Tests gleich.  Der Teil, der sich ändert, ist der Onlineshopdienst, von dem die Orchestrierung ausgeführt wird.  In einem Fall wird der Dienst als WCF-Dienst handcodiert.  Im anderen Fall wird der Dienst als WCF-Workflowdienst in WF4 geschrieben. [!INCLUDE[wf1](../../../includes/wf1-md.md)]-spezifische Funktionen wie Nachverfolgen und Persistenz sind für diesen Test deaktiviert.

### <a name="environment"></a>Environment
![Umgebungseinstellungen für Leistungsmessung](./media/performance/performance-test-environment.gif)

 Clientanforderungen werden dem Onlineshopdienst über HTTP von mehreren Computern gestellt.  Ein einzelner Computer hostet alle drei Dienste.  Die Transportebene zwischen dem Onlineshopdienst und den Back-End-Diensten ist TCP oder HTTP.  Die Messung der Vorgänge pro Sekunde basiert auf der Anzahl der beim Onlineshopdienst eingegangenen `PurchaseOrder`-Aufrufe.  Channelpooling ist eine neue, in WF4 verfügbare Funktion.  Im WCF-Teil dieses Testkanalpools wird das Pooling nicht sofort bereitgestellt, sodass eine handcodierte Implementierung einer einfachen Pooling-Technik im Online Store-Dienst verwendet wurde.

### <a name="performance"></a>Leistung
![Säulendiagramm mit der Leistung des Online-Store-Dienstes](./media/performance/online-store-performance-graph.gif)
...
#### <a name="workflow-activation-latency"></a>Workflowaktivierungslatenzzeit
 In einer WCF-Workflowdienstanwendung ist die Latenz zum Starten eines neuen Workflows oder zum Laden eines vorhandenen Workflows wichtig, da sie blockiert werden kann.  Dieser Testfall misst einen WF3-XOML-Host in einem typischen Szenario im Vergleich zu einem WF4-XAMLX-Host.
...
#### <a name="environment-setup"></a>Einrichten der Umgebung
![Umgebungseinrichtung für Workflowleistungstest](./media/performance/performance-test-environment.gif)

#### <a name="test-setup"></a>Testsetup
![Workflowtest für Korrelationsdurchsatz](./media/performance/correlation-throughput-workflow.gif "Workflow-Testeinrichtung für Korrelationsdurchsatz")

 Der vorherige Workflow ist derselbe, der im Abschnitt [Persistenz](#persistence) verwendet wurde. Für die Korrelationstests ohne Persistenz ist kein Persistenzanbieter in der Laufzeit installiert. Korrelation tritt an zwei Stellen auf: CreateOrder und CompleteOrder.

#### <a name="test-results"></a>Testergebnisse
![Korrelationsdurchsatz](./media/performance/correlation-throughput-graph.gif "Korrelationsdurchsatzdiagramm")
...
### <a name="test-setup"></a>Testsetup
 ...
 ![Gleichung zur Berechnung der Anzahl der WF3-Aktivitäten](./media/performance/wf3-number-activities-equation.gif)
 ...
 ![Flussdiagramm des komplexen Durchsatz-Workflows](./media/performance/complex-workflow-throughput-workflow.gif)
 ...
 ![Flussdiagrammausschnitt des WF3-Workflows](./media/performance/wf3-workflow-snippet.gif)
 ...
 ![Flussdiagramm einer verschachtelten Sequenz](./media/performance/nested-sequence-workflow.gif)

 Nachverfolgung und Persistenz werden nicht als Teil dieses Tests verwendet.

### <a name="test-results"></a>Testergebnisse
 ![Säulendiagramm mit Ergebnissen der Durchsatzleistung](./media/performance/throughput-performance-results.gif)
...
### <a name="memory"></a>Arbeitsspeicher
 Der Arbeitsspeicheraufwand von Windows Workflow Foundation wird in zwei Hauptbereichen gemessen: Workflowkomplexität und Anzahl von Workflowdefinitionen.  Arbeitsspeichermessungen wurden auf einem Windows 7-64-Bit-PC vorgenommen.  Es gibt viele Möglichkeiten, die Messung der Arbeitssatzgröße zu erhalten, z. B. Das Überwachen von Leistungsindikatoren, das Abrufen von Environment.WorkingSet oder die Verwendung eines Tools wie VMMap, das in [VMMap](/sysinternals/downloads/vmmap)verfügbar ist. Eine Kombination von Methoden wurde verwendet, um die Ergebnisse aller Tests abzurufen und zu überprüfen.
...
### <a name="workflow-complexity-test"></a>Workflowkomplexitätstest
 ...
 ![Säulendiagramm mit komplexer Workflowspeichernutzung für WF3- und WF4-Workflows](./media/performance/complex-memory-usage-wf3-wf4.gif)
...
Die folgende Abbildung zeigt einen WF3-Workflow mit ReceiveActivity und einen WF4-Workflow mit Anforderungs-/Antwortmuster:
 ...
 Die folgende Tabelle zeigt das Delta im Arbeitssatz zwischen einer einzelnen Workflowdefinition und 1001 Definitionen:
...
|Von IIS gehostete Workflowdienste|446 MB|364 MB|
 ...
 Das Hosten von Workflowdefinitionen in IIS <xref:System.ServiceModel.WorkflowServiceHost>verbraucht aufgrund der detaillierten WCF-Dienstartefakte und der dem Host zugeordneten Nachrichtenverarbeitungslogik viel mehr Arbeitsspeicher.
...
### <a name="test-setup"></a>Testsetup
 ...
 Die beiden Workflows wurden anhand einer empfangenen Meldung erstellt.  Der Workflow wird beibehalten, nachdem eine erste Antwort gesendet wurde.  In WF3 wird eine leere <xref:System.Workflow.ComponentModel.TransactionScopeActivity> verwendet, um die Persistenz zu initiieren.  Dasselbe könnte in WF3 erreicht werden, indem eine Aktivität als "persist on close" markiert wird.  Eine zweite, korrelierte Meldung schließt den Workflow ab.  Die Workflows werden beibehalten, aber nicht entladen.

### <a name="test-results"></a>Testergebnisse
 ![Säulendiagramm mit Durchsatzpersistenz](./media/performance/throughput-persistence-graph.gif)
...
- PhysicalDisk\\%Disk Lesezeit

- PhysicalDisk\\% Festplattenzeit

- PhysicalDisk\\% Disk Write Time

- PhysicalDisk\\% Avg. Disk Queue Länge
...
### <a name="tracking"></a>Tracking
 ...
 WF4 wird nicht mit einem SQL-Nachverfolgungsdienst bereitgestellt.  Das Aufzeichnen von Überwachungsinformationen in einer SQL-Datenbank kann besser von einem Anwendungsserver verarbeitet werden, anstatt in .NET Framework integriert zu werden. Daher wird die SQL-Nachverfolgung jetzt von AppFabric abgewickelt.  Der Standardnachverfolgungsanbieter in WF4 basiert auf der Ereignisablaufverfolgung für Windows (ETW).
...
- ETW-Tracking-Ereignisse können problemlos mit den WCF ETW-Ereignissen oder anderen ETW-Anbietern wie einem SQL Server oder einem Kernelanbieter kombiniert werden.
...
### <a name="test-results"></a>Testergebnisse
 ...
 ![Säulendiagramm mit Workflow-Tracking-Kosten](./media/performance/workflow-tracking-costs.gif)
...
## <a name="interop"></a>Interop
 WF4 ist eine fast vollständig neue Version von [!INCLUDE[wf1](../../../includes/wf1-md.md)]. Daher sind WF3-Workflows und Aktivitäten nicht direkt mit WF4 kompatibel.  Viele Kunden, die Windows Workflow Foundation frühzeitig übernommen haben, verfügen über interne oder Drittanbieter-Workflowdefinitionen und benutzerdefinierte Aktivitäten für WF3.  Eine Möglichkeit, den Übergang zu WF4 zu vereinfachen, ist die Verwendung der Interop-Aktivität, mit der WF3-Aktivitäten in WF4-Workflows ausgeführt werden können.  Es wird empfohlen, die <xref:System.Activities.Statements.Interop>-Aktivität nur bei Bedarf zu verwenden. Weitere Informationen zur Migration zu WF4 finden Sie im [WF4-Migrationsleitfaden](migration-guidance.md).
