...
Das folgende Diagramm zeigt die grundlegende Kompensierung-Workflow. Der WF3-Workflow befindet sich links, der WF4-Workflow rechts.
...
##### <a name="test-results"></a>Testergebnisse
 ...
 Kalte und warme Latenzzeit für WCF-Workflowdienste:
...
#### <a name="test-setup"></a>Testsetup
![Testen von Korrelation Durchsatz Workflows](./media/performance/correlation-throughput-workflow.gif "Korrelation Durchsatz Workflow testeinrichtung")

 Der vorherige Workflow entspricht dem in verwendet die [Persistenz](#persistence) Abschnitt. Für die Korrelationstests ohne Persistenz es ist kein Persistenzanbieter in der Laufzeit installiert. Korrelation tritt an zwei Stellen: CreateOrder und CompleteOrder.

#### <a name="test-results"></a>Testergebnisse
![Korrelationsdurchsatz](./media/performance/correlation-throughput-graph.gif "Korrelation durchsatzdiagramm")
...
Die folgende Abbildung zeigt ein WF3-Workflow mit ReceiveActivity und WF4-Workflows mit Anforderung/Antwort-Muster:

 ![Workflowdienste in WF3 und WF4](./media/performance/workflow-receive-activity.gif)

 Die folgende Tabelle zeigt die Differenz im Workingset zwischen einer einzelnen Workflowdefinition und 1001 Definitionen:
