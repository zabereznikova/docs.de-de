...
2. Der Vorgesetzten des Anforderers muss die Anforderung genehmigen:  
...
3. Nachdem der Vorgesetzten der Anforderer genehmigt hat, muss der Besitzer der Abteilung die Anforderung genehmigen:  
...
|Workflowdienste|`ResumeRequestService` wird unter Verwendung von Workflowdiensten implementiert. Workflow Definition und Dienst Informationen sind in resumerequestservice. xamlx enthalten. Der Dienst ist für die Verwendung von Persistenz und Überwachung konfiguriert.|ResumeRequestService|  
...
2. Klicken Sie in der Projekt Mappe mit der rechten Maustaste auf **InternalClient** , und wählen Sie **in Browser anzeigen aus**. Die Standardseite für `InternalClient` wird angezeigt. Vergewissern Sie sich, dass die Dienste ausgeführt werden, und klicken Sie auf den Link.  
...
3. Peter möchte auf die Anforderung von Michael reagieren. Er ist der Ansicht, dass für die Stelle 5 Jahre C#-Erfahrung erforderlich sind und teilt dies Michael Alexander in einer Anmerkung mit.  
...
5. Peter arbeitet an der geänderten Anforderung von Michael und akzeptiert Sie. Als Nächstes muss die Stellenausschreibung vom Entwicklungsvorstand Tsvi Reiter genehmigt werden.  
...
8. Die Anforderung wird aus dem Posteingang von Michael entfernt, und der Prozess der Einstellungsweise eines SGS hat jetzt begonnen.  
