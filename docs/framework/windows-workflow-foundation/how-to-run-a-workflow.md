...
> [!NOTE]
> Ein Thema im Lernprogramm "Erste Schritte" hängt jeweils von den vorherigen Themen ab. Um dieses Thema abzuschließen, müssen Sie zuerst [Folgendes ausführen: Erstellen Sie eine](how-to-create-an-activity.md) Aktivität [, und Gewusst wie: Erstellen Sie einen](how-to-create-a-workflow.md)Workflow.
...
1. Öffnen Sie die Projekt Mappe aus [der vorherigen Vorgehensweise: Erstellen Sie ein](how-to-create-an-activity.md) Aktivitäts Thema mithilfe von Visual Studio 2012.  
...
1. Doppelklicken Sie im **Projektmappen-Explorer** auf **Program.cs** oder **Module1.vb** , um den Code anzuzeigen.
    ...
    > [!NOTE]
    > Ersetzen `Workflow1` Sie in diesen Beispielen durch `FlowchartNumberGuessWorkflow`, `SequentialNumberGuessWorkflow`oder `StateMachineNumberGuessWorkflow`, je nachdem, welchen Workflow Sie im vorherigen [Abschnitt wie folgt abgeschlossen haben: Erstellen Sie einen](how-to-create-a-workflow.md) Workflow Schritt. Wenn Sie `Workflow1` nicht ersetzen, erhalten Sie Buildfehler, wenn Sie versuchen, den Workflow zu erstellen oder auszuführen.
...
2. Drücken Sie STRG+F5, um die Anwendung zu erstellen und auszuführen. Versuchen Sie, die Zahl in möglichst wenigen Durchgängen zu erraten.
     ...
     Anweisungen zum Hinzufügen von Persistenz zu einer Workflow Anwendung finden Sie im nächsten Thema [Vorgehensweise: Erstellen und Ausführen eines Workflows](how-to-create-and-run-a-long-running-workflow.md)mit langer Laufzeit.
