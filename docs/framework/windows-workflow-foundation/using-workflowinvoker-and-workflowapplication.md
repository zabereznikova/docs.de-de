---
title: "Verwenden von WorkflowInvoker und WorkflowApplication | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
[!INCLUDE[wf](../../../includes/wf-md.md)] bietet mehrere Methoden zum Hosten von Workflows.<xref:System.Activities.WorkflowInvoker> stellt eine einfache Möglichkeit zum Aufrufen eines Workflows bereit, als ob es sich dabei um einen Methodenaufruf handeln würde, und kann nur für Workflows verwendet werden, die keine Persistenz verwenden.<xref:System.Activities.WorkflowApplication> bietet ein umfangreicheres Modell zum Ausführen von Workflows mit Benachrichtigung über Lebenszyklusereignisse, Ausführungssteuerung, Wiederaufnahme von Lesezeichen und Persistenz.<xref:System.ServiceModel.Activities.WorkflowServiceHost> umfasst die Unterstützung von Messagingaktivitäten und wird hauptsächlich in Verbindung mit Workflowdiensten verwendet.In diesem Thema wird das Workflowhosting mit <xref:System.Activities.WorkflowInvoker> und <xref:System.Activities.WorkflowApplication> eingeführt.[!INCLUDE[crabout](../../../includes/crabout-md.md)] zum Hosten von Workflows mit <xref:System.ServiceModel.Activities.WorkflowServiceHost> finden Sie unter [Workflowdienste](../../../docs/framework/wcf/feature-details/workflow-services.md) und [Übersicht über das Hosten von Workflowdiensten](../../../docs/framework/wcf/feature-details/hosting-workflow-services-overview.md).  
...
**Steve**   
**Hallo, Steve**  [!code-csharp[CFX_WorkflowApplicationExample#14](../../../samples/snippets/csharp/VS_Snippets_CFX/cfx_workflowapplicationexample/cs/program.cs#14)]  
  
 Im folgenden Codebeispiel werden die in den <xref:System.Activities.WorkflowApplication.Idle%2A>\-Handler einer <xref:System.Activities.WorkflowApplication>\-Instanz übergebenen <xref:System.Activities.WorkflowApplicationIdleEventArgs> überprüft.In diesem Beispiel geht der Workflow in den Leerlauf über und verfügt über ein <xref:System.Activities.Bookmark> mit dem Namen `EnterGuess`, das sich im Besitz einer Aktivität mit dem Namen `ReadInt` befindet.Dieses Codebeispiel basiert auf [Vorgehensweise: Ausführen eines Workflows](../../../docs/framework/windows-workflow-foundation//how-to-run-a-workflow.md), das Bestandteil des [Lernprogramm 'Erste Schritte'](../../../docs/framework/windows-workflow-foundation//getting-started-tutorial.md) ist.Wenn der <xref:System.Activities.WorkflowApplication.Idle%2A>\-Handler in diesem Schritt geändert wird, um den Code aus diesem Beispiel zu enthalten, wird die folgende Ausgabe angezeigt.  
