---
title: Anordnen von Steuerelementen in Windows Forms
ms.date: 03/30/2017
f1_keywords:
- VisualSelection
helpviewer_keywords:
- controls [Windows Forms], positioning
- Windows Forms controls, positioning on form
ms.assetid: b2d62ed8-c391-4a7e-b72e-6bbabfca73dc
ms.openlocfilehash: 7a131df84cb7e02bb23558c7f1d2a6d3f0cfadd3
ms.sourcegitcommit: 0d0a6e96737dfe24d3257b7c94f25d9500f383ea
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 05/07/2019
ms.locfileid: "65211221"
---
# <a name="arranging-controls-on-windows-forms"></a>Anordnen von Steuerelementen in Windows Forms
Durch die verschiedenen Möglichkeiten, Steuerelemente in Formularen zu platzieren und zu bearbeiten, können Sie Benutzeroberflächen erstellen, die gleichermaßen intuitiv und funktionell sind.

## <a name="in-this-section"></a>In diesem Abschnitt
 [Vorgehensweise: Ausrichten von mehreren Steuerelementen in Windows Forms](how-to-align-multiple-controls-on-windows-forms.md)\
 Hier erfahren Sie, wie Sie die Positionen mehrerer Steuerelemente in Windows Forms aufeinander ausrichten.

 [Vorgehensweise: Verankern von Steuerelementen in Windows Forms](how-to-anchor-controls-on-windows-forms.md)\
 Hier erfahren Sie, wie Steuerelemente eingerichtet werden, deren Größe zur Laufzeit dynamisch verändert wird.

 [Vorgehensweise: Kopieren von Steuerelementen zwischen Windows Forms](how-to-copy-controls-between-windows-forms.md)\
 Hier erfahren Sie, wie Sie Steuerelemente zwischen Formularen duplizieren.

 [Vorgehensweise: Andocken von Steuerelementen in Windows Forms](how-to-dock-controls-on-windows-forms.md)\
 Hier erfahren Sie, wie Sie Steuerelemente an den Seiten eines Formulars „anheften“.

 [Vorgehensweise: Überlagern von Objekten in Windows Forms](how-to-layer-objects-on-windows-forms.md)\
 Hier erfahren Sie, wie Sie festlegen, welche Steuerelemente relativ zur Z-Achse (Z-Anordnung) oben angeordnet werden.

 [Vorgehensweise: Steuerelemente Sperren zu Windows Forms](how-to-lock-controls-to-windows-forms.md)\
 Hier erfahren Sie, wie Sie Steuerelemente dauerhaft mit einem Formular verbinden.

 [Vorgehensweise: Position-Steuerelementen in Windows Forms](how-to-position-controls-on-windows-forms.md)\
 Hier erfahren Sie, wie Sie die Koordinaten der Steuerelemente im Formular festlegen.

 [Vorgehensweise: Größe von Steuerelementen in Windows Forms](how-to-resize-controls-on-windows-forms.md)\
 Hier erfahren Sie, wie Sie die Größe von Steuerelementen im Formular festlegen.

 [Vorgehensweise: Festlegen von Rasteroptionen für alle Windows Forms](how-to-set-grid-options-for-all-windows-forms.md)\
 Hier erfahren Sie, wie Sie die Größe des Rasters kalibrieren, das ein Formular überlagert.

 [Vorgehensweise: Festlegen der Aktivierreihenfolge in Windows Forms](how-to-set-the-tab-order-on-windows-forms.md)\
 Hier erfahren Sie, wie Sie steuern, in welcher Reihenfolge Steuerelemente den Fokus erhalten, wenn der Benutzer die TAB-TASTE drückt.

 [Vorgehensweise: Anordnen von Steuerelementen mithilfe der Ausrichtungslinien und des Rasters in Windows Forms](how-to-arrange-controls-with-snaplines-and-the-grid-in-windows-forms.md)\
 Hier erfahren Sie, wie Sie Steuerelemente am Raster eines Formulars befestigen.

 [Vorgehensweise: Erneutes Zuweisen von vorhandenen Steuerelementen zu einem anderen übergeordneten Element](how-to-reassign-existing-controls-to-a-different-parent.md)\
 Hier erfahren Sie, wie vorhandene Steuerelemente einem neuen übergeordneten Container zugewiesen werden.

 [Exemplarische Vorgehensweise: Anordnen von Windows Forms-Steuerelementen mithilfe von Abständen, Rändern und der AutoSize-Eigenschaft](windows-forms-controls-padding-autosize.md)\
 Beschreibt, wie Sie mithilfe von Steuerelementen in Formularen platzieren können die <xref:System.Windows.Forms.Control.Margin%2A>, <xref:System.Windows.Forms.Control.Padding%2A>, und <xref:System.Windows.Forms.Control.AutoSize%2A> Eigenschaften innerhalb der **Formulardesigner** in Visual Studio.

 [Exemplarische Vorgehensweise: Anordnen von Steuerelementen in Windows Forms mithilfe von Ausrichtungslinien](walkthrough-arranging-controls-on-windows-forms-using-snaplines.md)\
 Demonstriert die verschiedenen Layoutmöglichkeiten mit Ausrichtungslinien.

## <a name="related-sections"></a>Verwandte Abschnitte
 [Vorgehensweise: Festlegen einer Windows Forms-Schaltfläche als "Abbrechen"-Schaltfläche mithilfe des Designers](designate-a-wf-button-as-the-cancel-button-using-the-designer.md)\
 Hier erfahren Sie, wie Sie eine Schaltfläche als das Steuerelement festlegen, mit dem die Anzeige des Formulars beendet wird.

 [Vorgehensweise: Definieren Sie eine Windows Forms-Schaltfläche als "Annehmen"-Schaltfläche mithilfe des Designers](designate-a-wf-button-as-the-accept-button-using-the-designer.md)\
 Hier erfahren Sie, wie Sie eine Schaltfläche (häufig die Schaltfläche „OK“) so konfigurieren, dass sie beim Drücken der EINGABETASTE als „Eingabe akzeptieren“-Schaltfläche behandelt wird, und zwar unabhängig davon, an welcher Stelle sich der Fokus im Dialogfeld befindet.

 [Vorgehensweise: Gruppe Windows Forms-RadioButton-Steuerelementen in als Gruppe](how-to-group-windows-forms-radiobutton-controls-to-function-as-a-set.md)\
 Hier erfahren Sie, wie Sie eine Reihe von `RadioButton`-Steuerelementen miteinander verknüpfen.

 [Windows Forms-Steuerelementen](index.md)\
 Stellt allgemeine Informationen zu Steuerelementen bereit.
