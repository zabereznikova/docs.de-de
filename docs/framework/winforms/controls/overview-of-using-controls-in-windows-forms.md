...
In diesem Thema werden die wesentlichen Elemente einer Windows Forms-Anwendung beschrieben und ein einfaches Beispiel für Steuerelemente und Handles in Ereignissen in einer Windows Forms-Anwendung beschrieben.  
...
- Eine oder mehrere Klassen, <xref:System.Windows.Forms.Form?displayProperty=nameWithType>die von stammen.  
  
- Eine `Main` Methode, die `static` `shared` die (in <xref:System.Windows.Forms.Application.Run%2A> Visual Basic) Methode aufruft und eine <xref:System.Windows.Forms.Form> Instanz an sie übergibt. Die <xref:System.Windows.Forms.Application.Run%2A> Methode verarbeitet Nachrichten vom Betriebssystem an die Anwendung.  
...
## <a name="using-controls-in-a-windows-forms-application"></a>Verwenden von Steuerelementen in einer Windows Forms-Anwendung  
 Das folgende Codebeispiel zeigt eine einfache Anwendung, die veranschaulicht, wie Windows Forms-Anwendungen Steuerelemente verwenden und Ereignisse verarbeiten. Das Beispiel besteht aus drei Schaltflächen in einem Formular. jede Schaltfläche ändert die Hintergrundfarbe, wenn Sie darauf klicken.  
