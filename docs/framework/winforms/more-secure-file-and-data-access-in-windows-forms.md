...
### <a name="other-files"></a>Weitere Dateien  
 ...
 Im folgenden Beispiel wird der isolierte Speicher dazu verwendet, Daten in eine Datei in einem Speicher zu schreiben. Für dieses Beispiel sind <xref:System.Security.Permissions.IsolatedStorageFilePermission> und der <xref:System.Security.Permissions.IsolatedStorageContainment.DomainIsolationByUser>-Enumerationswert erforderlich. In diesem Beispiel wird das Lesen und Schreiben bestimmter Eigenschaftswerte des <xref:System.Windows.Forms.Button>-Steuerelements in einer Datei im isolierten Speicher gezeigt. Die `Read`-Funktion wird nach dem Starten der Anwendung aufgerufen, und die `Write`-Funktion vor dem Beenden der Anwendung. Im Beispiel erfordert, dass die `Read` und `Write` Funktionen, die als Mitglieder der vorhanden ist eine <xref:System.Windows.Forms.Form> , enthält eine <xref:System.Windows.Forms.Button> -Steuerelement namens `MainButton`.  
