...
`DataDirectory` und der ~-Operator werden im <xref:System.Data.EntityClient.EntityConnection.ConnectionString%2A> als Teil der Schlüsselwörter `Metadata` und `Provider Connection String` verwendet. Die <xref:System.Data.EntityClient.EntityConnection> leitet den `DataDirectory`- und den ~-Operator an <xref:System.Data.Metadata.Edm.MetadataWorkspace> bzw. den Speicheranbieter weiter.
