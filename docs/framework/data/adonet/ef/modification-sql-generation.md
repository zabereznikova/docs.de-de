...
- DbIsNullExpression für einen DbPropertyExpression als eingeschränkte unten
...
Die erste Vorlage verfügt über einen Befehl, mit dem der Einfügevorgang mithilfe der Werte in der Liste mit SetClauses durchgeführt werden kann, sowie über eine SELECT-Anweisung, um die in der Returning-Eigenschaft für die eingefügte Zeile angegebenen Eigenschaften zurückzugeben, sofern die Returning-Eigenschaft nicht NULL ist. Das prädikatelement "\@ @ROWCOUNT > 0" ist true, wenn eine Zeile eingefügt wurde. Das prädikatelement "KeyMemberI = KeyValueI &#124; SCOPE_IDENTITY()-Funktion" nimmt die Form "KeyMemberI = SCOPE_IDENTITY()" "nur, wenn KeyMemberI einen vom Speicher generierte Schlüssel ist, da SCOPE_IDENTITY()-Funktion gibt, den letzten Identitätswert, der in einer Identität (eingefügt zurück die Spalte im Speicher generierte).
