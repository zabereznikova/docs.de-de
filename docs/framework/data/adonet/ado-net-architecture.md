...
## <a name="adonet-components"></a>ADO.NET-Komponenten  
 Die zwei Hauptkomponenten von ADO.net für den Zugriff auf und die Bearbeitung von Daten sind die .NET Framework-Datenanbieter und <xref:System.Data.DataSet> .  
...
### <a name="choosing-a-datareader-or-a-dataset"></a>Auswählen eines "DataReader" oder eines "DataSet"  
 Wenn Sie entscheiden, ob die Anwendung eine `DataReader` (siehe [Abrufen von Daten mit einem DataReader](retrieving-data-using-a-datareader.md)) oder eine `DataSet` (siehe [Datasets, DataTables und DataViews](./dataset-datatable-dataview/index.md)) verwenden soll, berücksichtigen Sie die Art der Funktionalität, die für die Anwendung erforderlich ist. Ein `DataSet` sollten Sie für folgende Aufgaben verwenden:  
