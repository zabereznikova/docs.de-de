...
Das folgende Diagramm zeigt ein konzeptionelles Modell mit vier Entitäts `Book`Typen `FictionBook`: `Publisher`,, `Author`und. Der `FictionBook`-Entitätstyp ist ein abgeleiteter Typ, der vom `Book`-Entitätstyp erbt. Der `FictionBook`-Typ erbt die Eigenschaften `ISBN (Key)`, `Title` und `Revision`, und definiert die zusätzliche Eigenschaft `Genre`.  
