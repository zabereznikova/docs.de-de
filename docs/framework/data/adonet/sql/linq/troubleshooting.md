...
## <a name="memory-issues"></a>Arbeitsspeicherprobleme  
 Wenn eine Abfrage eine Auflistung im Arbeitsspeicher und [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] <xref:System.Data.Linq.Table%601>enthält, wird die Abfrage möglicherweise im Arbeitsspeicher ausgeführt, abhängig von der Reihenfolge, in der die beiden Auflistungen angegeben sind. Wenn die Abfrage im Arbeitsspeicher ausgeführt werden muss, müssen die Daten aus der Datenbanktabelle abgerufen werden.  
...
## <a name="class-library-projects"></a>Klassenbibliotheksprojekte  
 Das objektrelationaler Designer erstellt eine Verbindungs Zeichenfolge `app.config` in der Datei des Projekts. In Klassenbibliotheksprojekten wird die Datei `app.config` nicht verwendet. [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] verwendet die in den Entwurfszeitdateien bereitgestellte Verbindungszeichenfolge. Wenn Sie den Wert in `app.config` ändern, wird die Datenbank, mit der die Anwendung eine Verbindung herstellt, nicht geändert.  
...
- Verwenden Sie eigenen Code, um zunächst die untergeordneten Objekte zu löschen, die das Löschen des übergeordneten Objekts verhindern.  
 ...
 Weitere Informationen finden Sie unter [Vorgehensweise: Löschen von Zeilen aus der](how-to-delete-rows-from-the-database.md)Datenbank.  
