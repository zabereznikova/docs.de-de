...
## <a name="dynamic-compilation"></a>Dynamische Kompilierung
 Wenn Sie <xref:System.Transactions> zur Verwendung in eine ASP.NET-Anwendung importieren möchten, die bei Zugriff dynamisch kompiliert wird, sollten Sie einen Verweis auf die <xref:System.Transactions>-Assembly in die Konfigurationsdatei einfügen. Insbesondere sollte der Verweis im Abschnitt `compilation/assemblies` der standardmäßigen **Web. config** -Konfigurationsdatei oder der Konfigurationsdatei einer bestimmten Webanwendung hinzugefügt werden. Dies wird im folgenden Beispiel veranschaulicht:
