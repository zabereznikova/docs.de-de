---
title: "&#220;berlegungen zu LINQ (WCF Data Services) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
helpviewer_keywords: 
  ...
  - "Abfragen von WCF Data Services"
...
## <a name="composing-linq-queries"></a>Verfassen von LINQ-Abfragen  
 LINQ ermöglicht es Ihnen, Abfragen für eine Auflistung von Objekten zu verfassen, die implementiert <xref:System.Collections.Generic.IEnumerable%601>. Sowohl die **Hinzufügen eines Dienstverweises** im Dialogfeld [!INCLUDE[vs_current_short](../../../../includes/vs-current-short-md.md)] und das Tool DataSvcUtil.exe zum Generieren einer Darstellung einer [!INCLUDE[ssODataShort](../../../../includes/ssodatashort-md.md)] -Diensts als entitätscontainerklasse, die von erbt <xref:System.Data.Services.Client.DataServiceContext>sowie Objekten, die die zurückgegebenen Entitäten in Feeds, darstellen. Diese Tools generieren auch Eigenschaften der Entitätscontainerklasse für die Auflistungen, die als Feeds vom Dienst verfügbar gemacht werden. Jede dieser Eigenschaften der Klasse, die den Datendienst zurück kapselt ein <xref:System.Data.Services.Client.DataServiceQuery%601>. Da die <xref:System.Data.Services.Client.DataServiceQuery%601> -Klasse implementiert die <xref:System.Linq.IQueryable%601> von LINQ definierte Schnittstelle können Sie eine LINQ-Abfrage für vom Datendienst verfügbar gemachte Feeds von der Clientbibliothek in einen Abfrage-URI übersetzt, die an den Datendienst Ausführung ist verfassen.  
  
> [!IMPORTANT]
>  In der LINQ-Syntax können mehr Abfragen ausgedrückt werden als in der von [!INCLUDE[ssODataShort](../../../../includes/ssodatashort-md.md)]-Datendiensten verwendeten URI-Syntax. Ein <xref:System.NotSupportedException> wird ausgelöst, wenn die Abfrage keinem URI im Zieldatendienst zugeordnet werden kann. [!INCLUDE[crdefault](../../../../includes/crdefault-md.md)]die [nicht unterstützte LINQ-Methoden](../../../../docs/framework/data/wcf/linq-considerations-wcf-data-services.md#unsupportedMethods) in diesem Thema.  
...
```  
 ...
 Weitere allgemeine Informationen über LINQ finden Sie unter [LINQ (Language-Integrated Query)](../Topic/LINQ%20\(Language-Integrated%20Query\).md).  
 ...
 Der [!INCLUDE[ssAstoria](../../../../includes/ssastoria-md.md)]-Client kann beide Abfragearten in einen Abfrage-URI übersetzen, und Sie können eine LINQ-Abfrage erweitern, indem Sie Abfragemethoden an einen Abfrageausdruck anfügen. Wenn Sie LINQ-Abfragen verfassen, indem Sie Methodensyntax an einen Abfrageausdruck anfügen oder einen <xref:System.Data.Services.Client.DataServiceQuery%601>, die Operationen zum Abfrage-URI in der Reihenfolge, in denen Methoden aufgerufen werden, hinzugefügt. Dies entspricht dem Aufrufen der <xref:System.Data.Services.Client.DataServiceQuery%601.AddQueryOption%2A> -Methode zum Hinzufügen von einzelnen Abfrageoptionen zum Abfrage-URI.  
  
## <a name="executing-linq-queries"></a>Ausführen von LINQ-Abfragen  
 Bestimmte LINQ-Abfragemethoden wie z. B. <xref:System.Linq.Enumerable.First%60%601%28System.Collections.Generic.IEnumerable%7B%60%600%7D%29> oder <xref:System.Linq.Enumerable.Single%60%601%28System.Collections.Generic.IEnumerable%7B%60%600%7D%29>an die Abfrage angefügt wird die Abfrage ausgeführt werden soll. Eine Abfrage wird auch ausgeführt, wenn Ergebnisse implizit aufgezählt werden, z. B. während einer `foreach`-Schleife oder wenn die Abfrage einer `List`-Auflistung zugewiesen wird. Weitere Informationen finden Sie unter [Abfragen des Datendiensts](../../../../docs/framework/data/wcf/querying-the-data-service-wcf-data-services.md).  
  
 Der Client führt eine LINQ-Abfrage in zwei Teilen aus. Nach Möglichkeit werden LINQ-Ausdrücke in einer Abfrage zuerst auf dem Client ausgewertet, und anschließend wird eine URI-basierte Abfrage generiert und zur Auswertung anhand der Daten im Dienst an den Datendienst gesendet. Weitere Informationen finden Sie im Abschnitt [Client serverausführung](../../../../docs/framework/data/wcf/querying-the-data-service-wcf-data-services.md#executingQueries) in [Abfragen des Datendiensts](../../../../docs/framework/data/wcf/querying-the-data-service-wcf-data-services.md).  
...
> [!NOTE]
>  Die `$select` Abfrageoption kann nicht in einem Abfrage-URI hinzugefügt werden, mithilfe der <xref:System.Data.Services.Client.DataServiceQuery%601.AddQueryOption%2A> Methode. Wir empfehlen die Verwendung von LINQ <xref:System.Linq.Enumerable.Select%2A> Methode, um dem Client generiert die `$select` -Abfrageoption im Anforderungs-URI.\</TSource, TResult>  
...
### <a name="expand"></a>Erweitern  
 Beim Abfragen eines [!INCLUDE[ssODataShort](../../../../includes/ssodatashort-md.md)]-Datendiensts können Sie anfordern, dass mit der Zielentität der Abfrage verknüpfte Entitäten in den zurückgegebenen Feed eingeschlossen werden. Die <xref:System.Data.Services.Client.DataServiceQuery%601.Expand%2A> Methode aufgerufen wird die <xref:System.Data.Services.Client.DataServiceQuery%601> legen Sie für die Entitätenmenge der LINQ-Abfrage, mit der verknüpften Entität als Namen der `path` Parameter. Weitere Informationen finden Sie unter [Content verzögerte Laden von](../../../../docs/framework/data/wcf/loading-deferred-content-wcf-data-services.md).  
  
 Die folgenden Beispiele zeigen die äquivalente Verfahren, um die Verwendung der <xref:System.Data.Services.Client.DataServiceQuery%601.Expand%2A> -Methode in einer Abfrage:  
...
|<xref:System.DateTime> Element<sup>1</sup>|Unterstützte [!INCLUDE[ssODataShort](../../../../includes/ssodatashort-md.md)]-Funktion|  
|-------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------|  
...
|<xref:System.DateTime.Year>|`int year(DateTime p0)`|  
 ...
 <sup>1</sup>das entsprechende Datum und die Eigenschaften des <xref:Microsoft.VisualBasic.DateAndTime?displayProperty=fullName>, als auch die <xref:Microsoft.VisualBasic.DateAndTime.DatePart%2A> -Methode in Visual Basic werden ebenfalls unterstützt.  
...
|<xref:> System.Linq.Expressions.Expression?qualifyHint=False&autoUpgrade=True Member|Unterstützte [!INCLUDE[ssODataShort](../../../../includes/ssodatashort-md.md)]-Funktion|  
|---------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------|  
|<xref:System.Linq.Expressions.Expression.TypeIs%28System.Linq.Expressions.Expression%2CSystem.Type%29>|`bool isof(type p0)`|  
  
 Der Client kann möglicherweise auch weitere CLR-Funktionen auf dem Client auswerten. Ein <xref:System.NotSupportedException> für einen Ausdruck, nicht auf dem Client ausgewertet werden und nicht in einen gültigen URI für die Auswertung auf dem Server übersetzt werden, ausgelöst.  
