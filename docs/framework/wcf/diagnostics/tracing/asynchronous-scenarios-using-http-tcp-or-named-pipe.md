...
### <a name="asynchronous-client-with-callback"></a>Asynchroner Client mit Rückruf  
 ...
 Dieser Abschnitt veranschaulicht lediglich die Verwendung von HTTP mit `propagateActivity` = `true`. Jedoch die zusätzlichen Aktivitäten und Übertragungen gelten auch für die anderen Fällen (d. h. `propagateActivity` = `false`, mit TCP oder Named Pipes).  
