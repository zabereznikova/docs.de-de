---
title: "Entwerfen von Dienstvertr&#228;gen | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
>  Bei der Vererbung von anderen Dienstvertragsschnittstellen können Sie Vorgangseigenschaften, wie z. B. den Name und den Namespace, nicht außer Kraft setzen.Wenn Sie dies dennoch versuchen, erstellen Sie im aktuellen Dienstvertrag einen neuen Vorgang.  
 ...
 [!INCLUDE[crexample](../../../includes/crexample-md.md)]r Erstellung eines Dienstvertrags mithilfe einer Schnittstelle finden Sie unter [Vorgehensweise: Erstellen eines Diensts mit einer Vertragsschnittstelle](../../../docs/framework/wcf/feature-details/how-to-create-a-service-with-a-contract-interface.md).  
 ...
 Ein Beispiel für die Verwendung einer Klasse zum Erstellen eines Dienstvertrags mit gleichzeitiger Implementierung finden Sie unter [Vorgehensweise: Erstellen eines Diensts mit einer Vertragsklasse](../../../docs/framework/wcf/feature-details/how-to-create-a-wcf-contract-with-a-class.md).  
...
##### Duplex  
 ...
 [!INCLUDE[crexample](../../../includes/crexample-md.md)] Erstellen eines Diensts und eines Clients, der auf diesen Dienst zugreift, finden Sie unter [Vorgehensweise: Erstellen eines Duplexvertrags](../../../docs/framework/wcf/feature-details/how-to-create-a-duplex-contract.md) und [Vorgehensweise: Zugreifen auf Dienste mit einem Duplexvertrag](../../../docs/framework/wcf/feature-details/how-to-access-services-with-a-duplex-contract.md).Ein Arbeitsbeispiel finden Sie unter [Duplex](../../../docs/framework/wcf/samples/duplex.md).[!INCLUDE[crabout](../../../includes/crabout-md.md)]\-Probleme mit Duplexverträgen finden Sie unter [Duplexdienste](../../../docs/framework/wcf/feature-details/duplex-services.md).  
...
```  
 ...
 Beim Interagieren mit einer `ISampleService`\-Implementierung in einem Endpunkt mit einer standardmäßigen <xref:System.ServiceModel.WSHttpBinding> \(der standardmäßige <xref:System.ServiceModel.SecurityMode?displayProperty=fullName> ist <xref:System.ServiceModel.SecurityMode>\) werden alle Meldungen verschlüsselt und signiert, da es sich dabei um die Standardschutzebene handelt.Wenn jedoch ein `ISampleService`\-Dienst mit einer standardmäßigen <xref:System.ServiceModel.BasicHttpBinding> verwendet wird \(der standardmäßige <xref:System.ServiceModel.SecurityMode> ist <xref:System.ServiceModel.SecurityMode>\), werden alle Nachrichten als Text gesendet, da es keine Sicherheit für diese Bindung gibt und somit die Schutzebene ignoriert wird \(die Nachrichten werden also weder verschlüsselt noch signiert\).Würde der <xref:System.ServiceModel.SecurityMode> in <xref:System.ServiceModel.SecurityMode> geändert, würden die Nachrichten verschlüsselt und signiert \(da es sich in diesem Fall um die Standardschutzebene der Bindung handeln würde\).  
...
```  
 ...
 Ein Dienst, der diesen `IExplicitProtectionLevelSampleService`\-Vertrag implementiert und über einen Endpunkt verfügt, der die standardmäßige <xref:System.ServiceModel.WSHttpBinding> \(der standardmäßige <xref:System.ServiceModel.SecurityMode?displayProperty=fullName> ist <xref:System.ServiceModel.SecurityMode>\) verwendet, hat das folgende Verhalten:  
...
-   Der `GetGuid`\-Vorgang <xref:System.Guid?displayProperty=fullName> wird in einer Nachricht zurückgegeben, die verschlüsselt und signiert wird.  
  
 [!INCLUDE[crabout](../../../includes/crabout-md.md)] zu Schutzebenen und deren Verwendung finden Sie unter [Grundlagen der Schutzebene](../../../docs/framework/wcf/understanding-protection-level.md).[!INCLUDE[crabout](../../../includes/crabout-md.md)] zur Sicherheit finden Sie unter [Sichern von Diensten](../../../docs/framework/wcf/securing-services.md).  
...
##### Andere Vorgangssignaturanforderungen  
 ...
 Ein anderes Beispiel ist die Verwendung von <xref:System.IO.Stream>\-Typen in Vorgängen.Da der <xref:System.IO.Stream>\-Parameter den gesamten Textinhalt einschließt, muss es sich bei einer Ein\- oder Ausgabe mit den Parametern `ref`, `out` oder einem Rückgabewert vom Typ <xref:System.IO.Stream> um die einzigen im Vorgang angegebenen Ein\- und Ausgaben handeln.Außerdem muss es sich bei dem Parameter oder dem Rückgabetyp um <xref:System.IO.Stream>, <xref:System.ServiceModel.Channels.Message?displayProperty=fullName> oder <xref:System.Xml.Serialization.IXmlSerializable?displayProperty=fullName> handeln.[!INCLUDE[crabout](../../../includes/crabout-md.md)] zu Datenströmen finden Sie unter [Umfangreiche Daten und Streaming](../../../docs/framework/wcf/feature-details/large-data-and-streaming.md).  
  
##### Namen, Namespaces und Verbergen  
...
## Siehe auch  
 [Gewusst wie: Erstellen eines Anforderung\-Antwort\-Vertrags](../../../docs/framework/wcf/feature-details/how-to-create-a-request-reply-contract.md)   
