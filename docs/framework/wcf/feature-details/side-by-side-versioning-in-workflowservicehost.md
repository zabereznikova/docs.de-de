...
### <a name="configuring-the-definitionidentity"></a>Konfigurieren der DefinitionIdentity  
 Wenn ein Workflow Dienst mit dem Workflow-Designer erstellt wird, <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> wird mit dem **Eigenschaften** Fenster festgelegt. Klicken Sie außerhalb der Stamm Aktivität des Dienstanbieter im Designer, um den Workflow Dienst auszuwählen, und wählen Sie im Menü **Ansicht** die Option **Eigenschaften Fenster** aus. Wählen Sie in der Dropdown Liste den Eintrag **workflowidentity** aus, der neben der **DefinitionIdentity** -Eigenschaft angezeigt wird, und <xref:System.Activities.WorkflowIdentity> erweitern Sie dann die gewünschten Eigenschaften. Im folgenden Beispiel wird der <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> mit dem <xref:System.Activities.WorkflowIdentity.Name%2A> `MortgageWorkflow` und <xref:System.Activities.WorkflowIdentity.Version%2A> dem von `1.0.0.0`konfiguriert. <xref:System.Activities.WorkflowIdentity.Package%2A> ist optional und ist in diesem Beispiel `null`.  
 ...
 Für einen selbst gehosteten Workflowdienst wird <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> beim Erstellen des Workflowdiensts konfiguriert. Im <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> folgenden Beispiel wird der mit denselben Werten wie im vorherigen Beispiel konfiguriert, mit dem <xref:System.Activities.WorkflowIdentity.Name%2A> `MortgageWorkflow` und <xref:System.Activities.WorkflowIdentity.Name%2A> dem von `1.0.0.0`.  
...
```  
 ...
 Eine <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> ist nicht erforderlich, obwohl nur eine Version des Workflow dienstanes einen **null**<xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A>-Wert aufweisen darf.  
...
### <a name="adding-a-new-version-to-a-web-hosted-workflow-service"></a>Hinzufügen einer neuen Version zu einem im Web gehosteten Workflowdienst  
 ...
 Wenn der Dienst neu gestartet wird, wird die frühere Version automatisch der <xref:System.ServiceModel.Activities.WorkflowServiceHost.SupportedVersions%2A>-Auflistung hinzugefügt, da sie sich im angegebenen Unterordner `App_Code` befindet. Beachten Sie Folgendes: Wenn die primäre Version des Workflow Dienstanbieter über eine `null` <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> verfügt, werden die vorherigen Versionen nicht hinzugefügt. Eine Version darf eine `null`-<xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> aufweisen, wenn jedoch mehrere Versionen vorhanden sind, darf die primäre Version nicht die Version mit der `null`-<xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> sein; andernfalls werden die früheren Versionen nicht der <xref:System.ServiceModel.Activities.WorkflowServiceHost.SupportedVersions%2A>-Auflistung hinzugefügt.  
