...
|und|Verwendet die <xref:System.ServiceModel.Dispatcher.StrictAndMessageFilter>-Klasse, die vor der Rückgabe immer beide Bedingungen auswertet.|"filterdata" wird nicht verwendet. Stattdessen verfügen "filter1" und "filter2" haben die Namen der entsprechenden Nachrichtenfilter (ebenfalls in der Tabelle), sollte sich **und**verbunden.|\<filter name="and1" filterType="And" filter1="address1" filter2="action1" />|  
