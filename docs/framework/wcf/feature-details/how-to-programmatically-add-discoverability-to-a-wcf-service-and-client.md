...
In diesem Thema wird erläutert, wie Sie einen Windows Communication Foundation (WCF)-Dienst auffindbar machen. Es basiert auf dem [selbst](https://docs.microsoft.com/dotnet/framework/wcf/samples/self-host) gehosteter Beispiel.  
...
11. Erstellen Sie innerhalb der `InvokeCalculatorService`-Methode eine Instanz der `CalculatorServiceClient`-Klasse. Diese Klasse wird durch das [Self-Host-](https://docs.microsoft.com/dotnet/framework/wcf/samples/self-host) Beispiel definiert. Sie wurde mithilfe von "Svcutil.exe" generiert.  
