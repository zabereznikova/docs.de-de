...
Anwendungen, die mit Windows Communication Foundation (WCF) erstellt wurden, können Sicherheitsereignisse (entweder Erfolg, Fehler oder beides) mit der Überwachungsfunktion protokollieren. Die Ereignisse werden in das Ereignisprotokoll von Windows geschrieben und können in der Ereignisanzeige untersucht werden.  
...
- Nachrichtenebene, in der WCF nach der Gültigkeit der Nachricht sucht und den Aufrufer authentifiziert.  
  
 Sie können beide Überwachungsebenen auf Erfolg oder Fehler überprüfen, was als *Überwachungsverhalten*bezeichnet wird.  
...
|<xref:System.ServiceModel.Description.ServiceSecurityAuditBehavior.SuppressAuditFailure%2A>|Legt fest, was mit der Clientanforderung geschieht, wenn bei der Überwachung ein Fehler auftritt. Zum Beispiel, wenn der Dienst versucht, in das Sicherheitsprotokoll zu schreiben, ohne über die `SeAuditPrivilege`-Berechtigung zu verfügen. Der Standardwert `true` gibt an, dass Fehler ignoriert werden und die Clientanforderung normal verarbeitet wird.|  
 ...
 Ein Beispiel für das Einrichten einer Anwendung zum Protokollieren von Überwachungsereignissen finden Sie unter [Gewusst wie: Überwachen](../../../../docs/framework/wcf/feature-details/how-to-audit-wcf-security-events.md)von Sicherheitsereignissen .  
  
### <a name="configuration"></a>Konfiguration  
 Sie können die Konfiguration auch verwenden, um das Überwachungsverhalten [ \< ](../../../../docs/framework/configure-apps/file-schema/wcf/behaviors.md)anzugeben, indem Sie eine [ \<serviceSecurityAudit->](../../../../docs/framework/configure-apps/file-schema/wcf/servicesecurityaudit.md) unter den Verhaltensweisen>hinzufügen. Sie müssen das Element [ \<](../../../../docs/framework/configure-apps/file-schema/wcf/behavior-of-endpointbehaviors.md) unter einem Verhalten>hinzufügen, wie im folgenden Code gezeigt.  
...
## <a name="security-considerations"></a>Überlegungen zur Sicherheit  
 ...
 Überwachungsereignisse, die in das Anwendungsprotokoll unter Windows XP geschrieben werden, sind für jeden authentifizierten Benutzer sichtbar.  
...
- [\<dienstSecurityAudit>](../../../../docs/framework/configure-apps/file-schema/wcf/servicesecurityaudit.md)
