...
Die Interoperabilität zwischen ASP.NET-Webdiensten und-Windows Communication Foundation (WCF)-Webdiensten kann erreicht werden, indem sichergestellt wird, dass die mithilfe beider Technologien implementierten Dienste der Spezifikation WS-I Basic Profile 1,1 entsprechen. ASP.NET-Webdienste, die WS-I Basic Profile 1,1 entsprechen, sind mithilfe der vom System bereitgestellten WCF-Bindung <xref:System.ServiceModel.BasicHttpBinding>mit WCF-Clients interoperabel.  
  
 Verwenden Sie die Option ASP.NET 2,0, <xref:System.Web.Services.WebService> um <xref:System.Web.Services.WebMethodAttribute> das-Attribut und das-Attribut zu einer Schnittstelle anstatt zu einer Klasse hinzuzufügen, und schreiben Sie eine Klasse, um die Schnittstelle zu implementieren, wie im folgenden Beispielcode gezeigt.  
...
- [Vorgehensweise: Konfigurieren des WCF-Diensts für die Zusammenarbeit mit ASP.NET-Webdienst Clients](../../../../docs/framework/wcf/feature-details/config-wcf-service-with-aspnet-web-service.md)
