---
title: "Messagingprotokolle | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|cdp|http:\/\/schemas.microsoft.com\/net\/2006\/06\/duplex \(möglicherweise in englischer Sprache\)|  
...
-   B1112: [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] gibt die `mustUnderstand`\-Werte 0 \(null\) und 1 sowohl für die SOAP 1.1\- als auch für die SOAP 1.2\-Versionen des SOAP\-Umschlags aus.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] akzeptiert den gesamten Wertbereich von `xs:boolean` für den `mustUnderstand`\-Header \(0, 1, `false`, `true`\)  
  
#### SOAP\-Fehler  
 Die folgende Liste führt spezifische [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]\-SOAP\-Fehler\-Implementierungen auf.  
  
-   B2121: [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] gibt die folgenden SOAP 1.1\-Fehlercodes zurück: `s11:mustUnderstand`, `s11:Client` und `s11:Server`.  
...
#### Endpunktverweise und WS\-Adressierungsversionen  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] implementiert eine Reihe von Infrastrukturprotokollen, die WS\-Adressierung und insbesondere das Element `EndpointReference` und die `W3C.WsAddressing.EndpointReferenceType`\-Klasse verwenden \(u. a. WS\-ReliableMessaging, WS\-SecureConversation und WS\-Trust\).[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] unterstützt die Verwendung beider Versionen der WS\-Adressierung mit anderen Infrastrukturprotokollen.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]\-Endpunkte unterstützen eine Version der WS\-Adressierung pro Endpunkt.  
...
#### Nachrichtenheader  
 Für beide WS\-Adressierungsversionen verwendet [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] die folgenden Nachrichtenheader gemäß den Spezifikationen: `wsa:To`, `wsa:ReplyTo`, `wsa:Action`, `wsa:MessageID`und `wsa:RelatesTo`.  
...
### Nachrichtenaustauschmuster  
 Die Nachrichtensequenz, die am Webdienstvorgangsaufruf beteiligt ist, wird *Nachrichtenaustauschmuster* genannt.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] unterstützt unidirektionale, Anfrage\-Antwort\- und Duplexnachrichten\-Austauschmuster.Dieser Abschnitt klärt die WS\-Adressierungsanforderungen während der Nachrichtenverarbeitung, die vom verwendeten Nachrichtenaustauschmuster abhängig sind.  
...
-   R3321: Der Anforderungsdienst muss in der Anforderung `wsa:To`, `wsa:Action`, `wsa:MessageID` und Header für alle \[Verweisparameter\] oder \[Verweiseigenschaften\], die vom Endpunktverweis angegeben werden, enthalten.  
...
-   R3324: Der Anforderungsdienst muss in der Anforderung `wsa:To`\- `wsa:Action`\- und `wsa:RelatesTo`\-Header und Header für alle \[Verweisparameter\] oder \[Verweiseigenschaften\] \(oder beides\), die vom `ReplyTo`\-Endpunktverweis in der Anforderung angegeben werden, enthalten.  
...
|Code|Ursache|  
|----------|-------------|  
|wsa10:InvalidAddressingHeader|Duplizieren Sie `wsa:To`, `wsa:ReplyTo`, `wsa:From`  oder `wsa:MessageID`.Duplizieren Sie `wsa:RelatesTo`  mit dem gleichen `RelationshipType`.|  
...
#### Verwenden des Endpunktverweises im WSDL\-Anschluss  
 WS\-ADDR10\-WSDL Abschnitt 4.1 erweitert das `wsdl:port`\-Element, um das untergeordnete `<wsa10:EndpointReference…/>`\-Element aufzunehmen, das den Endpunkt in WS\-Adressierungsbegriffen beschreibt.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] erweitert dieses Hilfsprogramm in der WS\-Adressierung 2004\/08, wodurch ermöglicht wird, dass `<wsa:EndpointReference…/>` als untergeordnetes Element von `wsdl:port` angezeigt wird.  
  
-   R3531: Wenn einem Endpunkt eine Richtlinienalternative mit einer `<wsaw10:UsingAddressing/>``wsdl:port-Richtlinienassertion angehängt ist, kann das entsprechende` `<wsa10:EndpointReference …/>-Element ein untergeordnetes Element`  enthalten.  
  
-   R3532: Wenn ein `wsdl:port` ein untergeordnetes Element `<wsa10:EndpointReference …/>``wsa10:EndpointReference/wsa10:Address enthält, muss der Wert des untergeordneten` `@address-Elements dem Wert des` `wsdl:port-Attributs des gleichgeordneten` `wsdl:location/`\-Elements entsprechen.  
  
-   R3533: Wenn einem Endpunkt eine Richtlinienalternative mit einer `<wsap:UsingAddressing/>``wsdl:port-Richtlinienassertion angehängt ist, kann das entsprechende` `<wsa:EndpointReference …/>-Element ein untergeordnetes Element`  enthalten.  
  
-   R3534: Wenn ein `wsdl:port` ein untergeordnetes Element `<wsa:EndpointReference …/>``wsa:EndpointReference/wsa:Address enthält, muss der Wert des untergeordneten` `@address-Elements dem Wert des` `wsdl:port-Attributs des gleichgeordneten` `wsdl:location/`\-Elements entsprechen.  
...
#### Unidirektionale Nachricht  
 ...
 HTTP\-Header: Das Ziel in POST stimmt mit dem URI im Element `wsa10:To`  überein.  
...
4.  Erstellen Sie einen XOP SOAP\-Umschlag, der eine Kopie des Original\-SOAP\-Umschlags ist, aber bei dem das untergeordnete Element jedes Elementinformationselements, das im vorherigen Schritt identifiziert wurde, durch ein `xop:Include`\-Elementinformationselement ersetzt wurde, das wie folgt erstellt wird:  
    ...
    6.  Fügen Sie ein `href`\-Attribut zum `xop:Include`\-Element mit dem Wert "cid: uri" hinzu, der in Schritt 4b aus dem Content\-ID\-Headerwert erstellt wurde.Entfernen Sie die umschließenden Zeichen "\<" und "\>", versehen Sie die verbleibende Zeichenfolge mit URL\-Escape, und fügen Sie das Präfix `cid:` hinzu.Der folgende minimale Zeichensatz ist erforderlich, um RFC1738 und RFC2396 mit einem Escapezeichen zu versehen.Andere Zeichen können mit Escapezeichen versehen werden.  
...
3.  Für jedes Elementinformationselement im erstellten SOAP\-Umschlag, der, als einziges Mitglied seiner \[untergeordneten\] Eigenschaft, ein `xop:Include`\-Elementinformationselement enthält:  
    ...
    1.  Löschen Sie das Präfix `cid:`, und entfernen Sie alle URI\-Escape\-Zeichensequenzen \(RFC 2396\) im Wert des `@href`\-Attributs des Elements `xop:Include`.Umschließen Sie die Ergebniszeichenfolge mit "\<", "\>".  
...
-   R4132: Ein HTTP Content\-Type\-Header muss über einen Typparameter mit dem Wert `application/xop+xml` verfügen, der von doppelten Anführungszeichen umschlossen ist.  
 ...
 Während die Anforderung, Anführungszeichen zu verwenden, in RFC 2387 nicht explizit ist, vermerkt der Text, dass alle der mehrteiligen\/verwandten Medientypparameter am wahrscheinlichsten reservierte Zeichen wie "@" oder "\/" enthalten und daher doppelte Anführungszeichen benötigen.  
...
```  
 ...
 definiert wird und tatsächlich eine von "\< " und "\>" umschlossene E\-Mail\-Adresse ist.Das `[CFWS]`\-Präfix und \-Suffix wurden in RFC 2822 hinzugefügt, um Kommentare auszuführen, und sollten nicht verwendet werden, um die Kompatibilität aufrechtzuerhalten.  
 ...
 Eine Reihe von MIME\-Implementierungen hat die Anforderungen an den Wert, der von "\<" und "\>" umschlossen ist, so gelockert, dass er eine E\-Mail\-Adresse ist und zusätzlich zur E\-Mail\-Adresse den von "\<" , "\>" umschlossenen `absoluteURI` verwendet.Diese Version von [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] verwendet Werte des Content\-ID MIME Headers in der Form:  
...
-   R4148: Ein SOAP1.1 Infosetteil muss den Content\-Type\-Header mit Medientyp application\/xop\+xml und Parametertyp \= "Text\/XML" und charset enthalten.  
...
-   R4149: Der SOAP1.2\-Infosetteil muss den Content\-Type\-Header mit Medientyp `application/xop+xml` und Parametertyp\="`application/soap+xml`" und `charset` enthalten.  
