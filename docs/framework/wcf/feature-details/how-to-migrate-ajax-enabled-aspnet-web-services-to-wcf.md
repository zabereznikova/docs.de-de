...
- Sie können von WCF-Features profitieren, wie z. b. Ablauf Verfolgung, usw.
 ...
 Weitere Informationen zum verfügbar machen eines WCF-Dienstanbieter über einen AJAX-fähigen Endpunkt finden Sie im Thema Gewusst [wie: Verwenden der Konfiguration zum Hinzufügen eines ASP.NET AJAX-Endpunkts](../../../../docs/framework/wcf/feature-details/how-to-use-configuration-to-add-an-aspnet-ajax-endpoint.md) .
...
5. Fügen Sie in die Datei WCFHello.svc.cs anstelle des folgenden Codes ein, um die Implementierung des `HelloWorld` Vorgangs in die Datei einzufügen.
...
6. Geben Sie das `Namespace`-Attribut für <xref:System.ServiceModel.ServiceContractAttribute> als `WCFHello`an.
...
7. Fügen Sie den <xref:System.ServiceModel.Web.WebInvokeAttribute> dem `HelloWorld` Vorgang hinzu, und legen Sie die Eigenschaft <xref:System.ServiceModel.Web.WebInvokeAttribute.ResponseFormat%2A> auf Rückgabe <xref:System.ServiceModel.Web.WebMessageFormat.Xml>fest. Beachten Sie, dass der Standardrückgabetyp <xref:System.ServiceModel.Web.WebMessageFormat.Json> ist, sofern nichts anderes festgelegt wird.
...
11. Die `WCFHello.svc/HelloWorld` und die `Service1.aspx/HelloWorld` Endpunkte sind nun funktional äquivalent.
...
- Wenn zuerst der WCF-Dienst hinzugefügt wird, verursacht der Aufruf der-Methode für den ASMX-Webdienst eine Ausnahme in <xref:System.Web.UI.ObjectConverter.ConvertValue%28System.Object%2CSystem.Type%2CSystem.String%29>, da die WCF-Format Definition der Reihenfolge im Proxy Vorrang hat.

- Wenn der ASMX-Webdienst zuerst hinzugefügt wird, wird beim Aufrufen der Methode für den WCF-Dienst eine Ausnahme in <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer> ausgelöst, da die Webdienst-Format Definition der Reihenfolge im Proxy Vorrang hat.
...
- {"One": 1, "Two": 2} von der ASP.NET AJAX-<xref:System.Web.Script.Serialization.JavaScriptSerializer>
...
|<xref:System.DateTime> UTC (Coordinated Universal Time)|Unterstützt nicht das Format "\\/Date (123456789U)\\/" oder "\\/Date\\(\d + (&#124;U (\\+\\-[\d{4}]))?\\)\\\\/) ".|Unterstützt das Format "\\/Date (123456789U)\\/" und "\\/Date\\(\d +&#124;(U (\\+\\-[\d{4}]))?\\)\\\\/) "als DateTime-Werte.|
