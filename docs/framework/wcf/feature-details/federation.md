...
Dieses Thema enthält eine kurze Übersicht über den Begriff Verbundsicherheit. Außerdem wird Windows Communication Foundation (WCF)-Unterstützung für die Bereitstellung von Verbund Sicherheitsarchitekturen beschrieben. Eine Beispielanwendung, die Verbund veranschaulicht, finden Sie unter [Federation Sample](../../../../docs/framework/wcf/samples/federation-sample.md)(Verbund Beispiel).  
...
### <a name="example-scenario"></a>Beispielszenario  
 ...
 Dieses Szenario umfasst zwei Organisationen: A und b. Organisation b verfügt über eine Webressource (einen Webdienst), die einige Benutzer in der Organisation a wertvoll finden.  
...
## <a name="support-for-federated-security-in-wcf"></a>Unterstützung für Verbundsicherheit in WCF  
 WCF bietet eine sofort einsetzbare Unterstützung für die Bereitstellung von Verbund Sicherheitsarchitekturen über die [ \<WSFederationHttpBinding->](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md).  
  
 [ Das\<WSFederationHttpBinding->](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md) -Element bietet eine sichere, zuverlässige und interoperable Bindung, die die Verwendung von http als zugrunde liegenden Transportmechanismus für den Anforderungs-Antwort-Kommunikationsstil und die Verwendung von Text und XML als Wire-Format für die Codierung.  
  
 Die Verwendung von [ \<WSFederationHttpBinding >](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md) in einem Verbund Sicherheitsszenario kann in zwei logisch unabhängige Phasen entkoppelt werden, wie in den folgenden Abschnitten beschrieben.  
  
### <a name="phase-1-design-phase"></a>Phase 1: Entwurfs Phase  
 Während der Entwurfsphase verwendet der Client das [Service Model Metadata Utility Tool (Svcutil. exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) , um die Richtlinie zu lesen, die vom Dienst Endpunkt verfügbar gemacht wird, und um die Authentifizierungs-und Autorisierungs Anforderungen des Dienstanbieter zu erfassen. Die entsprechenden Proxys werden erzeugt, um das folgende Verbundsicherheitskommunikationsmuster beim Client zu erstellen:  
...
### <a name="phase-2-run-time-phase"></a>Phase 2: Lauf Zeit Phase  
...
### <a name="example-myservice"></a>Beispiel MyService  
 ...
 Der Dienst Endpunkt `MyServiceEndpoint` verwendet die [ \<WSFederationHttpBinding->](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md) und erfordert ein gültiges SAML-Token (Security Assertionen Markup Language `accessAuthorized` ) mit einem von STS B ausgestellten Anspruch. Dies wird deklarativ in der Dienst Konfiguration angegeben.  
...
> [!NOTE]
> Auch hier ist `userAuthenticated` der Anspruch der Anspruchstyp, der von STS B benötigt wird. Der voll qualifizierte Name dieses Anspruchs Typs ist `http://tempuri.org:userAuthenticated` (zusammen mit dem zugeordneten Namespace), der in der STS-Konfigurationsdatei verwendet wird. Der Wert dieses Anspruchs zeigt das Vorhandensein dieses Anspruchs an und wird als von STS A auf `true` festgelegt angenommen.  
