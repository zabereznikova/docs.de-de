...
Aktivieren Sie auch den ASP.NET-Kompatibilitätsmodus in der Web.config-Datei, wie im folgenden Beispiel gezeigt.  
...
```  
 ...
 Anschließend müssen Sie die SQL-Cache Abhängigkeit innerhalb eines <`caching`>-Elements innerhalb des <`system.web`> Elements aktivieren, wie im folgenden Konfigurationsbeispiel gezeigt.  
...
```  
 ...
 Im nächsten Schritt müssen Sie die Ausgabe Cache Einstellungen innerhalb des <>-Elements konfigurieren, `caching` wie im folgenden Beispiel gezeigt.  
...
> [!IMPORTANT]
> Damit ASP.net auf eine SQL-Datenbank zugreifen kann, müssen Sie das [Registrierungs Tool ASP.NET SQL Server](/previous-versions/dotnet/netframework-3.5/ms229862(v=vs.90))verwenden. Außerdem müssen Sie für das entsprechende Benutzerkonto den Zugriff auf die Datenbank und die Tabelle zulassen. Weitere Informationen finden Sie unter [zugreifen auf SQL Server aus einer Webanwendung](/previous-versions/aspnet/ht43wsex(v=vs.100)).  
