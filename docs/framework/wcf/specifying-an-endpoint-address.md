...
description: Erfahren Sie mehr über eine Endpunkt Adresse, einen Teil eines Service Endpoint in WCF. Die gesamte Kommunikation mit einem WCF-Dienst erfolgt über seine Endpunkte.
ms.date: 03/30/2017
dev_langs:
- csharp
- vb
...
Die optionalen Header werden in einem deklariert [\<headers>](../configure-apps/file-schema/wcf/headers-element.md) . Im folgenden finden Sie ein Beispiel für die Elemente, die zum Angeben von Endpunkten für einen Dienst in einer Konfigurationsdatei verwendet werden, die zwischen zwei Headern unterscheidet: "Gold"-Clients von `http://tempuri1.org/` und "Standard"-Clients von `http://tempuri2.org/` . Der Client, der diesen Dienst aufrufen, muss über die entsprechende [\<headers>](../configure-apps/file-schema/wcf/headers-element.md) in der Konfigurationsdatei verfügen.
