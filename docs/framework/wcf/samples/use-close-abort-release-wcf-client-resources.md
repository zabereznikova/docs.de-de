...
Dieses Beispiel veranschaulicht die Verwendung der Methoden `Close` und `Abort` zum Bereinigen von Ressourcen, wenn ein typisierter Client verwendet wird. Die `using`-Anweisung verursacht Ausnahmen, wenn die Netzwerkverbindung nicht stabil ist. Dieses Beispiel basiert auf den ersten [Schritten, mit](../../../../docs/framework/wcf/samples/getting-started-sample.md) denen ein Rechner Dienst implementiert wird. In diesem Beispiel ist der Client eine Konsolenanwendung (.exe), und der Dienst wird von IIS (Internet Information Services, Internetinformationsdienste) gehostet.
