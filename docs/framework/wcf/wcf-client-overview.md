...
## <a name="obtain-the-service-contract-bindings-and-addresses"></a>Abrufen von Dienstvertrag, Bindungen und Adressen  
 In WCF modellieren Dienste und Clients Verträge mithilfe verwalteter Attribute, Schnittstellen und Methoden. Soll eine Verbindung zu einem Dienst in einer Clientanwendung hergestellt werden, muss der Informationstyp für den Dienstvertrag abgerufen werden. In der Regel verwenden Sie hierfür das [Service Model Metadata Utility-Tool (Svcutil. exe)](../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md), das Metadaten aus dem Dienst herunterlädt, Sie in eine verwaltete Quell Code Datei in der Sprache Ihrer Wahl umwandelt und eine Client Anwendungs Konfigurationsdatei erstellt. die Sie zum Konfigurieren des WCF-Client Objekts verwenden können. Wenn Sie z. b. ein WCF-Client Objekt zum Aufrufen `MyCalculatorService`von erstellen und wissen, dass die Metadaten für diesen Dienst unter `http://computerName/MyCalculatorService/Service.svc?wsdl`veröffentlicht werden, wird im folgenden Codebeispiel gezeigt, wie Sie mit "Svcutil. exe" eine `ClientCode.vb` Datei abrufen, die enthält den Dienstvertrag in verwaltetem Code.  
...
```  
 ...
 Ein Beispiel für diesen Prozess finden [Sie unter Gewusst wie: Erstellen Sie einen](../../../docs/framework/wcf/how-to-create-a-wcf-client.md)-Client. Ausführlichere Informationen zu Verträgen finden Sie unter [Verträge](../../../docs/framework/wcf/feature-details/contracts.md).  
  
## <a name="create-a-wcf-client-object"></a>Erstellen eines WCF-Clientobjekts  
 Ein WCF-Client ist ein lokales Objekt, das einen WCF-Dienst in einer Form darstellt, die der Client für die Kommunikation mit dem Remote Dienst verwenden kann. WCF-Client Typen implementieren den Ziel Dienstvertrag. Wenn Sie also einen erstellen und konfigurieren, können Sie das Client Objekt direkt zum Aufrufen von Dienst Vorgängen verwenden. Die WCF-Laufzeit konvertiert die Methodenaufrufe in Nachrichten, sendet Sie an den Dienst, lauscht auf die Antwort und gibt diese Werte als Rückgabewerte oder `out` `ref` Parameter an das WCF-Client Objekt zurück.  
  
 Sie können auch WCF-Client Kanal Objekte verwenden, um eine Verbindung mit Diensten herzustellen und diese zu verwenden. Weitere Informationen finden Sie unter [WCF-Client Architektur](../../../docs/framework/wcf/feature-details/client-architecture.md).  
...
> Wenn Sie den WCF-Client mithilfe von Visual Studio erstellen, werden Objekte automatisch in den Objektkatalog geladen, wenn Sie dem Projekt einen Dienst Verweis hinzufügen.  
 ...
 Wenn Sie Visual Studio nicht verwenden, überprüfen Sie den generierten Vertrags Code, um den Typ zu <xref:System.ServiceModel.ClientBase%601> finden, der erweitert, `ISampleService`und die Dienstvertragschnittstelle. In diesem Fall entspricht der Typ dem folgenden Code:  
 ...
 Es wird empfohlen, zuerst das WCF-Client Objekt zu erstellen und es dann zu verwenden und in einem einzelnen try/catch-Block zu schließen. Verwenden Sie die `using` -Anweisung (`Using` in Visual Basic) nicht, da Ausnahmen in bestimmten Fehlermodi möglicherweise maskiert werden. Weitere Informationen finden Sie in den folgenden Abschnitten sowie [unter schließen und Abbrechen, um WCF-Client Ressourcen freizugeben](../../../docs/framework/wcf/samples/use-close-abort-release-wcf-client-resources.md).  
  
### <a name="contracts-bindings-and-addresses"></a>Verträge, Bindungen und Adressen  
 Bevor Sie ein WCF-Client Objekt erstellen können, müssen Sie das-Client Objekt konfigurieren. Insbesondere muss Sie über einen Dienst *Endpunkt* verfügen, der verwendet werden kann. Ein Endpunkt ist die Kombination eines Dienstvertrags, einer Bindung und einer Adresse. (Weitere Informationen zu Endpunkten finden [Sie unter Endpunkte: Adressen, Bindungen und Verträge](../../../docs/framework/wcf/feature-details/endpoints-addresses-bindings-and-contracts.md).) Diese Informationen befinden sich in der [ \<Regel im Endpunkt >](../../../docs/framework/configure-apps/file-schema/wcf/endpoint-of-client.md) Element in einer Konfigurationsdatei der Client Anwendung, z. b. die vom Tool "Svcutil. exe" generierte Informationen, die beim Erstellen des Client Objekts automatisch geladen werden. Beide WCF-Client Typen verfügen auch über über Ladungen, die es Ihnen ermöglichen, diese Informationen Programm gesteuert anzugeben.  
 ...
 Mit dieser Konfigurationsdatei wird ein Zielendpunkt im `<client>`-Element angegeben. Weitere Informationen zum Verwenden mehrerer Ziel Endpunkte finden Sie unter <xref:System.ServiceModel.ClientBase%601.%23ctor%2A?displayProperty=nameWithType> oder in den <xref:System.ServiceModel.ChannelFactory%601.%23ctor%2A?displayProperty=nameWithType> Konstruktoren.  
...
- Erstellen Sie eine Instanz der Rückruf Vertrags Implementierungs Klasse, und verwenden Sie Sie zum <xref:System.ServiceModel.InstanceContext?displayProperty=nameWithType> Erstellen des-Objekts, das Sie an den WCF-Clientkonstruktor übergeben.  
...
## <a name="calling-services-asynchronously"></a>Asynchrones Aufrufen von Diensten  
 Die Art und Weise eines Vorgangsaufrufs ist ausschließlich Angelegenheit des Cliententwicklers. Dies liegt daran, dass die Nachrichten, aus denen sich ein Vorgang zusammensetzt, bei der Darstellung in verwaltetem Code entweder synchronen oder asynchronen Methoden zugeordnet werden können. Wenn Sie einen Client erstellen möchten, mit dem Vorgänge asynchron aufgerufen werden, können Sie daher mit Svcutil.exe und der Option `/async` asynchronen Clientcode generieren. Weitere Informationen finden Sie unter [Vorgehensweise: Dienst Vorgänge werden asynchron](../../../docs/framework/wcf/feature-details/how-to-call-wcf-service-operations-asynchronously.md)aufgerufen.  
  
## <a name="calling-services-using-wcf-client-channels"></a>Aufrufen von Diensten mithilfe der WCF-Clientkanäle.  
 WCF-Client Typen <xref:System.ServiceModel.ClientBase%601>erweitern, die selbst von <xref:System.ServiceModel.IClientChannel?displayProperty=nameWithType> der-Schnittstelle abgeleitet werden, um das zugrunde liegende Channel-System verfügbar Sie können Dienste aufrufen, indem Sie den Zieldienstvertrag mit der <xref:System.ServiceModel.ChannelFactory%601?displayProperty=nameWithType>-Klasse verwenden. Weitere Informationen finden Sie unter [WCF-Client Architektur](../../../docs/framework/wcf/feature-details/client-architecture.md).  
