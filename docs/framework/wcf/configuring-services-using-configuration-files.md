...
### <a name="the-service-element"></a>Das \<Dienst > Element  
...
### <a name="the-behavior-element"></a>Das \<Verhalten > Elements  
 Jedes `behavior` Element wird durch ein `name` -Attribut identifiziert und stellt entweder ein vom System bereitgestelltes Verhalten dar`throttling`, z. b. < > oder ein benutzerdefiniertes Verhalten. Wenn kein Name angegeben wird, entspricht dieses Verhaltenselement dem standardmäßigen Dienst- oder Endpunktverhalten.  
...
```  
 ...
 Das `name` -Attribut der `bindingConfiguration` wird im `<binding>` -Element festgelegt. Muss eine eindeutige Zeichenfolge im Bereich des Bindungs Typs sein – in diesem Fall die [<\>BasicHttpBinding](../../../docs/framework/configure-apps/file-schema/wcf/basichttpbinding.md)oder ein leerer Wert, um auf die Standard Bindung zu verweisen. `name` Der Endpunkt wird mit der Konfiguration verknüpft, indem das `bindingConfiguration` -Attribut auf diese Zeichenfolge festgelegt wird.  
...
## <a name="behavior-merge"></a>Verhaltenszusammenführung  
 ...
 Und die `~\Web.config` Datei hat folgenden Inhalt:  
...
```  
 ...
 Sie können Verhaltens Auflistungen auch löschen, \<indem Sie das >-Tag löschen verwenden und einzelne Verhalten aus der \<Auflistung entfernen, indem Sie das Tag remove > verwenden. Die folgenden beiden Konfigurationen führen beispielsweise dazu, dass der untergeordnete Dienst lediglich das serviceMetadata-Verhalten verwendet:  
