...
>
> Außerdem werden die `/r`-und `/ct` Schalter für Verweis Typen zum Erstellen von Daten Verträgen dienen. Diese Schalter funktionieren nicht, wenn Sie XmlSerializer verwenden.
...
|/targetClientVersion|Geben Sie an, auf welche Version von .NET Framework die Anwendung abzielt. Gültige Werte sind `Version30` und `Version35`. Der Standardwert ist `Version30`.<br /><br /> Kurzform: `/tcv`.<br /><br /> `Version30`: Verwenden Sie `/tcv:Version30`, wenn Sie Code für Clients erzeugen, die WinFX verwenden.<br /><br /> `Version35`: Verwenden Sie `/tcv:Version35`, wenn Sie Code für Clients erzeugen, die .NET Framework 3,5 verwenden. Wenn Sie `/tcv:Version35` mit dem Schalter `/async` verwenden, werden ereignisbasierte und rückruf-/delegatbasierte asynchrone Methoden generiert. Außerdem wird damit Unterstützung für LINQ-aktivierte DataSets und <xref:System.DateTimeOffset> aktiviert.|
