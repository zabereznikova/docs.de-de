...
```  
 ...
 Anschließend kann im [\<System. Service Model >](../../configure-apps/file-schema/wcf/system-servicemodel.md) Konfigurations Abschnitt darauf verwiesen werden.  
...
#### <a name="wsdl-export"></a>WSDL-Export  
 Um Adressierungs Informationen zu exportieren, implementiert das `UdpTransportBindingElement` die <xref:System.ServiceModel.Description.IWsdlExportExtension?displayProperty=nameWithType>-Schnittstelle. Die <xref:System.ServiceModel.Description.IWsdlExportExtension.ExportEndpoint%2A?displayProperty=nameWithType>-Methode fügt dem WSDL-Port die richtigen Adressierungsinformationen hinzu.  
...
1. Zeigen Sie Svcutil. exe mithilfe der Datei/SvcutilConfig:\<Datei > auf die Konfigurationsdatei.  
