...
-   WPF unterstützt das Konzept zusammengeführter Ressourcenwörterbücher.  Schlüssel können zwischen den zusammengeführten Wörterbüchern freigegeben sein, und auf das Verhalten der freigegebenen Schlüssel kann mit <xref:System.Windows.FrameworkContentElement.FindResource%2A> zugegriffen werden.  Weitere Informationen finden Sie unter [Zusammengeführte Ressourcenwörterbücher](../../../ocs/framework/wpf/advanced/merged-resource-dictionaries.md).  
 ...
 Schlüssel und Namen verwenden unterschiedliche Direktiven und Sprachelemente \(`x:Key` gegenüber `x:Name`\) in der grundlegenden XAML\-Definition.  Schlüssel und Namen werden auch in unterschiedlichen Situationen von der WPF\-Definition und Anwendung dieser Konzepte genutzt.  Ausführliche Informationen finden Sie unter [WPF\-XAML\-Namescopes](../../../ocs/framework/wpf/advanced/wpf-xaml-namescopes.md).  
  
 Wie zuvor angegeben, kann der Wert eines Schlüssels durch eine Markuperweiterung angegeben werden und braucht kein Zeichenfolgenwert zu sein.  Bei einem WPF\-Szenario kann der `x:Key`\-Wert beispielsweise ein [ComponentResourceKey](../../../ocs/framework/wpf/advanced/componentresourcekey-markup-extension.md) sein.  Bestimmte Steuerelemente machen einen Stilschlüssel dieses Typs verfügbar, mit dem eine benutzerdefinierte Stilressource erstellt werden kann, die einen Teil des Aussehens und Verhaltens dieses Steuerelements beeinflusst, ohne den Stil vollständig zu ersetzen.  Ein Beispiel für einen solchen Schlüssel ist <xref:System.Windows.Controls.ToolBar.ButtonStyleKey%2A>.  
  
 Die zusammengeführte Wörterbuchfunktion von WPF führt weitere Überlegungen zur Schlüsseleindeutigkeit und zum Schlüsselsuchverhalten ein.  Weitere Informationen finden Sie unter [Zusammengeführte Ressourcenwörterbücher](../../../ocs/framework/wpf/advanced/merged-resource-dictionaries.md).  
...
## Siehe auch  
 ...
 [Ressourcen und Code](../../../ocs/framework/wpf/advanced/resources-and-code.md)   
 [StaticResource\-Markuperweiterung](../../../ocs/framework/wpf/advanced/staticresource-markup-extension.md)