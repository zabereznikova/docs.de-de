...
## Werte\-Serialisierungsprogramme  
 ...
 Im XAML\-Typsystem und für XAML\-Writer\-Implementierungen, die die Verarbeitung von XAML\-Knotenschleifen für die Serialisierung verwenden, wird ein Wertkonverter, der einem Typ oder Member zugeordnet ist, durch seine eigene <xref:System.Xaml.XamlType.ValueSerializer%2A?displayProperty=fullName>\-Eigenschaft gemeldet. Die Bedeutung für XAML\-Writer, die Serialisierungen durchführen, ist, dass wenn ein <xref:System.Xaml.XamlType.TypeConverter%2A?displayProperty=fullName> und <xref:System.Xaml.XamlType.ValueSerializer%2A?displayProperty=fullName> vorhanden sind, der Typkonverter für den Pfad zum Laden verwendet werden sollte und das Werte\-Serialisierungsprogramm für den Pfad zum Speichern. Wenn <xref:System.Xaml.XamlType.TypeConverter%2A?displayProperty=fullName> vorhanden ist, aber <xref:System.Xaml.XamlType.ValueSerializer%2A?displayProperty=fullName>`null` ist, wird der Typkonverter auch für den Pfad zum Speichern verwendet.  
