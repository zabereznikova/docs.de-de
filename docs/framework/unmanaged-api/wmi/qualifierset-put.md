...
`pVal` [in] Ein Zeiger auf ein gültiges `VARIANT` , enthält den Qualifizierer, der zu schreiben. Dieser Parameter darf nicht sein `null`.

`lFlavor` [in] Einer der folgenden Konstanten, die die gewünschten Qualifizierern für diesen Qualifizierer definiert. Der Standardwert ist `WBEM_FLAVOR_OVERRIDABLE` (0).
