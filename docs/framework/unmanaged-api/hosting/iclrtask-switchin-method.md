...
|-------------|-----------------|  
|S_OK|`SwitchIn` erfolgreich zurückgegeben.|  
...
|HOST_E_INVALIDOPERATION|`SwitchIn` wurde ohne einen früheren Aufruf der [SwitchOut-Methode](../../../../docs/framework/unmanaged-api/hosting/iclrtask-switchout-method.md)aufgerufen.|  
  
## <a name="remarks"></a>Hinweise  
 Der `threadHandle`-Parameter stellt ein Handle für den Betriebssystem Thread dar, auf dem der von der aktuellen `ICLRTask`-Instanz dargestellte Task geplant wurde. Wenn in diesem Thread ein Identitätswechsel aufgetreten ist, müssen Sie [IHostSecurityManager:: revertdeself](../../../../docs/framework/unmanaged-api/hosting/ihostsecuritymanager-reverttoself-method.md) vor dem Umschalten in der Aufgabe anrufen.  
  
> [!NOTE]
> Ein `SwitchIn`-Aufrufe ohne einen früheren `SwitchOut` Fehler mit einem HRESULT-Wert von HOST_E_INVALIDOPERATION.  
