...
## <a name="remarks"></a>Hinweise  
 `SetMaxThreads` stellt der CLR eine Gelegenheit zum Festlegen der maximalen Anzahl von Threads zur Verfügung, die für Service Requests an e/a-Ports verfügbar sind. Ein Host benötigt möglicherweise exklusive Kontrolle über die Größe des Thread Pools, wie z. b. Implementierung, Leistung oder Skalierbarkeit. Aus diesem Grund ist es nicht erforderlich, dass der Host `SetMaxThreads`implementiert. In diesem Fall sollte ein Host E_NOTIMPL von dieser Methode zurückgeben.  
