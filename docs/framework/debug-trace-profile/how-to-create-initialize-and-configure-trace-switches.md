...
## Konfigurieren von Ablaufverfolgungsschaltern  
 ...
 Zusätzlich zum Angeben des Namens eines Schalters, der konfiguriert werden soll, müssen Sie auch einen Wert für den Schalter angeben. Dieser Wert ist eine ganze Zahl. Für <xref:System.Diagnostics.BooleanSwitch> entspricht der Wert 0 **Deaktiviert**, und ein Wert ungleich null entspricht **Aktiviert**. Für <xref:System.Diagnostics.TraceSwitch> entsprechen 0,1,2,3 bzw. 4 **Deaktiviert**, **Fehler**, **Warnung**, **Info** bzw. **Ausführlich**. Jede Zahl, die größer als 4 ist, wird als **Ausführlich**, und jede Anzahl, kleiner als 0 ist, wird als **Deaktiviert** angesehen.  
...
1.  Um Ablaufverfolgungsschalter zu verwenden, müssen Sie diese zuerst erstellen und sie in Ihrem Code platzieren, wie im Abschnitt [So erstellen und initialisieren Sie einen Ablaufverfolgungsschalter](#create) beschrieben.  
  
2.  Enthält Ihr Projekt keine Konfigurationsdatei \("app.config" oder "Web.config"\), wählen Sie im Menü **Projekt** den Befehl **Neues Element hinzufügen** aus.  
    ...
    -   **Visual C\#:** Klicken Sie im Dialogfeld **Neues Element hinzufügen** auf **XML\-Datei**. Geben Sie dieser Datei den Namen **app.config**. Fügen Sie im XML\-Editor nach der XML\-Deklaration die folgende XML hinzu:  
...
4.  Wenn Sie einen **BooleanSwitch**, etwa `DataMessagesSwitch` im vorhergehenden Beispiel, aktivieren möchten, ändern Sie **value** in eine beliebige ganze Zahl ungleich 0.  
  
5.  Wenn Sie einen **TraceSwitch**, etwa `TraceLevelSwitch` im vorhergehenden Beispiel, aktivieren möchten, ändern Sie **value** in die entsprechende Ebeneneinstellung \(1 bis 4\).  
