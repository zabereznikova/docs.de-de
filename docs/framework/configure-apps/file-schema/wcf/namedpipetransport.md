...
|-------------|-----------------|  
|ChannelInitializationTimeout|Dient zum Abrufen oder <xref:System.TimeSpan> Festlegen eines-Werts, der die maximale Zeit bestimmt, die ein Kanal im Initialisierungs Status sein kann, bevor er getrennt wird.|  
...
|maxBufferPoolSize|Ruft die maximale Größe (in Bytes) aller Pufferpools ab, die vom Transport verwendet werden, oder legt diese fest.|  
...
|maxPendingAccepts|Ruft die maximale Anzahl von Kanälen ab, die ein Dienst auf einen Listener warten kann, um eingehende Verbindungen mit dem Dienst zu verarbeiten, oder legt diese fest.|  
