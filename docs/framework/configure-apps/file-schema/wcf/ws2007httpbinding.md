...
|`messageEncoding`|Definiert den Encoder, der verwendet wird, um die SOAP\-Nachricht zu codieren.  Folgende Werte sind gültig:<br /><br /> -   `Text`: Verwenden Sie einen Textnachrichtenencoder.<br />-   `Mtom`: Verwenden Sie einen MTOM\-Encoder \(Message Transmission Organisation Mechanism 1.0\).<br /><br /> Die Standardeinstellung ist `Text`.<br /><br /> Dieses Attribut ist vom Typ <xref:System.ServiceModel.WSMessageEncoding>.|  
...
|[\<readerQuotas\>](../Topic/%3CreaderQuotas%3E.md)|Definiert die Beschränkungen bezüglich der Komplexität von SOAP\-Nachrichten, die von Endpunkten verarbeitet werden können, die mit dieser Bindung konfiguriert sind.  Dieses Element ist vom Typ <xref:System.ServiceModel.Configuration.XmlDictionaryReaderQuotasElement>.|  
