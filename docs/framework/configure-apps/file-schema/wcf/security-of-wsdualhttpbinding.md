...
Definiert die Sicherheitsfunktionen des [\<WSDualHttpBinding->](wsdualhttpbinding.md).  
...
|---------------|-----------------|  
|Modus|Optionale. Gibt den angewendeten Sicherheitstyp an. Der Standardwert ist `Message`sein. Dieses Attribut ist vom Typ <xref:System.ServiceModel.WSDualHttpSecurityMode>.|  
...
|Nachricht|Sicherheit wird über die SOAP-Nachrichtensicherheit bereitgestellt.|  
