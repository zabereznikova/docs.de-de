...
Sie können eine ASP.NET-Anwendung so konfigurieren, dass Sie Kompatibilitäts Einstellungen verwendet, indem Sie [\<Add>](../appsettings/add-element-for-appsettings.md) dem- [\<appSettings>](../appsettings/index.md) Abschnitt der Datei Web. config ein-Element hinzufügen.
