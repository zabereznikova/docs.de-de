...
## <a name="the-window-class"></a>Die Fensterklasse  
 ...
 Der *nicht-Client Bereich* eines Fensters wird von [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] implementiert und enthält die Teile eines Fensters, die für die meisten Fenster gelten, einschließlich der folgenden:  
...
```  
 ...
 Weitere Informationen zum entwickeln [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] von Anwendungen finden Sie unter [Building a WPF Application](building-a-wpf-application-wpf.md).  
...
### <a name="opening-a-window"></a>Öffnen eines Fensters  
 ...
 Wenn ein Fenster instanziiert wird, wird automatisch ein Verweis darauf zu einer Liste von Fenstern hinzugefügt, die vom <xref:System.Windows.Application> -Objekt verwaltet wird (siehe <xref:System.Windows.Application.Windows%2A?displayProperty=nameWithType>). Außerdem wird das erste zu instanziierte Fenster standardmäßig <xref:System.Windows.Application> als Hauptanwendungsfenster festgelegt (siehe <xref:System.Windows.Application.MainWindow%2A?displayProperty=nameWithType>).  
...
>  <xref:System.Windows.Window.ShowDialog%2A>wird aufgerufen, um Fenster (z. b. Dialogfelder Modal) zu öffnen. Weitere Informationen finden Sie unter [Übersicht über Dialog Felder](dialog-boxes-overview.md) .  
 ...
 Wenn die Anwendung gestartet wird, wird das Fenster, das durch <xref:System.Windows.Application.StartupUri%2A> den Wert von angegeben wird, modelblily geöffnet. intern wird das Fenster <xref:System.Windows.Window.Show%2A> durch Aufrufen der zugehörigen-Methode geöffnet.  
  
<a name="Ownership"></a>   
#### <a name="window-ownership"></a>Besitz von Fenstern  
 Ein Fenster, das mit der <xref:System.Windows.Window.Show%2A> -Methode geöffnet wird, verfügt nicht über eine implizite Beziehung zu dem Fenster, das es erstellt hat. Benutzer können mit einem der beiden Fenster interagieren, was bedeutet, dass beide Fenster folgende Aktionen ausführen können:  
...
- Eine Schaltfläche **Schließen** (in der Regel in einem nicht modalem Dialogfeld).  
 ...
 An <xref:System.Windows.Window.Closing> den Ereignishandler wird ein <xref:System.ComponentModel.CancelEventArgs>übermittelt, der `Boolean` die <xref:System.ComponentModel.CancelEventArgs.Cancel%2A> Eigenschaft implementiert, die `true` Sie auf festlegen, um zu verhindern, dass ein Fenster geschlossen wird.  
...
## <a name="window-location"></a>Fensterposition  
 ...
 Sie können auch den ursprünglichen Speicherort <xref:System.Windows.Window> eines angeben, wenn er zum ersten Mal angezeigt wird, indem Sie die <xref:System.Windows.Window.WindowStartupLocation%2A> - <xref:System.Windows.WindowStartupLocation> Eigenschaft mit einem der folgenden Enumerationswerte festlegen:  
...
- <xref:System.Windows.WindowStartupLocation.Manual>  
 ...
 Wenn der Start <xref:System.Windows.WindowStartupLocation.Manual>Speicherort als angegeben wird und die-Eigenschaft und die- <xref:System.Windows.Window.Top%2A> Eigenschaft nicht <xref:System.Windows.Window> festgelegt wurden, fragt Windows nach einem Speicherort, der <xref:System.Windows.Window.Left%2A> in angezeigt wird.  
...
- <xref:System.Windows.WindowStyle.ToolWindow>  
 ...
 Sie können entweder <xref:System.Windows.Window.WindowStyle%2A> [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] mithilfe von Markup oder Code festlegen, da es unwahrscheinlich ist, dass es sich während der Lebensdauer eines Fensters ändert, wird es wahrscheinlich [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] mit Markup konfiguriert.  
  
 [!code-xaml[WindowsOverviewSnippets#WindowStyleWindowMARKUP1](~/samples/snippets/csharp/VS_Snippets_Wpf/WindowsOverviewSnippets/CSharp/WindowStyleWindow.xaml#windowstylewindowmarkup1)]  
  
#### <a name="non-rectangular-window-style"></a>Nicht rechteckiger Fensterstil  
 Es gibt auch Situationen, in denen die Rahmen <xref:System.Windows.Window.WindowStyle%2A> Stile, die Ihnen die Möglichkeit bieten, nicht ausreichen. Beispielsweise können Sie eine Anwendung mit einem nicht rechteckigen Rahmen erstellen, wie [!INCLUDE[TLA#tla_wmp](../../../../includes/tlasharptla-wmp-md.md)] z. b. verwendet.  
...
## <a name="security-considerations"></a>Sicherheitsüberlegungen  
 ...
 Darüber hinaus [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] können Fenster oder Dialogfelder nicht standardmäßig angezeigt werden. Eine Erläuterung zu den Sicherheitsüberlegungen für eigenständige Anwendungen finden Sie unter [WPF-Sicherheitsstrategie-Plattformsicherheit](../wpf-security-strategy-platform-security.md).  
