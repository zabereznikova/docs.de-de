...
### <a name="security-critical-methodology"></a>Sicherheitsrelevante Methode  
 ...
 Die sicherheitsrelevante Methode ermöglicht die Organisation von WPF-Code, der Berechtigungen in den *sicherheitskritischen Kernel*erweitert, wobei der Rest transparent ist. Das Isolieren des sicherheitskritischen Codes ermöglicht dem WPF-Engineering-Team, eine zusätzliche Sicherheitsanalyse und Quell Code Verwaltung auf dem sicherheitskritischen Kernel zu verwenden, der über die Standard Sicherheitsverfahren hinausgeht (Weitere Informationen finden Sie unter [WPF-Sicherheitsstrategie-Security Engineering](wpf-security-strategy-security-engineering.md)).  
