...
-   Definieren Sie eine Methode für die <xref:System.Windows.CoerceValueCallback> mit dem Namen `CoerceValue`. `CoerceValue` Stellt sicher, dass `Value` ist größer oder gleich `MinValue` und kleiner als oder gleich `MaxValue`.  
