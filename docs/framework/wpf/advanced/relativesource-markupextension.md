...
## <a name="remarks"></a>Hinweise  
 ...
 Im folgenden Beispiel zeigt der erste <xref:System.Windows.Controls.TextBlock> in der Elementvorlage die aktuelle Zahl an. Die zweite <xref:System.Windows.Controls.TextBlock> Bindung ist eine <xref:System.Windows.Data.MultiBinding> , die nominell zwei <xref:System.Windows.Data.Binding> -Bestandteile besitzt –: der aktuelle Datensatz ein, und eine Bindung, die absichtlich den vorherigen Datensatz verwendet, indem `{RelativeSource PreviousData}`. Anschließend berechnet ein Konverter in der <xref:System.Windows.Data.MultiBinding> den Unterschied und gibt ihn an die Bindung zurück.  
