...
## <a name="validation-callbacks"></a>Überprüfungsrückrufe  
 ...
 Im folgenden finden Sie Beispielcode für ein sehr einfaches Überprüfungs Rückruf Szenario: die Validierung, dass eine als <xref:System.Double> primitiv typisierte Eigenschaft nicht <xref:System.Double.PositiveInfinity> oder <xref:System.Double.NegativeInfinity>ist.  
...
## <a name="coerce-value-callbacks-and-property-changed-events"></a>Coerce-Wert-Rückrufe und Eigenschaftenänderungsereignisse  
 Coerce-Wert-Rückrufe übergeben die <xref:System.Windows.DependencyObject> spezifische-Instanz für Eigenschaften, <xref:System.Windows.PropertyChangedCallback> ebenso wie Implementierungen, die vom-Eigenschaften System aufgerufen werden, wenn sich der Wert einer Abhängigkeits Eigenschaft ändert. Mithilfe dieser zwei Rückrufe können Sie eine Reihe von Eigenschaften für Elemente erstellen, wobei Änderungen an einer Eigenschaft eine Umwandlung oder eine erneute Auswertung von einer anderen Eigenschaft erzwingt.  
...
> [!NOTE]
> Standardwerte von Eigenschaften werden nicht erzwungen. Ein Eigenschafts Wert, der dem Standardwert entspricht, kann auftreten, wenn ein Eigenschafts Wert noch seinen ursprünglichen Standardwert aufweist oder <xref:System.Windows.DependencyObject.ClearValue%2A>wenn andere Werte mit gelöscht werden.  
...
### <a name="using-coercevalue-to-cancel-value-changes"></a>Abbrechen von Wertänderungen mit CoerceValue  
 Das Eigenschaften System behandelt alle <xref:System.Windows.CoerceValueCallback> , die den Wert <xref:System.Windows.DependencyProperty.UnsetValue> als Sonderfall zurückgibt. Dieser Sonderfall bedeutet, dass die Eigenschaften Änderung, die dazu <xref:System.Windows.CoerceValueCallback> geführt hat, dass das aufgerufen wird, vom Eigenschaften System abgelehnt werden sollte und dass das Eigenschaften System stattdessen den vorherigen Wert der Eigenschaft melden sollte. Dieser Mechanismus kann bei der Überprüfung hilfreich sein, dass asynchron initiierte Änderungen an einer Eigenschaft für den aktuellen Objektzustand noch gültig sind, sowie bei der Unterdrückung dieser Änderungen, sofern sie nicht gültig sind. Ein anderes mögliches Szenario ist die selektive Unterdrückung eines Werts, je nachdem welche der Komponenten der Eigenschaftswertermittlung für den gemeldeten Wert verantwortlich ist. Zu diesem Zweck können Sie den <xref:System.Windows.DependencyProperty> im Rückruf und den Eigenschaften Bezeichner als Eingabe für <xref:System.Windows.DependencyPropertyHelper.GetValueSource%2A>den-Wert verwenden und dann verarbeiten <xref:System.Windows.ValueSource>.  
