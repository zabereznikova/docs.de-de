...
2. Wenn die Eigenschaft mit einer attributierten <xref:System.ComponentModel.TypeConverter>deklariert wird oder der Werttyp dieser Eigenschaft mit einem attributierten <xref:System.ComponentModel.TypeConverter>deklariert wird, wird der Zeichen folgen Wert des Attributs als Konvertierungs Eingabe an den Typkonverter übermittelt, und der Konverter gibt ein neues Objekt zurück. lichen.  
...
- Der Typ wird von <xref:System.Array> abgeleitet (Weitere Informationen zu Arrays in XAML finden Sie unter [x:Array-Markup Erweiterung](../../xaml-services/x-array-markup-extension.md).)  
...
## <a name="markup-extensions"></a>Markuperweiterungen  
 ...
 Hier identifiziert `StaticResource` die <xref:System.Windows.StaticResourceExtension> Klasse, die die Implementierung der Markup Erweiterung bereitstellt. Die nächste Zeichenfolge `MyStyle` wird als Eingabe für den nicht standardmäßigen <xref:System.Windows.StaticResourceExtension> Konstruktor verwendet, wobei der Parameter, der aus der Erweiterungs Zeichenfolge entnommen wurde, die angeforderte <xref:System.Windows.ResourceKey>deklariert. `MyStyle` wird als [x:Schlüsselwert](../../xaml-services/x-key-directive.md) eines <xref:System.Windows.Style>, der als Ressource definiert ist, erwartet. Die Verwendung der [statikresource-Markup Erweiterung](staticresource-markup-extension.md) fordert an, dass die Ressource verwendet wird, um den <xref:System.Windows.Style>-Eigenschafts Wert über die statische Ressourcen Suchlogik zur Ladezeit bereitzustellen.  
