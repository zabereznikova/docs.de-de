...
- **Imperative und deklarative Syntax**: Code, der den Common Language Runtime als Ziel hat, kann mit dem Sicherheitssystem interagieren, indem er Berechtigungen anfordert, Aufrufer anfordert, Berechtigungen festzulegen und bestimmte Sicherheitseinstellungen außer Kraft zu setzen (bei ausreichenden Berechtigungen) Für die programmgesteuerte Interaktion mit dem Sicherheitssystem von .NET Framework verwenden Sie zwei verschiedene Syntaxformen: die deklarative und die imperative Syntax. Deklarative Aufrufe werden mithilfe von Attributen ausgeführt, imperative Aufrufe werden mit neuen Instanzen von Klassen im Code ausgeführt. Einige Aufrufe können nur imperativ ausgeführt werden, andere können nur deklarativ ausgeführt werden, und einige Aufrufe können auf beide Arten ausgeführt werden.
