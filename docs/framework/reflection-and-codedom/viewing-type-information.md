---
title: Anzeigen von Typinformationen | Microsoft-Dokumentation
ms.custom: 
ms.date: 03/30/2017
...
Die Klasse <xref:System.Type?displayProperty=fullName> ist für die Reflektion wesentlich. Die Common Language Runtime (CLR) erstellt **Type** für einen geladenen Typ, wenn die Reflektion diesen anfordert. Sie können die Methoden, Felder, Eigenschaften und geschachtelten Klassen eines **Type**-Objekts dazu verwenden, alle Informationen zu diesem Typ zu erhalten.  
  
 Verwenden Sie <xref:System.Reflection.Assembly.GetType%2A?displayProperty=fullName> und <xref:System.Reflection.Assembly.GetTypes%2A?displayProperty=fullName>, um **Type**-Objekte von Assemblys abzurufen, die nicht geladen wurden, und übergeben Sie den Namen des Typs oder der Typen, die Sie möchten. Verwenden Sie <xref:System.Type.GetType%2A?displayProperty=fullName>, um **Type**-Objekte von Assemblys abzurufen, die bereits geladen wurden. Verwenden Sie <xref:System.Reflection.Module.GetType%2A?displayProperty=fullName> und <xref:System.Reflection.Module.GetTypes%2A?displayProperty=fullName>, um **Type**-Modulobjekte zu erhalten.  
  
> [!NOTE]
>  Weitere Informationen zum Untersuchen und Manipulieren von generischen Typen und Methoden finden Sie in den Artikeln [Reflection and Generic Types (Reflektion und generische Typen)](../../../docs/framework/reflection-and-codedom/reflection-and-generic-types.md) und [How to: Examine and Instantiate Generic Types with Reflection (Vorgehensweise: Untersuchen und Instanziieren von generischen Typen mit Reflektion)](../../../docs/framework/reflection-and-codedom/how-to-examine-and-instantiate-generic-types-with-reflection.md).  
 ...
 Außerdem können Sie Methoden auf die **Type**-Klasse anwenden, um Informationen zu einem oder mehreren Konstruktoren, Methoden, Ereignissen, Felder oder Eigenschaften abzurufen, die Sie durch Namen angeben. <xref:System.Type.GetConstructor%2A?displayProperty=fullName> kapselt z.B. einen spezifischen Konstruktor für die aktuelle Klasse.  
  
 Wenn Sie ein **Type**-Objekt haben, können Sie die <xref:System.Type.Module%2A?displayProperty=fullName>-Eigenschaft verwenden, um ein Objekt zu erhalten, das das Modul kapselt, das den Typ enthält. Verwenden Sie die <xref:System.Reflection.Module.Assembly%2A?displayProperty=fullName>-Eigenschaft, um ein Objekt zu finden, das die Assembly kapselt, die das Modul enthält. Sie können die Assembly, die den Typ kapselt, direkt mithilfe der <xref:System.Type.Assembly%2A?displayProperty=fullName>-Eigenschaft erhalten.  
