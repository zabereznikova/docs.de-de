---
title: "Sicherheitsaspekte bei der Reflektionsausgabe | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "Anonym gehostete dynamische Methoden [.NET Framework]"
  - "Dynamische Assemblys, Sicherheit"
  - "Ausgeben von dynamischen Assemblys, Sicherheit"
  - "Ausgeben von dynamischen Assemblys, Szenarios mit teilweiser Vertrauenswürdigkeit"
  - "Teilweise vertrauenswürdig, Ausgeben von dynamischen Methoden"
  - "Teilweise vertrauenswürdiger Code"
  - "Reflektionsausgabe, Szenarios mit teilweiser Vertrauenswürdigkeit"
  - "Reflektionsausgabe, Sicherheit"
...
## Dynamische Assemblys  
 Dynamische Assemblys werden unter Verwendung von Überladungen der <xref:System.AppDomain.DefineDynamicAssembly%2A?displayProperty=fullName>\-Methode erstellt.  Die meisten Überladungen dieser Methode werden in [!INCLUDE[net_v40_long](../../../includes/net-v40-long-md.md)] nicht mehr unterstützt, weil computerweite Sicherheitsrichtlinien beseitigt wurden.  \(Siehe [Sicherheitsänderungen](../../../docs/framework/security/security-changes.md).\) Die verbleibenden Überladungen können von jedem Code, unabhängig von der Vertrauensebene, ausgeführt werden.  Diese Überladungen fallen in zwei Gruppen: solche, die eine Liste mit Attributen angeben, die auf die dynamische Assembly angewendet werden sollen, wenn sie erstellt wird, und solche, die dies unterlassen.  Wenn Sie das Transparenzmodell für die Assembly, wenn Sie sie erstellen, nicht durch Anwendung des <xref:System.Security.SecurityRulesAttribute>\-Attributs angeben, wird das Transparenzmodell von der ausgebenden Assembly geerbt.  
...
> [!NOTE]
>  Dynamische Assemblys verwenden nicht die Flags <xref:System.Security.Permissions.ReflectionPermissionFlag?displayProperty=fullName> und <xref:System.Security.Permissions.ReflectionPermissionFlag?displayProperty=fullName>, die es dynamischen Methoden gestatten, auf nicht öffentliche Typen und Member zuzugreifen.  
...
-   Wenn Sie angeben, dass JIT\-Sichtbarkeitsprüfungen übersprungen werden sollen, erfordert der Konstruktor <xref:System.Security.Permissions.ReflectionPermission> mit dem <xref:System.Security.Permissions.ReflectionPermissionFlag?displayProperty=fullName>\-Flag.  
  
-   Wenn Sie die dynamische Methode einem anderen Typ zuordnen, auch wenn es ein anderer Typ in Ihrer eigenen Assembly sein sollte, erfordert der Konstruktor <xref:System.Security.Permissions.ReflectionPermission> mit dem <xref:System.Security.Permissions.ReflectionPermissionFlag?displayProperty=fullName>\-Flag sowie <xref:System.Security.Permissions.SecurityPermission> mit dem <xref:System.Security.Permissions.SecurityPermissionFlag?displayProperty=fullName>\-Flag.  
  
-   Wenn Sie die dynamische Methode einem Typ oder Modul in einer anderen Assembly zuordnen, erfordert der Konstruktor zwei Dinge: <xref:System.Security.Permissions.ReflectionPermission> mit dem <xref:System.Security.Permissions.ReflectionPermissionFlag?displayProperty=fullName>\-Flag sowie den Berechtigungssatz der Assembly, die das andere Modul enthält.  Das heißt, Ihre Aufrufliste muss alle Berechtigungen aus dem Berechtigungssatz des Zielmoduls enthalten, zuzüglich <xref:System.Security.Permissions.ReflectionPermissionFlag?displayProperty=fullName>.  
  
    > [!NOTE]
    >  Um Abwärtskompatibilität zu gewährleisten, wenn die Anforderung des Zielberechtigungssatzes zuzüglich <xref:System.Security.Permissions.ReflectionPermissionFlag?displayProperty=fullName> fehlschlägt, erfordert der Konstruktor <xref:System.Security.Permissions.SecurityPermission> mit dem <xref:System.Security.Permissions.SecurityPermissionFlag?displayProperty=fullName>\-Flag.  
