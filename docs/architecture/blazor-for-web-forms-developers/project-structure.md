...
Blazor-apps verfügen über eine- `Startup` Klasse anstelle einer *Global. asax* -Datei, um die Start Logik für die APP zu definieren. Die `Startup` -Klasse wird verwendet, um die APP und alle App-spezifischen Dienste zu konfigurieren. In der blazor-Server-APP `Startup` wird die-Klasse verwendet, um den Endpunkt für die Echtzeitverbindung einzurichten, die von blazor zwischen den Client Browsern und dem Server verwendet wird. In der blazor Webassembly `Startup` -App definiert die-Klasse die Stamm Komponenten für die APP und wo Sie gerendert werden sollen. Wir werfen einen tieferen Einblick in die- `Startup` Klasse im Abschnitt [App-Start](./app-startup.md) .
...
Blazor Webassembly-apps unterstützen das Debuggen mit Visual Studio nicht. Verwenden Sie anstelle von, um die APP auszuführen `Ctrl+F5` `F5` . Stattdessen können Sie blazor-webassemblyanwendungen direkt im Browser Debuggen. Weitere Informationen finden Sie unter [Debug ASP.net Core blazor](/aspnet/core/blazor/debug) .
