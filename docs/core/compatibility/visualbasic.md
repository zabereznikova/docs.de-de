---
title: Breaking Changes bei Visual Basic
description: Hier werden die Breaking Changes in Visual Basic für .NET Core 3.0 aufgelistet.
ms.date: 09/20/2019
ms.openlocfilehash: b814edbf3a3a58d37b0a2b66a0e1c4f680f14b57
ms.sourcegitcommit: d8020797a6657d0fbbdff362b80300815f682f94
ms.translationtype: HT
ms.contentlocale: de-DE
ms.lasthandoff: 11/24/2020
ms.locfileid: "95713144"
---
# <a name="visual-basic-breaking-changes-in-net-core-30"></a>Breaking Changes in Visual Basic für .NET Core 3.0

Im Folgenden werden die Breaking Changes bei Visual Basic aufgeführt:

- [Microsoft.VisualBasic.Constants.vbNewLine ist veraltet](#microsoftvisualbasicconstantsvbnewline-is-obsolete)

## <a name="net-core-30"></a>.NET Core 3.0

[!INCLUDE[vbNewLine is obsolete](~/includes/core-changes/visualbasic/3.0/vbnewline-is-obsolete.md)]
