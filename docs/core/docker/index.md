---
title: Docker und .NET Core
description: Erfahren Sie in diesen Tutorials mehr zur Verwendung von Docker mit .NET Core.
author: jralexander
ms.author: johalex
ms.date: 01/10/2018
ms.openlocfilehash: e31a3a221604e7aca7d35a4826e78ade0f980a6b
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: HT
ms.contentlocale: de-DE
ms.lasthandoff: 05/04/2018
---
# <a name="docker-and-net-core"></a>Docker und .NET Core 

Die folgenden Tutorials sind verfügbar, um etwas über die Verwendung von Docker mit .NET Core zu erfahren.

- [Einführung in .NET and Docker](intro-net-docker.md)
- [Erlernen der Docker-Grundlagen mit .NET Core](docker-basics-dotnet-core.md)
- [Erstellen von Docker-Images für .NET Core-Anwendungen](building-net-docker-images.md)
- [Visual Studio-Tools für Docker](/aspnet/core/host-and-deploy/docker/visual-studio-tools-for-docker)

Tutorials über das Entwickeln von ASP.NET Core-Webanwendungen finden Sie in der [ASP.NET Core-Dokumentation](/aspnet/core/).
