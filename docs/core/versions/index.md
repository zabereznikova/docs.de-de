...
description: "Erfahren Sie mehr über das Versionsschema für .NET Core."
...
- Beschreibt die Pakete, die für die moderne App-Entwicklung als erforderlich gelten und für .NET-Implementierungen implementiert werden müssen, um als [.NET Standard](../../standard/net-standard.md)-Plattform zu gelten.
...
Das `netcoreapp`-Framework ist an die .NET Core-Verteilung gebunden. Es befolgt die Versionsnummern, die von .NET Core verwendet werden. Wenn z.B. .NET Core 2.0 veröffentlicht wird, wird es auf `netcoreapp2.0` abzielen. Das `netstandard`-Framework entspricht nicht dem Schema der Versionierung einer beliebigen .NET-Implementierung, da es gleichmäßig auf alle Laufzeiten angewendet werden kann.
