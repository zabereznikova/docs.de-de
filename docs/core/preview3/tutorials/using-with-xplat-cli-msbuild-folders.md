...
3. Initialisieren Sie das Verzeichnis mit dem Befehl `dotnet new -t Xunittest`. Hierbei wird von Xunit ausgegangen. Sie können aber auch MS Test nutzen, indem Sie `Xunittest` durch `Mstest` ersetzen.
...
1. Eine ordnungsgemäße Datei `NewTypesTests.csproj`, die Folgendes enthält:

   * Einen Verweis auf `xunit`
   * Einen Verweis auf `dotnet-test-xunit`
   * Einen Verweis auf den Namespace für den zu testenden Code

   Dies kann erstellt werden, indem Sie im Verzeichnis `NewTypesTests` einfach `dotnet new -t Xunittest` an der Befehlszeile ausführen und dann einen Projektverweis zum Projekt `NewTypes` hinzufügen.
