...
1.  Ermitteln Sie, ob eine Konvertierung eines beliebigen Typs von `<type1>` in `<type2>` vorhanden ist. Wenn beide Type elementare Typen von [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] oder Instanzen von Klassen sind, können Sie diese Ermittlung in der Regel über die Tabelle in [Widening and Narrowing Conversions](../../visual-basic/programming-guide/language-features/data-types/widening-and-narrowing-conversions.md) vornehmen.  
