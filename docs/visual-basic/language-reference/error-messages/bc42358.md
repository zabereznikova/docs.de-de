...
---
# <a name="because-this-call-is-not-awaited-execution-of-the-current-method-continues-before-the-call-is-completed"></a>Da auf diesen-Befehl nicht gewartet wird, wird die Ausführung der aktuellen Methode vor Abschluss des Aufrufes fortgesetzt.
...
Die aktuelle Methode ruft eine asynchrone Methode auf, die eine <xref:System.Threading.Tasks.Task> oder eine <xref:System.Threading.Tasks.Task%601> zurückgibt und den [Erwartungs Operator nicht](../operators/await-operator.md) auf das Ergebnis anwendet. Der Aufruf der asynchronen Methode startet eine asynchrone Aufgabe. Da jedoch kein `Await`-Operator angewendet wird, wird das Programm fortgesetzt, ohne auf den Abschluss der Aufgabe zu warten. Dieses Verhalten wird in den meisten Fällen nicht erwartet. Andere Aspekte der aufrufenden Methode hängen in der Regel von den Ergebnissen des Aufrufs ab, mindestens jedoch wird erwartet, dass die aufgerufene Methode abgeschlossen wird, bevor Sie von der Methode zurückgegeben, die den Aufruf enthält.
