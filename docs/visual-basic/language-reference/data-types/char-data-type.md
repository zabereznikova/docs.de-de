...
- **Tet.** Der `Char` -Datentyp wird zu `String`erweitert. Dies bedeutet, dass Sie `Char` in `String` konvertieren können <xref:System.OverflowException?displayProperty=nameWithType> und keinen Fehler finden.
