...
Um ein `Date`-Literal in das Format Ihres Gebietsschemas oder in ein benutzerdefiniertes Format zu konvertieren, müssen Sie das Literal für die <xref:Microsoft.VisualBasic.Strings.Format%2A>-Funktion bereitstellen und entweder ein vordefiniertes oder ein benutzerdefiniertes Datumsformat angeben. Das wird im folgenden Beispiel veranschaulicht.
...
- **Geben Sie Zeichen ein.** `Date`weist kein Literaltyp Zeichen oder Bezeichnertyp Zeichen auf. Der Compiler behandelt jedoch Literale, die in Nummernzeichen (`# #`) eingeschlossen sind, als `Date`.
...
- [Datentypen](index.md)
- [Standard-Formatzeichenfolgen für Datum und Uhrzeit](../../../standard/base-types/standard-date-and-time-format-strings.md)
