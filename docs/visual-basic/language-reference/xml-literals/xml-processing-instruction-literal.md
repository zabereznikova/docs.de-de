...
## <a name="parts"></a>Teile  
 `<?`  
 Erforderlich. Denotes the start of the XML processing instruction literal.  
  
 `piName`  
 Erforderlich. Name indicating which application the processing instruction targets. Cannot begin with "xml" or "XML".  
  
 `piData`  
 Dies ist optional. String indicating how the application targeted by `piName` should process the XML document.  
  
 `?>`  
 Erforderlich. Denotes the end of the processing instruction.  
