...
## <a name="parts"></a>Teile  
 `On`  
 Dies ist optional. Ermöglicht `Option Explicit` Überprüfung. Wenn `On` oder `Off` nicht angegeben ist, wird der Standardwert `On`.  
  
 `Off`  
 Dies ist optional. Deaktiviert die `Option Explicit` Überprüfung.  
