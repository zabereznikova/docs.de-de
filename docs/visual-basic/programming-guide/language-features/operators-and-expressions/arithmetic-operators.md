...
### Versuchte Division durch 0 \(null\)  
 Die Division durch 0 kann abhängig von den verwendeten Datentypen unterschiedliche Ergebnisse erzielen.  Bei der ganzzahligen Division \(`SByte`, `Byte`, `Short`, `UShort`, `Integer`, `UInteger`, `Long`, `ULong`\), löst [!INCLUDE[dnprdnshort](~/includes/dnprdnshort-md.md)] eine <xref:System.DivideByZeroException>\-Ausnahme aus.  Bei Divisionen mit dem `Decimal`\-Datentyp oder dem `Single`\-Datentyp löst [!INCLUDE[dnprdnshort](~/includes/dnprdnshort-md.md)] ebenfalls eine <xref:System.DivideByZeroException>\-Ausnahme aus.  
