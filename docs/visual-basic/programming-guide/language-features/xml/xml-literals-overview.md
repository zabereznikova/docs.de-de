...
Ein *XML-Literal* ermöglicht die Integration von XML-Daten direkt in Ihre [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] Code. Die XML-literal-Syntax stellt [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)] Objekte und ähnelt der XML 1.0-Syntax. Dies erleichtert die XML-Elementen und Dokumenten programmgesteuert erstellt werden, da der Code dieselbe Struktur auf wie die endgültigen XML hat.  
  
 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]XML-Literale in kompiliert [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)] Objekte. [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)]Stellt ein einfaches Objektmodell zum Erstellen und Bearbeiten von XML, und dieses Modell nahtlose Integration in [!INCLUDE[vbteclinqext](~/includes/vbteclinqext-md.md)]. Weitere Informationen finden Sie unter <xref:System.Xml.Linq.XElement>.  
  
 Sie können Einbetten einer [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] Ausdruck in einem XML-Literal. Zur Laufzeit erstellt die Anwendung eine [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)] Objekt für jedes Literal, die Werte von eingebetteten Ausdrücken einbinden. Dadurch können Sie die dynamischen Inhalt in einem XML-literal angeben. Weitere Informationen finden Sie unter [eingebettete Ausdrücke in XML](../../../../visual-basic/programming-guide/language-features/xml/embedded-expressions-in-xml.md).  
...
## <a name="simple-literals"></a>Einfache Literale  
 Können Sie erstellen eine [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)] Objekt in Ihrem [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] durch eingeben oder Einfügen von gültigen XML-Code. Gibt ein XML-Elementliteral ein <xref:System.Xml.Linq.XElement> Objekt. Weitere Informationen finden Sie unter [XML-Element Literal](../../../../visual-basic/language-reference/xml-literals/xml-element-literal.md) und [XML-Literale und XML 1.0-Spezifikation](../../../../visual-basic/programming-guide/language-features/xml/xml-literals-and-the-xml-1-0-specification.md). Das folgende Beispiel erstellt ein XML-Element, das mehrere untergeordnete Elemente verfügt.  
...
## <a name="how-the-compiler-creates-objects-from-xml-literals"></a>Der Compiler erstellt wie Objekte von XML-Literalen  
 Die [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] -Compiler übersetzt XML-Literale in Aufrufe in die entsprechende [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)] Konstruktoren zum Erstellen der [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)] Objekt. Z. B. die [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] Compiler übersetzt im folgenden Codebeispiel wird in einen Aufruf der <xref:System.Xml.Linq.XProcessingInstruction> Konstruktor für die XML-Version-Anweisung aufruft, um die <xref:System.Xml.Linq.XElement> Konstruktor für die `<contact>`, `<name>`, und `<phone>`Elemente und Aufrufe an die <xref:System.Xml.Linq.XAttribute> Konstruktor für die `type` Attribut. Insbesondere anhand der Attribute im folgenden Beispiel wird die [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] Compiler aufrufen, wird die <xref:System.Xml.Linq.XAttribute.%23ctor%28System.Xml.Linq.XName%2CSystem.Object%29> zweimal Konstruktor. Die erste übergeben den Wert `type` für die `name` -Parameter und den Wert `home` für die `value` Parameter. Die zweite übergeben den Wert auch `type` für die `name` Parameter, aber der Wert `work` für die `value` Parameter.  
