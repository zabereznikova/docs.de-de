...
6.  Wenn der Parameter optional ist, stellen Sie dem Übergabemechanismus [Optional](../../../../visual-basic/language-reference/modifiers/optional.md) voran, und geben Sie nach dem Datentyp des Parameters ein Gleichheitszeichen \(`=`\) und einen Standardwert ein.  
     ...
     Wenn der Aufrufcode keinen Wert für den `level` \-Parameter enthält, wird er von [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] auf den Standardwert 0 festgelegt.  
     ...
     Die Angabe von Datentypen für alle Programmierelemente wird als *starke Typisierung* bezeichnet.  Wenn Sie `Option Strict On` auswählen, erzwingt [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] die starke Typisierung.  Dies wird aus den folgenden Gründen dringend empfohlen:  
    ...
    -   Sie ermöglicht die schnellere Ausführung des Codes.  Ein Grund hierfür ist, dass der [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]\-Compiler dem Programmierelement den `Object`\-Datentyp zuweist, wenn Sie keinen Datentyp dafür festlegen.  Der kompilierte Code müsste zwischen `Object` und anderen Datentypen hin und her konvertieren, wodurch die Leistung reduziert wird.  
