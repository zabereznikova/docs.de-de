...
## <a name="purpose"></a>Zweck  
 Der Hauptzweck von shadowingbesteht darin, die Definition der Klassenmember zu schützen. Die Basisklasse kann einer Änderung unterzogen werden, die ein Element mit dem gleichen Namen erstellt, den Sie bereits definiert haben. Wenn dies der Fall ist, erzwingt der `Shadows`-Modifizierer, dass Verweise durch Ihre Klasse in den von Ihnen definierten Member aufgelöst werden, anstelle des neuen Basisklassen Elements.  
...
### <a name="shadowing-through-scope"></a>Shadothrough im Gültigkeitsbereich  
 ...
 Die folgende Abbildung zeigt zwei Variablen, die beide den Namen "`temp`" haben. Die lokale Variable `temp` Shadowing die Element Variable `temp`, wenn in ihrer eigenen Prozedur `p` darauf zugegriffen wird. Allerdings umgeht das Schlüsselwort "`MyClass`" den shadodown und greift auf die Element Variable zu.  
  
 ![Grafik, die shadothrough durch den Gültigkeitsbereich anzeigt.](./media/shadowing/shadow-scope-diagram.gif)
  
 Ein Beispiel für die über schattung durch den Bereich finden Sie unter [gewusst wie: Blenden Sie eine Variable mit dem gleichen Namen wie die Variable @ no__t-0 aus.  
...
### <a name="shadowing-through-inheritance"></a>Über schattung durch Vererbung  
 ...
 In der folgenden Abbildung wird eine Basisklasse `b` und eine abgeleitete Klasse `d` gezeigt, die von `b` erbt. Die-Basisklasse definiert eine Prozedur mit dem Namen `proc`, und die abgeleitete Klasse überschattet Sie mit einer anderen Prozedur desselben Namens. Die erste `Call`-Anweisung greift auf die Shadowing`proc` in der abgeleiteten Klasse zu. Allerdings umgeht das Schlüsselwort "`MyBase`" den Shadowing und greift auf die Shadowing-Prozedur in der Basisklasse zu.  
  
 ![Grafisches Diagramm des Shadowings durch Vererbung](./media/shadowing/shadowing-inherit-diagram.gif)  
  
 Ein Beispiel für shadowingthrough Vererbung finden Sie unter [gewusst wie: Blenden Sie eine Variable mit dem gleichen Namen wie die Variable @ no__t-0 und [Wie folgt aus: Blenden Sie eine geerbte Variable @ no__t-0 aus.  
...
## <a name="accessing-a-shadowed-element"></a>Zugreifen auf ein Shadowing Element  
 Wenn Sie auf ein Element aus einer abgeleiteten Klasse zugreifen, verwenden Sie normalerweise die aktuelle Instanz der abgeleiteten Klasse, indem Sie den Elementnamen mit dem Schlüsselwort "`Me`" qualifizieren. Wenn die abgeleitete Klasse ein Shadowing für das Element in der Basisklasse durchführt, können Sie auf das Basisklassen Element zugreifen, indem Sie es mit dem Schlüsselwort `MyBase` qualifizieren.  
  
 Ein Beispiel für den Zugriff auf ein Shadowing Element finden Sie unter [gewusst wie: Zugreifen auf eine Variable, die durch eine abgeleitete Klasse (@ no__t-0) ausgeblendet wird  
...
```  
 ...
 Im vorherigen Beispiel wird die Variable `basObj` als Basisklasse deklariert. Wenn Sie ein `dervCls`-Objekt zuweisen, ist dies eine erweiternde Konvertierung, die daher gültig ist. Die Basisklasse kann jedoch nicht auf die Shadowingversion der Variablen `z` in der abgeleiteten Klasse zugreifen, sodass der Compiler `basObj.z` in den ursprünglichen Basisklassen Wert auflöst.  
