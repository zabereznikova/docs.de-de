...
Windows-APIs sind Dynamic Link Libraries (DLLs), die Teil des Windows-Betriebssystems sind. Sie verwenden Sie zum Ausführen von Aufgaben, wenn es schwierig ist, gleichwertige, eigene Prozeduren zu schreiben. Windows stellt z. b. eine Funktion `FlashWindowEx` mit dem Namen zur Verfügung, mit der Sie die Titelleiste für eine Anwendung zwischen hellen und dunklen Schattierungen wechseln können.  
...
## <a name="api-calls-using-declare"></a>API-Aufrufe mithilfe von DECLARE  
 Die gängigste Methode zum Abrufen von Windows-APIs ist die `Declare` Verwendung der-Anweisung.  
...
4. Fügen Sie die `Declare` folgende Funktion entweder der Klasse oder dem Modul hinzu, in der Sie die DLL verwenden möchten:  
...
1. Fügen Sie dem Start `Button1` Formular für das Projekt eine Schaltfläche mit dem Namen hinzu, und doppelklicken Sie darauf, um den Code anzuzeigen. Der Ereignishandler für die Schaltfläche wird angezeigt.  
...
2. Fügen Sie zum Vereinfachen des `MarshalAs` Zugriffs auf das- `Imports` Attribut eine-Anweisung am Anfang des Codes für die Klasse oder das Modul hinzu, wie im folgenden Beispiel gezeigt:  
  
     [!code-vb[VbVbalrInterop#13](~/samples/snippets/visualbasic/VS_Snippets_VBCSharp/VbVbalrInterop/VB/Class1.vb#13)]  
  
3. Fügen Sie der von Ihnen verwendeten Klasse oder dem verwendeten Modul einen Funktionsprototyp für die importierte Funktion hinzu, `MarshalAs` und wenden Sie das-Attribut auf die Parameter oder den Rückgabewert an. Im folgenden Beispiel wird ein API-Befehl, der den- `void*` Typ erwartet, wie `AsAny`folgt gemarshallt:  
...
## <a name="api-calls-using-dllimport"></a>API-Aufrufe mithilfe von DllImport  
 ...
 Sie können mit `DllImport` den meisten Windows-API-aufrufen verwenden, solange der Aufruf auf eine freigegebene Methode (manchmal als *statisch*bezeichnet) verweist. Sie können keine Methoden verwenden, die eine Instanz einer Klasse erfordern. Im `Declare` Gegensatz zu `DllImport` -Anweisungen können Aufrufe `MarshalAs` das-Attribut nicht verwenden.  
...
3. Fügen Sie dem Start `Button2` Formular eine Schaltfläche mit dem Namen hinzu.  
  
4. Doppelklicken Sie `Button2` , um die Code Ansicht für das Formular zu öffnen.  
  
5. Fügen Sie zum vereinfachen `DllImport`des Zugriffs auf `Imports` eine-Anweisung am Anfang des Codes für die Startup-Formular Klasse ein:  
  
     [!code-vb[VbVbalrInterop#13](~/samples/snippets/visualbasic/VS_Snippets_VBCSharp/VbVbalrInterop/VB/Class1.vb#13)]  
  
6. Deklarieren Sie eine leere Funktion `End Class` vor der-Anweisung für das Formular, und `MoveFile`benennen Sie die Funktion.  
  
7. Wenden Sie `Public` die `Shared` Modifizierer und auf die Funktionsdeklaration an `MoveFile` , und legen Sie Parameter für basierend auf den Argumenten fest, die die Windows-API-Funktion  
...
8. Wenden Sie `DllImport` das-Attribut auf die leere Funktion an. Der erste Parameter ist der Name und Speicherort der dll, die die Funktion enthält, die Sie aufrufen. Sie müssen den Pfad für Dateien, die sich in den Windows-Systemverzeichnissen befinden, nicht angeben. Der zweite Parameter ist ein benanntes Argument, das den Namen der Funktion in der Windows-API angibt. In diesem Beispiel erzwingt `DllImport` das-Attribut, `MoveFile` dass Aufrufe von `MoveFileW` in Kernel32 weitergeleitet werden. DLL. Mit `MoveFileW` der-Methode wird eine Datei aus `src` dem Pfad in `dst`den Pfad kopiert.  
...
- [Automatisch](../../../visual-basic/language-reference/modifiers/auto.md)
